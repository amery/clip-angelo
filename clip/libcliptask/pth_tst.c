...
main(int argc, char *argv[])
{
   fprintf(stderr, "\n=== TESTING GLOBAL LIBRARY API ===\n\n");
   {
      ...
      version = pth_version();
      FAILED_IF(version == 0x0) fprintf(stderr, "version = 0x%X\n", version);
   }
   ...
   fprintf(stderr, "\n=== TESTING BASIC OPERATION ===\n\n");
   {
      ...
      rc = pth_init();
      FAILED_IF(rc == FALSE) fprintf(stderr, "Killing Pth system for testing purposes\n");
      pth_kill();
   ...
   fprintf(stderr, "\n=== TESTING BASIC THREAD OPERATION ===\n\n");
   {
      ...
      attr = pth_attr_new();
      FAILED_IF(attr == NULL) rc = pth_attr_set(attr, PTH_ATTR_NAME, "test1");
      FAILED_IF(rc == FALSE) rc = pth_attr_set(attr, PTH_ATTR_PRIO, PTH_PRIO_MAX);
      FAILED_IF(rc == FALSE) fprintf(stderr, "Spawning thread\n");
      tid = pth_spawn(attr, t1_func, (void *) (123));
      FAILED_IF(tid == NULL) pth_attr_destroy(attr);

      fprintf(stderr, "Joining thread\n");
      rc = pth_join(tid, &val);
   FAILED_IF(rc == FALSE) FAILED_IF(val != (void *) (1123))}
   ...
   fprintf(stderr, "\n=== TESTING NESTED THREAD OPERATION ===\n\n");
   {
      ...
      FAILED_IF(tid == NULL) rc = pth_join(tid, &val);
      fprintf(stderr, "Joined thread 1\n");
   FAILED_IF(rc == FALSE) FAILED_IF(val != (void *) (1 * 2 * 3 * 4 * 5 * 6 * 7 * 8 * 9))}
