	  ...
	  switch (np->isC)
	   ...
	   case 2:
	      fprintfOffs(out, level,
			  "if ( (_ret=_clip_%s(_mp, clip_%s, %d, %d, %s))) goto _trap_%d;\n",
	   ...
	   case 0:
	      fprintfOffs(out, level,
			  ...
			  np->node.isTop ? "proc" : "func",
			  (long) hashstr(np->name), np->name, np->argc, np->rest, ref_name(np), np->node.seqNo);
	      break;
	   }
	  return 0;
       }
