   ...
   switch (pass)
    ...
    case Print:
       fprintfOffs(stdout, level,
		   "FMemvar node: %s (%d) isArg=%d firstMemvar=%d\n", np->vp->name, np->vp->no, np->isArg, np->p1);
       break;
    case PrintSrc:
       {
    ...
    case CText:
	  ...
	  if (np->p1)
	     fprintfOffs(out, level,
			 "if ( (_ret=_clip_memvarf( _mp, %ld /* %s */ ))) goto _trap_%d;\n",
			 (long) hashstr(np->vp->name), np->vp->name, np->node.seqNo);
	  else
	     fprintfOffs(out, level,
			 "if ( (_ret=_clip_fmemvar( _mp, %ld /* %s */ ))) goto _trap_%d;\n",
			 (long) hashstr(np->vp->name), np->vp->name, np->node.seqNo);
       }
       break;
    case CTextLval:
    ...
    case CTextRef:
	  ...
	  if (np->isArg)
	   ...
	   {
	      fprintfOffs(out, level,
			  "if ( (_ret=_clip_ref_fmemvar( _mp, %ld /* %s */ ))) goto _trap_%d;\n",
			  (long) hashstr(np->vp->name), np->vp->name, np->node.seqNo);
	   }
       }
       break;
