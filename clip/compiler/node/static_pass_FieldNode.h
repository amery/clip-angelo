    ...
    case CText:
	  ...
	  if (np->name && !np->area && !np->areaExpr)
	     fprintfOffs(out, level,
			 "if ( (_ret=_clip_field( _mp, %ld /* %s */, -1 ))) goto _trap_%d;\n",
	  ...
	  else if (np->name && np->areaExpr)
	   {
		 ...
		 np->areaExpr->pass(np->areaExpr, pass, level, par);
	      fprintfOffs(out, level,
			  "if ( (_ret=_clip_field( _mp, %ld /* %s */, _clip_pop_hash(_mp) ))) goto _trap_%d;\n",
			  (long) hashstr(np->name), np->name, np->node.seqNo);
	   }
	  else if (np->area && np->nameExpr)
	   {
	      fprintfOffs(out, level,
			  "if ( (_ret=_clip_push_area( _mp, %ld /* %s */ ))) goto _trap_%d;\n",
	  ...
	  else if (np->areaExpr && np->nameExpr)
	   {
		 ...
		 np->areaExpr->pass(np->areaExpr, pass, level, par);
	      fprintfOffs(out, level,
			  "if ( (_ret=_clip_push_area( _mp, _clip_pop_hash(_mp) ) )) goto _trap_%d;\n", np->node.seqNo);
	      np->nameExpr->pass(np->nameExpr, pass, level + 1, par);
	      fprintfOffs(out, level, "if ( (_ret=_clip_pop_area( _mp ) )) goto _trap_%d;\n", np->node.seqNo);
	   }
	  else if (np->nameExpr)
	   {
		 ...
		 np->nameExpr->pass(np->nameExpr, pass, level + 1, par);
	      fprintfOffs(out, level,
			  "{ long _a, _f; _clip_pop_fieldhash(_mp, &_a, &_f); if ( (_ret=_clip_field( _mp, _f, _a )) ) goto _trap_%d; }\n",
			  np->node.seqNo);
	   }
       }
       return 0;
