	  ...
	  if (np->expr)
	   {
		  ...
		  if (np->op == '=')
		     fprintfOffs(out, level,
				 "if ((_ret=_clip_store( _mp, %d, %d ))) goto _trap_%d;\n",
				 np->dim, np->expr->isRef ? (np->expr->isNil ? 2 : 1) : 0, np->node.seqNo);
		  else
		     fprintfOffs(out, level,
				 "if ((_ret=_clip_opstore( _mp, %d, '%c' ))) goto _trap_%d;\n",
		  ...
		  if (np->op == '=')
		     fprintfOffs(out, level,
				 "if ((_ret=_clip_istore( _mp, %d, %d ))) goto _trap_%d;\n",
				 np->dim, np->expr->isRef ? (np->expr->isNil ? 2 : 1) : 0, np->node.seqNo);
		  else
		     fprintfOffs(out, level,
				 "if ((_ret=_clip_opistore( _mp, %d, '%c' ))) goto _trap_%d;\n",
				 np->dim, np->op, np->node.seqNo);
	       }
	   }
	  else
