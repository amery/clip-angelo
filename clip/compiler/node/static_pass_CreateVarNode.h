	  ...
	  if (np->vp->macro)
	   {
		 ...
		 fprintfOffs(out, level,
			     "_clip_memvar_space( _mp, _clip_space( _mp, %ld /* %s */) ,  _clip_pop_hash( _mp ), 1 );\n",
	  ...
	  else
	   {
	      if (np->space == 1)
		 fprintfOffs(out, level,
			     "_clip_memvar_public( _mp, %ld /* %s */ );\n", (long) hashstr(np->vp->name), np->vp->name);
	      else if (np->space == 0)
		 fprintfOffs(out, level,
			     "_clip_memvar_private( _mp, %ld /* %s */ );\n", (long) hashstr(np->vp->name), np->vp->name);
	      else
		 fprintfOffs(out, level,
			     "_clip_memvar_space( _mp, _clip_space( _mp, %ld /* %s */) , %ld /* %s */, 1 );\n",
