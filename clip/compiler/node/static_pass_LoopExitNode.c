...
pass_LoopExitNode(void *self, Pass pass, int level, void *par)
{
   ...
   case OTextLoop:
	 ...
	 {
	    if (np->isLoop)
			 SETSHORT(ld->StrBuf_out_of_LoopData, np->offs, ld->boffs_of_LoopData - (np->offs + sizeof(short)));

	    else
			 SETSHORT(ld->StrBuf_out_of_LoopData, np->offs, ld->eoffs_of_LoopData - (np->offs + sizeof(short)));
