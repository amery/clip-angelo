...
write_names(File * file)
{
   ...
   if (out)
   {
      ...
      {
			Function *fp = (Function *) file->Coll_functions_of_File.items_of_Coll[i];
   ...
   if (out)
   {
		for (i = 0; i < file->Coll_undeclExternFunctions_of_File.count_of_Coll; i++)
      {
			char *s = (char *) file->Coll_undeclExternFunctions_of_File.items_of_Coll[i];
