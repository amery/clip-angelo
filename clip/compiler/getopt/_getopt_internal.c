   ...
   if (longopts != NULL && (argv[optind][1] == '-' || (long_only && (argv[optind][2] || !my_index(optstring, argv[optind][1])))))
   {
	 ...
	 if (*nameend)
	 {
	    ...
	    else
	    {
		  ...
		  if (argv[optind - 1][1] == '-')
		     ...
		     /* +option or -option */
#if NLS
		     fprintf(stderr, catgets(_libc_cat, GetoptSet, GetoptNoArgumentsAllowed2, "%s: option `%c%s' doesn't allow an argument\n"), argv[0], argv[optind - 1][0], pfound->name);
#else
	 ...
	 if (opterr)
	 {
	    if (argv[optind][1] == '-')
	       ...
	       /* +option or -option */
#if NLS
	       fprintf(stderr, catgets(_libc_cat, GetoptSet, GetoptUnrecognized2, "%s: unrecognized option `%c%s'\n"), argv[0], argv[optind][0], nextchar);
#else
