...
#if NLS
	      fprintf(stderr,
		      catgets(_libc_cat, GetoptSet, GetoptAmbiguous, "%s: option `%s' is ambiguous\n"), argv[0], argv[optind]);
#else
		  ...
		  if (opterr)
		   {
		     /* 1003.2 specifies the format of this message.  */
		      ...
		      fprintf(stderr,
			      catgets(_libc_cat, GetoptSet,
				      GetoptRequiresArgument2, "%s: option requires an argument -- %c\n"), argv[0], c);
#else
