...
installName(char *name)
{
   ...
   {
		VAR(Var, vp, curFunction->VarColl_code_params_of_Function->coll.items_of_Coll[no]);
      Node *rp;
   ...
   {
		VAR(Var, vp, curFunction->VarColl_params_of_Function->coll.items_of_Coll[no]);
   ...
   {
		VAR(Var, vp, fp->VarColl_locals_of_Function->coll.items_of_Coll[no]);
      if (level > 0)
      {
	 vp->isRef = 1;
   ...
   {
		VAR(Var, vp, curFile->Function_main_of_File->VarColl_memvars_of_Function->coll.items_of_Coll[no]);
      return new_MemvarNode(vp);
   }
   if (search_VarColl(curFunction->VarColl_fields_of_Function, name, &no))
   {
		VAR(Var, vp, curFunction->VarColl_fields_of_Function->coll.items_of_Coll[no]);
      return new_FieldNode(vp);
   }
   if (fp != curFunction && search_VarColl(fp->VarColl_fields_of_Function, name, &no))
   {
		VAR(Var, vp, fp->VarColl_fields_of_Function->coll.items_of_Coll[no]);
      return new_FieldNode(vp);
   }
   ...
   {
		VAR(Var, vp, curFile->Function_main_of_File->VarColl_fields_of_Function->coll.items_of_Coll[no]);
      return new_FieldNode(vp);
   }
   if (search_VarColl(curFunction->VarColl_parameters_of_Function, name, &no))
   {
		VAR(Var, vp, curFunction->VarColl_parameters_of_Function->coll.items_of_Coll[no]);
      return new_ParameterNode(vp);
   }
   if (fp != curFunction && search_VarColl(fp->VarColl_parameters_of_Function, name, &no))
   {
		VAR(Var, vp, fp->VarColl_parameters_of_Function->coll.items_of_Coll[no]);
      return new_ParameterNode(vp);
   }

   if (search_VarColl(curFunction->VarColl_privates_of_Function, name, &no))
   {
		VAR(Var, vp, curFunction->VarColl_privates_of_Function->coll.items_of_Coll[no]);
   ...
   {
		VAR(Var, vp, fp->VarColl_privates_of_Function->coll.items_of_Coll[no]);
      /*return new_PrivateNode(vp); */
      if (auto_memvar)
	 return new_MemvarNode(vp);
      else
   ...
   {
		VAR(Var, vp, curFunction->VarColl_publics_of_Function->coll.items_of_Coll[no]);
   ...
   {
		VAR(Var, vp, fp->VarColl_publics_of_Function->coll.items_of_Coll[no]);
      /*return new_PublicNode(vp); */
      if (auto_memvar)
