...
codegen_Function(Function * fp)
{
   ...
   for (j = fp->VarColl_locals_of_Function->unsorted.count_of_Coll - 1; j >= 0; --j)
   ...
   for (j = fp->VarColl_statics_of_Function->unsorted.count_of_Coll - 1; j >= 0; --j)
   {
		VAR(Var, vp, fp->VarColl_statics_of_Function->unsorted.items_of_Coll[j]);
   ...
   for (i = 0; i < fp->VarColl_params_of_Function->unsorted.count_of_Coll; i++)
   {
		VAR(Var, vp, fp->VarColl_params_of_Function->unsorted.items_of_Coll[i]);
