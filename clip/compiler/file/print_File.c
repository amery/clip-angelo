...
print_File(File * file)
{
   int i, count;
	...
	printf("\nFile '%s' (%d functions):\n", file->name_of_File, file->Coll_functions_of_File.count_of_Coll);

	count = file->Coll_externFunctions_of_File.count_of_Coll;
   ...
   print_Constants(&file->Coll_unsortedStrings_of_File, "strings");
	printf("codeblocks: %d\n", file->Coll_codeblocks_of_File.count_of_Coll);
   ...
   /*print_Function( file->main); */
	count = file->Coll_unsortedFunctions_of_File.count_of_Coll;
   for (i = 0; i < count; ++i)
   {
