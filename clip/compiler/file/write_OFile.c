...
/* generate hash-buckets string */
   {
      ...
      int sum = 0;
		...
		int count = file->Coll_names_of_File.count_of_Coll;
	 ...
	 sum += l;
      ...
      {
			VAR(Function, fp, file->Coll_unsortedFunctions_of_File.items_of_Coll[i]);
			for (j = 0; j < fp->VarColl_statics_of_Function->coll.count_of_Coll; j++)
	 {
		 VAR(Var, vp, fp->VarColl_statics_of_Function->coll.items_of_Coll[j]);
      ...
      for (i = 0; i < count; ++i)
      {
			ClipHashBucket *buck = (ClipHashBucket *) coll.items_of_Coll[i];
   ...
   *len = 0;
   ...
   /* 8,4l */
	putShort_StrBuf(bp, file->Coll_unsortedNumbers_of_File.count_of_Coll);
   /* 8,4l,1s */
	putShort_StrBuf(bp, file->Coll_unsortedStrings_of_File.count_of_Coll + 1);
   /* 8,4l,2s */
...
/* eadd */
   ...
   count = file->Coll_unsortedStrings_of_File.count_of_Coll;
