static int
rm_setscope(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_FILTER * fp, unsigned int *map, int oper, char *lval, char *rval, int bytes, int *optimize, int test, int *ic, const char *__PROC__)
{
   int i, j;
   ...
   *d = 0;
   ...
   for (i = 0; i < rd->ords_opened; i++)
   {
	 ...
	 if (!test)
	 {
	    if (oper == RM_EQU)
	    {
	       if (rd->orders[i]->vtbl->setscope(ClipMachineMemory, rd, rd->orders[i], vp, vp, map, fp->size, ClipMachineMemory->flags & EXACT_FLAG, __PROC__))
		  goto err;
	    }
	    ...
	    else if (oper == RM_LAR)
	    {
	       ...
	       if (rd->orders[i]->vtbl->setscope(ClipMachineMemory, rd, rd->orders[i], vp, NULL, map, fp->size, 0, __PROC__))
		  goto err;
	       if (rd->orders[i]->vtbl->setscope(ClipMachineMemory, rd, rd->orders[i], vp, vp, tm, fp->size, 0, __PROC__))
		  goto err;
	       for (j = 0; j < bytes; j++)
	    ...
	    else if (oper == RM_LES)
	    {
	       ...
	       if (rd->orders[i]->vtbl->setscope(ClipMachineMemory, rd, rd->orders[i], NULL, vp, map, fp->size, 0, __PROC__))
		  goto err;
	       if (rd->orders[i]->vtbl->setscope(ClipMachineMemory, rd, rd->orders[i], vp, vp, tm, fp->size, 0, __PROC__))
		  goto err;
	       for (j = 0; j < bytes; j++)
