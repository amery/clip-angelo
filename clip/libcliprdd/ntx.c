...
static int ntx_seek(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro,
...
static int _ntx_put_node(ClipMachine * ClipMachineMemory, RDD_DATA * rd,
			 RDD_ORDER * ro, NTX_PAGE * page,
			 unsigned int pageoffs, char *key,
			 unsigned int recno, unsigned int child, int buckno, int isright, int hostcs, const char *__PROC__);
static int _ntx_add_node(ClipMachine * ClipMachineMemory, RDD_DATA * rd,
			 RDD_ORDER * ro, char *key, int recno, int child, const char *__PROC__);
...
static int
_ntx_compare(RDD_DATA * rd, char unique, char descend, char *s1, int l1,
	     unsigned int recno1, char *s2, int l2, unsigned int recno2)
{
   int       l = (l1 < l2 ? l1 : l2);
   ...
   while (l <= h)
    {
       ...
       c =
	_ntx_compare(rd, ro->unique, ro->descend, (char *) buck->key,
		     ro->keysize, exact ? _rdd_uint(buck->recno) : 0, key, len, exact ? recno : 0);
       if (c && f)
	  return 1;
       if (c < 0)
	  l = i + 1;
       ...
       else
	{
	   if (exact)
	       ...
	       while (!c && i--)
		{
		   buck = _ntx_buck(page, i);
		   c =
		    _ntx_compare(rd, ro->unique, ro->descend,
				 (char *) buck->key, ro->keysize, _rdd_uint(buck->recno), key, len, recno);
		}
	       *no = i + 1;
	    }
	   else
...
static int
__ntx_tuneprev(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro,
	       char *key, int len, unsigned int recno, int *ok, const char *__PROC__)
{
   NTX_PAGE  page;
...
static int
__ntx_tunenext(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro,
	       char *key, int len, unsigned int recno, int *ok, const char *__PROC__)
{
   NTX_PAGE  page;
...
static int
_ntx_search_tree(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro,
		 char *key, int len, unsigned int recno, int *res, int exact, const char *__PROC__)
{
   NTX_PAGE  page;
...
_ntx_calc_key(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   ...
   if (ro->simpexpr)
	...
	{
	   memcpy(ro->key, v.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		  ...
		  _rdd_put_backuint(fp.next, 0);
	       if ((er =
		    rdd_write(ClipMachineMemory, &ro->index->file,
			      *page + i * sizeof(NTX_PAGE), sizeof(NTX_FREEPAGE), (char *) &fp, __PROC__)))
		  return er;
	    }
	}
    }
...
static int
_ntx_split_node(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro,
		char *kk, int rn, unsigned int cc, const char *__PROC__)
{
   NTX_PAGE  rpp, lpp;
...
_ntx_put_node(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro,
	      ...
	      char *key, unsigned int recno, unsigned int child, int buckno, int isright, int hostcs, const char *__PROC__)
{
...
static int
_ntx_add_node(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro,
	      char *key, int recno, int child, const char *__PROC__)
{
   NTX_PAGE  page;
...
static int
_ntx_remove_node(ClipMachine * ClipMachineMemory, RDD_ORDER * ro,
		 unsigned int page, NTX_PAGE * pp, int no, const char *__PROC__)
{
...
static int
_ntx_first(ClipMachine * ClipMachineMemory, RDD_ORDER * ro, unsigned int root,
...
static int
_ntx_last(ClipMachine * ClipMachineMemory, RDD_ORDER * ro, unsigned int root,
	  unsigned int *recno, int *out, const char *__PROC__)
{
   NTX_PAGE  page;
...
static int
ntx_gobottom(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   NTX_HEADER hdr;
...
/*	printf("go bottom\n? 'go bottom',recno(),bof()\n");*/
   ro->level = 0;
   if ((er = _ntx_last(ClipMachineMemory, ro, _rdd_uint(hdr.root), &recno, &out, __PROC__)))
      return er;
   if (out)
    {
       if ((er = rd->vtbl->lastrec(ClipMachineMemory, rd, (int *) (&lastrec), __PROC__)))
...
static int
ntx_seek(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro,
	 ClipVar * v, int soft, int last, int *found, const char *__PROC__)
{
   NTX_HEADER hdr;
...
static int
ntx_descend(ClipMachine * ClipMachineMemory, RDD_ORDER * ro, int descend, int *res, const char *__PROC__)
{
   *res = ro->descend;
   return 0;
}
...
static int
ntx_destroy(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   return 0;
}
...
static int
_ctx_openorder(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_INDEX * ri,
	       CTX_TAG * tag, RDD_ORDER ** pro, const char *__PROC__)
{
   RDD_ORDER *ro;
...
ctx_zap(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__)
{
   ...
   if ((er = rdd_trunc(ClipMachineMemory, &ri->file,
		       sizeof(CTX_HEADER) + chdr.ntags * (sizeof(NTX_HEADER) + sizeof(NTX_PAGE)), __PROC__)))
      return er;
   ...
   for (i = 0; i < chdr.ntags; i++)
    {
       ...
       if ((er =
	    rdd_write(ClipMachineMemory, &ri->file,
		      sizeof(CTX_HEADER) + i * sizeof(NTX_HEADER), sizeof(NTX_HEADER), &hdr[i], __PROC__)))
	  return er;
	  ...
	  _rdd_put_ushort(page.offs + j * 2, offs);
       ...
       if ((er = rdd_write(ClipMachineMemory, &ri->file, sizeof(CTX_HEADER) +
			   chdr.ntags * sizeof(NTX_HEADER) + i * sizeof(NTX_PAGE), sizeof(NTX_PAGE), &page, __PROC__)))
	  return er;
...
ctx_create(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_INDEX * ri,
	   RDD_ORDER ** rop, const char *tag, const char *expr,
	   ClipVar * block, int unique, int first, unsigned int h, const char *__PROC__)
{
   CTX_HEADER chdr;
