...
static int dbf_create(ClipMachine * ClipMachineMemory, RDD_DATA_VTBL * vtbl,
...
static int dbf_rawgoto(ClipMachine * ClipMachineMemory, RDD_DATA * rd,
		       unsigned int rec, char valid_stack, const char *__PROC__);
...
static int dbf_getmemoid(ClipMachine * ClipMachineMemory, RDD_DATA * rd, int no,
...
static int vfp_create(ClipMachine * ClipMachineMemory, RDD_DATA_VTBL * vtbl,
		      char *name, RDD_STRUCT * rddstru, int nfields, const char *__PROC__);
   ...
   *r = 1;
   if (rd->shared)
       ...
       for (i = 0; i < rd->nlocks; i++)
	{
       ...
       if (!rd->flocked)
	{
	   if (_clip_setlock(ClipMachineMemory, rd->file.filehash, rd->file.fd,
   ...
   if (rd->shared)
    {
	  ...
	  if ((er = dbf_ulock(ClipMachineMemory, rd, rd->locks[i], 0, __PROC__)))
       ...
       if (_clip_setlock(ClipMachineMemory, rd->file.filehash, rd->file.fd,
			 (rd->lockstyle ? 0x10000000 : 1000000000) + 1, CLIP_LOCK_WRITE | CLIP_LOCK_FLOCK | CLIP_LOCK_HILEVEL))
...
dbf_ulock(ClipMachine * ClipMachineMemory, RDD_DATA * rd, unsigned int rec, int flock, const char *__PROC__)
{
   if (rd->shared)
       ...
       if (rec)
	{
	   ...
	   if (!rd->flocked)
	    {
	       if (_clip_unlock(ClipMachineMemory, rd->file.filehash, rd->file.fd,
       ...
       else
	{
	   if (rd->flocked)
		...
		{
		   if (_clip_unlock
		       (ClipMachineMemory, rd->file.filehash, rd->file.fd,
			(rd->lockstyle ? 0x10000000 : 1000000000) + 1, CLIP_LOCK_FLOCK))
...
{
   *r = _clip_setlock(ClipMachineMemory, rd->file.filehash, rd->file.fd,
		      (rd->lockstyle ? 0x10000000 : 1000000000) + rec, CLIP_LOCK_WRITE | CLIP_LOCK_HILEVEL);
   if (!*r)
    {
...
dbf__rlock(ClipMachine * ClipMachineMemory, RDD_DATA * rd, const char *__PROC__)
{
   if (rd->shared)
	...
	{
	   if (_clip_setlock(ClipMachineMemory, rd->file.filehash, rd->file.fd,
			     rd->lockstyle ? 0x10000000 : 1000000000, CLIP_LOCK_READ | CLIP_LOCK_LOWLEVEL | CLIP_LOCK_WAIT))
...
dbf__wlock(ClipMachine * ClipMachineMemory, RDD_DATA * rd, const char *__PROC__)
{
   if (rd->shared)
	...
	{
	   if (_clip_setlock(ClipMachineMemory, rd->file.filehash, rd->file.fd,
			     rd->lockstyle ? 0x10000000 : 1000000000, CLIP_LOCK_WRITE | CLIP_LOCK_LOWLEVEL | CLIP_LOCK_WAIT))
...
dbf_open(ClipMachine * ClipMachineMemory, RDD_DATA * rd, const char *__PROC__)
{
   ...
   for (i = 0; i < rd->nfields; i++)
    {
       if ((er =
	    rdd_read(ClipMachineMemory, &rd->file,
		     sizeof(DBF_HEADER) + sizeof(DBF_FIELD) * i, sizeof(DBF_FIELD), (char *) &field, __PROC__)))
	  return er;
       if (field.type == '0')
	{
	   rd->nulloffs = offs;
...
static int
dbf_create(ClipMachine * ClipMachineMemory, RDD_DATA_VTBL * vtbl, char *name,
	   RDD_STRUCT * stru, int nfields, const char *__PROC__)
{
   RDD_FILE  file;
...
static int
vfp_create(ClipMachine * ClipMachineMemory, RDD_DATA_VTBL * vtbl, char *name,
	   RDD_STRUCT * stru, int nfields, const char *__PROC__)
{
   RDD_FILE  file;
...
dbf_close(ClipMachine * ClipMachineMemory, RDD_DATA * rd, const char *__PROC__)
{
   ...
   if (!rd->readonly && rd->updated)
	...
	{
	   if ((er =
		rdd_trunc(ClipMachineMemory, &rd->file,
			  _rdd_ushort(hdr.hdrsize) + _rdd_ushort(hdr.recsize) * lastrec + 1, __PROC__)))
	      return er;
	}
       if ((er =
	    rdd_write(ClipMachineMemory, &rd->file,
		      _rdd_ushort(hdr.hdrsize) + _rdd_ushort(hdr.recsize) * lastrec, 1, &end, __PROC__)))
	  return er;
    }
   if (rd->updated)
    {
...
dbf_append(ClipMachine * ClipMachineMemory, RDD_DATA * rd, const char *__PROC__)
{
	   ...
	   if (flen > rd->file.mapsize)
	    {
	       ...
	       if ((er =
		    rdd_write(ClipMachineMemory, &rd->file,
			      rd->hdrsize + rd->recsize * (lastrec + 1 + delta) - 1, 1, "", __PROC__)))
		  goto err;
	    }
	}
    }
   ...
   if ((er =
	rdd_write(ClipMachineMemory, &rd->file,
		  rd->hdrsize + rd->recsize * (lastrec - 1), rd->recsize + 1, rd->record, __PROC__)))
      goto err;
...
static int
dbf_getmemoid(ClipMachine * ClipMachineMemory, RDD_DATA * rd, int no,
	      unsigned int *memoid, unsigned short int *vlen, const char *__PROC__)
{
   char      memo[11];
   ...
   *memoid = 0;
   if (strchr("MPG", rd->fields[no].type))
       ...
       if (rd->sig != 0x30)
	{
	   if ((er =
		rdd_read(ClipMachineMemory, &rd->file,
			 rd->hdrsize + rd->recsize * (rd->recno - 1) + rd->fields[no].offs, 10, memo, __PROC__)))
	      return er;
	   memo[10] = 0;
	   *memoid = atoi(memo);
	}
       else
	{
	   if ((er =
		rdd_read(ClipMachineMemory, &rd->file,
			 rd->hdrsize + rd->recsize * (rd->recno - 1) + rd->fields[no].offs, 4, memo, __PROC__)))
	      return er;
   ...
   else if (rd->fields[no].type == 'V')
    {
       if ((er =
	    rdd_read(ClipMachineMemory, &rd->file,
		     rd->hdrsize + rd->recsize * (rd->recno - 1) +
...
{
   rd->valid = 1;
   return rdd_read(ClipMachineMemory, &rd->file,
		   rd->hdrsize + rd->recsize * (rd->recno - 1), rd->recsize, rd->record, __PROC__);
}
...
{
   rd->changed = 0;
   return rdd_write(ClipMachineMemory, &rd->file,
		    rd->hdrsize + rd->recsize * (rd->recno - 1), rd->recsize, rd->record, __PROC__);
}
...
dbf_getvalue(ClipMachine * ClipMachineMemory, RDD_DATA * rd, int no, ClipVar * vp, const char *__PROC__)
{
   ...
   switch (rd->fields[no].type)
    ...
    case 'C':
	  ...
	  loc_read(rd->loc, (unsigned char *) (vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf),
       ...
       if (rd->memo)
	{
	   if (rd->sig != 0x30)
	   ...
	   else
	    {
	       ...
	       if ((er = rd->memo->vtbl->getvalue(ClipMachineMemory, rd->memo,
		  ...
		  if (rd->indexing)
		   {
		      ...
		      vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf =
		       malloc(vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 1);
		      vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf[vp->ClipStrVar_s_of_ClipVar.
											   ClipBuf_str_of_ClipStrVar.
											   len_of_ClipBuf] = 0;
		      memcpy(vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			     (char *) rd->record + rd->fields[no].offs,
			     vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
		      s =
		       vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf +
		       vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf;
			 ...
			 *s = ' ';
		      if (!rd->fields[no].binary)
			 loc_read(rd->loc,
				  (unsigned char *) (vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf),
				  vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
		      return 0;
		   }
    ...
    case 'X':
	  ...
	  switch (((char *) rd->record)[rd->fields[no].offs])
	   ...
	   case 'C':
		 ...
		 vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType = CHARACTER_type_of_ClipVarType;
		 vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf =
		  ((unsigned char *) rd->record)[rd->fields[no].offs + 1];
		 vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf =
		  malloc(vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 1);
		 memcpy(vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			(char *) rd->record + rd->fields[no].offs + 2,
			vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
		 vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf[vp->ClipStrVar_s_of_ClipVar.
										      ClipBuf_str_of_ClipStrVar.
										      len_of_ClipBuf] = 0;
	   ...
	   case 'L':
	      {
		 vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType = LOGICAL_type_of_ClipVarType;
		 vp->ClipLogVar_l_of_ClipVar.value_of_ClipLogVar =
...
/* Visual FoxPro types */
    ...
    case 'Y':
	  ...
	  if (rd->fields[no].type == 'Y')
	     vp->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar =
	      (double) (*(long long *) &vp->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar) / 10000;
	  ...
	  if (rd->fields[no].len == 3)
	  ...
	  else
	   {
		 ...
		 goto err;
	      ...
	      memcpy(str, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		     vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
	      if (!rd->fields[no].binary)
		 loc_write(rd->loc, (unsigned char *) str,
			   vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
	      if (vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf <= rd->fields[no].len - 2)
	       {
		  memcpy(rd->record + rd->fields[no].offs, str,
			 vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
	       }
	      else
	       {
		  ...
		  if (rd->memo->vtbl->setvchar)
		   {
		      if ((er = rd->memo->vtbl->setvchar(ClipMachineMemory, rd->memo,
							 vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf -
							 (rd->fields[no].len -
							  6), oldlen, &memoid, str + rd->fields[no].len - 6, __PROC__)))
		       {
			  free(str);
			  return er;
		       }
    ...
    case 'X':
	  ...
	  switch (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
	   ...
	   case CHARACTER_type_of_ClipVarType:
	      {
		 unsigned int len =
		  (unsigned char) min(rd->fields[no].len - 2,
				      vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
		 ...
		 memcpy((char *) rd->record + rd->fields[no].offs + 2,
			vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, len);
		 if (!rd->fields[no].binary)
...
static int
dbf_recall(ClipMachine * ClipMachineMemory, RDD_DATA * rd, const char *__PROC__)
{
   char      d = 0x20;
