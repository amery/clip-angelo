...
rm_checkpar(char *c, char *end, int func)
{
   ...
   while (ps && (++c < end))
	...
	if ((*c == '<' || (*c == '>' && *(c - 1) != '-') || *c == '!'
	     || *c == '=' || (*c == '.' && isalpha(*(c + 1)) && isalpha(*(c + 2)))) && !func)
	{
	   return;
	}
    }
...
static int
rm_setscope(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_FILTER * fp,
	    unsigned int *map, int oper, char *lval, char *rval, int bytes,
	    int *optimize, int test, int *ic, const char *__PROC__)
{
   int       i, j;
	   ...
	   if (!test)
	       ...
	       if (oper == RM_EQU)
		{
		   if (rd->orders[i]->vtbl->setscope(ClipMachineMemory, rd, rd->orders[i], vp, vp, map,
						     fp->size, ClipMachineMemory->flags & EXACT_FLAG, __PROC__))
		      goto err;
		}
	       else if (oper == RM_EEQU)
		{
		   if (rd->orders[i]->vtbl->setscope(ClipMachineMemory, rd, rd->orders[i], vp, vp, map, fp->size, 1, __PROC__))
		      goto err;
		}
	       else if (oper == RM_NEQU)
		{
		   ...
		   if (rd->orders[i]->vtbl->setscope(ClipMachineMemory, rd, rd->orders[i], vp, vp, tm, fp->size, 0, __PROC__))
		      goto err;
		   rm_not(map, tm, bytes);
		}
	       else if (oper == RM_LAR)
...
rm_cmp(ClipMachine * ClipMachineMemory, int oper, ClipVar * vp1, ClipVar * vp2, int ic)
{
   ...
   if (ic && (vp1->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType)
       && (vp2->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType))
    {
       if (oper == RM_EEQU)
	{
	   if (vp1->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf !=
	       vp2->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf)
	      r =
	       vp1->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf -
	       vp2->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf;
	   else
	      r =
	       _clip_strnncasecmp(vp1->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
       ...
       else
	{
	   if (!vp1->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf
	      ...
	      r =
	       _clip_strncasecmp(vp1->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
				 vp2->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
				 min(vp1->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf,
				     vp2->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf));
	}
    }
   else
...
static int
rm_checkscope(ClipMachine * ClipMachineMemory, RDD_DATA * rd, unsigned int *map,
	      int size, int oper, char *fname, ClipVar * val, int ic, const char *__PROC__)
{
   ClipVar   key;
...
rm_intersectscope(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_FILTER * fp,
		  unsigned int *map, int oper, char *lval, char *rval,
		  int bytes, int *optimize, int test, int *ic, const char *__PROC__)
{
   int       i;
...
static unsigned int *
rm_prim(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_FILTER * fp, int bytes,
	int *optimize, int npseudo, RDD_PSEUDO * pseudo, int test, unsigned int *obm, const char *__PROC__)
{
   unsigned int *bm;
   ...
   switch (rd->curlex)
    ...
    case RM_WORD:
	  ...
	  if (oper != RM_EQU && oper != RM_EEQU && oper != RM_NEQU
	      && oper != RM_LAR && oper != RM_LARE && oper != RM_LES && oper != RM_LESE)
	   {
		  ...
		  if (obm)
		   {
			  ...
			  (ClipMachineMemory, rd, fp, bm, RM_EEQU, lval, ".T.", bytes, optimize, test, NULL, __PROC__))
			 return NULL;
		   }
		  else
		   {
...
static unsigned int *
rm_term(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_FILTER * fp, int bytes,
	int *optimize, int npseudo, RDD_PSEUDO * pseudo, int test, const char *__PROC__)
{
	       ...
	       for (i = 0; i < bytes; i++)
		{
		  ...
		  //if (tmp1 & 0x000000FF)
		   recs += ((tmp1 & 0x00000001) != 0) +
		    ...
		    ((tmp1 & 0x00000008) != 0) +
		    ((tmp1 & 0x00000010) != 0) +
		    ((tmp1 & 0x00000020) != 0) + ((tmp1 & 0x00000040) != 0) + ((tmp1 & 0x00000080) != 0);
		  //if (tmp1 & 0x0000FF00)
		   recs += ((tmp1 & 0x00000100) != 0) +
		    ...
		    ((tmp1 & 0x00000800) != 0) +
		    ((tmp1 & 0x00001000) != 0) +
		    ((tmp1 & 0x00002000) != 0) + ((tmp1 & 0x00004000) != 0) + ((tmp1 & 0x00008000) != 0);
		  //if (tmp1 & 0x00FF0000)
		   recs += ((tmp1 & 0x00010000) != 0) +
		    ...
		    ((tmp1 & 0x00080000) != 0) +
		    ((tmp1 & 0x00100000) != 0) +
		    ((tmp1 & 0x00200000) != 0) + ((tmp1 & 0x00400000) != 0) + ((tmp1 & 0x00800000) != 0);
		  //if (tmp1 & 0xFF000000)
		   recs += ((tmp1 & 0x01000000) != 0) +
		    ...
		    ((tmp1 & 0x08000000) != 0) +
		    ((tmp1 & 0x10000000) != 0) +
		    ((tmp1 & 0x20000000) != 0) + ((tmp1 & 0x40000000) != 0) + ((tmp1 & 0x80000000) != 0);
		  /*
		     for(b=(i<<2),bb=0;bb<4;b++,bb++){
		     if( !(((char*)bm)[b]) )
		     continue;
...
unsigned int *
rm_expr(ClipMachine * ClipMachineMemory, RDD_DATA * rd, RDD_FILTER * fp, int bytes,
	int *optimize, int npseudo, RDD_PSEUDO * pseudo, int test, const char *__PROC__)
{
...
rm_checkjoin(ClipMachine * ClipMachineMemory, RDD_FILTER * f1, RDD_FILTER * f2, const char *__PROC__)
{
   ...
   if ((f1->rmap && !f2->rmap) || (!f1->rmap && f2->rmap))
    {
       rdd_err(ClipMachineMemory, EG_ARG, 0, __FILE__, __LINE__, __PROC__,
...
int
rm_union(ClipMachine * ClipMachineMemory, RDD_FILTER * f1, RDD_FILTER * f2, const char *__PROC__)
{
   int       i;
...
int
rm_evalpartial(ClipMachine * ClipMachineMemory, RDD_FILTER * fp, ClipVar * block,
	       unsigned int *ret, unsigned int *cnt, const char *__PROC__)
{
   ClipVar   v;
	   ...
	   if (fp->optimize == 2)
	       ...
	       if (block)
		{
		   ...
		   if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != LOGICAL_type_of_ClipVarType)
		    {
		       _clip_destroy(ClipMachineMemory, &v);
		       rdd_err(ClipMachineMemory, EG_SYNTAX, 0, __FILE__, __LINE__,
			       __PROC__, "Codeblock returns non-logical value");
		       goto err;
		    }
