...
static int _hs_add(ClipMachine * ClipMachineMemory, HIPER * hs, const char *str,
		   int len, unsigned int rec, const char *__PROC__);
static int hs_add(ClipMachine * ClipMachineMemory, HIPER * hs, const char *str,
		  int len, unsigned int *rno, const char *__PROC__);
static int hs_close(ClipMachine * ClipMachineMemory, HIPER * hs, const char *__PROC__);

static int hs_create(ClipMachine * ClipMachineMemory, const char *fname,
		     int pagesize, int lcase, int filtset, HIPER ** hsp, const char *__PROC__);
static int hs_filter(ClipMachine * ClipMachineMemory, HIPER * hs, const char *val,
		     int len, unsigned int **map, unsigned int lastrec, unsigned int *count, const char *__PROC__);
...
clip_HS_ADD(ClipMachine * ClipMachineMemory)
{
   ...
   if (_clip_parinfo(ClipMachineMemory, 2) == CHARACTER_type_of_ClipVarType)
       ...
       if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != CHARACTER_type_of_ClipVarType)
	{
       ...
       if ((er =
	    hs_add(ClipMachineMemory, hs, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		   vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf, &rno, __PROC__)))
	  goto err;
...
clip_HS_FILTER(ClipMachine * ClipMachineMemory)
{
   ...
   if (_clip_parinfo(ClipMachineMemory, 4) == NUMERIC_type_of_ClipVarType
       || _clip_parinfo(ClipMachineMemory, 4) == UNDEF_type_of_ClipVarType)
      land = 0;
...
clip_HS_NEXT(ClipMachine * ClipMachineMemory)
{
   ...
   while ((++hs->recno <= hs->lastrec) &&
	  ((!_rm_getbit(hs->rmap, hs->lastrec, hs->recno)) || (_rm_getbit(hs->map, hs->lastrec, hs->recno))));
...
clip_HS_REPLACE(ClipMachine * ClipMachineMemory)
{
   ...
   if (_clip_parinfo(ClipMachineMemory, 2) == CHARACTER_type_of_ClipVarType)
    {
       if ((er =
	    _hs_add(ClipMachineMemory, hs, val->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		    val->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf, rno, __PROC__)))
	  goto err;
    }
   else
    {
       ...
       if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != CHARACTER_type_of_ClipVarType)
	{
       ...
       if ((er =
	    _hs_add(ClipMachineMemory, hs, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		    vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf, rno, __PROC__)))
	  goto err;
       _clip_destroy(ClipMachineMemory, &v);
    }
...
static int
_hs_loadbits(ClipMachine * ClipMachineMemory, HIPER * hs, unsigned int poffs,
	     unsigned int lastrec, unsigned int **rmap, const char *__PROC__)
{
   unsigned char c4[4];
...
static int
_hs_tobitstyle(ClipMachine * ClipMachineMemory, HIPER * hs, unsigned int poffs,
	       unsigned int rec, unsigned int npairs, int *tobit, const char *__PROC__)
{
...
static int
_hs_compare(ClipMachine * ClipMachineMemory, HIPER * hs, unsigned int poffs,
	    int pos, unsigned int rec, int *c, unsigned int *offs,
	    unsigned int *roffs, int *rpos, int *nextp, const char *__PROC__)
{
...
static int
_hs_insert(ClipMachine * ClipMachineMemory, HIPER * hs, unsigned int offs, int pos,
	   unsigned int rec, unsigned int roffs, int rpos, int nextp, const char *__PROC__)
{
   unsigned char c4[4];
...
_hs_setpair(ClipMachine * ClipMachineMemory, HIPER * hs, unsigned int poffs, unsigned int rec, const char *__PROC__)
{
   ...
   if (binary)
       ...
       while (l <= h)
	{
       ...
       if ((er =
	    _hs_insert(ClipMachineMemory, hs, offs, i % hs->intsonpage + 1, rec,
		       noffs ? noffs : roffs, noffs ? 0 : rpos, nextp, __PROC__)))
	  goto err;
...
static int
_hs_delete(ClipMachine * ClipMachineMemory, HIPER * hs, unsigned int offs, int pos,
	   unsigned int roffs, int rpos, int nextp, const char *__PROC__)
{
...
static int
hs_create(ClipMachine * ClipMachineMemory, const char *fname, int pagesize,
	  int lcase, int filtset, HIPER ** hsp, const char *__PROC__)
{
...
static int
hs_filter(ClipMachine * ClipMachineMemory, HIPER * hs, const char *val, int len,
	  unsigned int **map, unsigned int lastrec, unsigned int *count, const char *__PROC__)
{
   int       ints = (lastrec + 31) / 32;
