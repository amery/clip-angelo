...
/*  INFO_NONEXPR (2) */
   ...
   var.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf =
    strlen(var.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf);
   _clip_aset(ClipMachineMemory, rp, &var, 1, vect);
...
clip_M6_ADDSCOPED(ClipMachine * ClipMachineMemory)
{
   ...
   READLOCK;
   if ((er =
	wa->rd->orders[ord]->vtbl->setscope(ClipMachineMemory, wa->rd,
					    wa->rd->orders[ord], t, b, fp->rmap, fp->size, 0, __PROC__)))
      goto err_unlock;
   if (opttype)
    {
       int       found;
...
clip_SX_BLOB2FILE(ClipMachine * ClipMachineMemory)
{
   ...
   if (fd == -1)
      goto err_create;
   if (write
       (fd, v.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
	v.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf) == -1)
      goto err_create;
...
clip_SX_MEMOPACK(ClipMachine * ClipMachineMemory)
{
   ...
   wa->rd->memo->file.fd =
    _clip_creat(ClipMachineMemory, wa->rd->memo->path, O_RDWR, ClipMachineMemory->fileCreateMode, !wa->rd->shared);
...
clip_RDD_INFO(ClipMachine * ClipMachineMemory)
{
   ...
   v.ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType = CHARACTER_type_of_ClipVarType;
   v.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf =
    strdup((*ClipMachineMemory->dbdrivers)[no].id_of_DBDriver);
   v.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf =
    strlen(v.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf);
...
clip_SXCHAR(ClipMachine * ClipMachineMemory)
{
   ...
   switch (_clip_parinfo(ClipMachineMemory, 2))
    ...
    case CHARACTER_type_of_ClipVarType:
	  ...
	  memcpy(r->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		 v->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, l);
    ...
    case NUMERIC_type_of_ClipVarType:
       {
	  int       d =
	   min(v->ClipType_t_of_ClipVar.dec_of_ClipType,
	  ...
	  while (*s == ' ')
	     s++;
	  memmove(r->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, s,
		  len - (s - r->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf));
	  memset(r->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf + len -
		 (s - r->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf), ' ',
		 (s - r->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf));
	  break;
       }
	  ...
	  if (len > 0)
	   {
	      *r->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf =
	       v->ClipLogVar_l_of_ClipVar.value_of_ClipLogVar ? 'T' : 'F';
...
clip_SXNUM(ClipMachine * ClipMachineMemory)
{
   ...
   switch (_clip_parinfo(ClipMachineMemory, 1))
    ...
    case CHARACTER_type_of_ClipVarType:
	  ...
	  RETPTR(ClipMachineMemory)->ClipType_t_of_ClipVar.dec_of_ClipType =
	   max(0, v->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf - d - 1);
	  break;
       }
    case DATE_type_of_ClipVarType:
       {
...
clip_SXLOG(ClipMachine * ClipMachineMemory)
{
   ...
   switch (_clip_parinfo(ClipMachineMemory, 1))
    ...
    case LOGICAL_type_of_ClipVarType:
       ...
       break;
    case CHARACTER_type_of_ClipVarType:
       if (toupper(*v->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf) == 'T'
	   || toupper(*v->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf) == 'Y')
	  _clip_retl(ClipMachineMemory, 1);
       else
	  _clip_retl(ClipMachineMemory, 0);
       break;
...
clip_SXDATE(ClipMachine * ClipMachineMemory)
{
   ...
   switch (_clip_parinfo(ClipMachineMemory, 1))
    ...
    case DATE_type_of_ClipVarType:
       ...
       break;
    case CHARACTER_type_of_ClipVarType:
       _clip_retdj(ClipMachineMemory,
		   _clip_str_to_date(v->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
				     ClipMachineMemory->date_format, ClipMachineMemory->epoch));
       break;
    case NUMERIC_type_of_ClipVarType:
       {
...
clip_SX_KILLTAG(ClipMachine * ClipMachineMemory)
{
   ...
   if (lAll && (_clip_parinfo(ClipMachineMemory, 2) == CHARACTER_type_of_ClipVarType))
	...
	{
	   if (strcasecmp(wa->rd->indices[i]->name, index->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf) ==
	       0)
	    {
	       ri = wa->rd->indices[i];
	       break;
		   ...
		   if (ext && ext[0])
		    {
		       ...
		       for (i = 0; i < 4; i++)
			  (*ClipMachineMemory->memo_drivers)[j]->suff[i] =
			   tolower((*ClipMachineMemory->memo_drivers)[j]->suff[i]);
		    }
		   break;
		}
	    }
...
clip_SX_RLOCK(ClipMachine * ClipMachineMemory)
{
   ...
   if (t == ARRAY_type_of_ClipVarType)
       ...
       for (i = 0; i < ap->ClipArrVar_a_of_ClipVar.count_of_ClipArrVar; i++)
	{
	   ...
	   if ((er =
		rdd_rlock(ClipMachineMemory, wa->rd, (unsigned int) vp->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar, &aok,
			  __PROC__)))
	      goto err_unlock;
	   if (!aok)
	      ok = 0;
...
clip_SX_SETTAG(ClipMachine * ClipMachineMemory)
{
   ...
   if ((order->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == NUMERIC_type_of_ClipVarType)
       && (order->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar == 0))
    {
       ord = -1;
    }
   else
...
clip_SX_DESCEND(ClipMachine * ClipMachineMemory)
{
   ...
   if ((er =
	wa->rd->orders[ord]->vtbl->descend(ClipMachineMemory, wa->rd->orders[ord],
					   !wa->rd->orders[ord]->descend, &res, __PROC__)))
      return er;
   return 0;
 err:
   return er;
