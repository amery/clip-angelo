...
int
clip_INIT_DBT(ClipMachine * ClipMemoryStore)
{
   rdd_registermemodriver(ClipMemoryStore, dbt_vtbl());
   return 0;
}
...
static int
dbt_create(ClipMachine * ClipMemoryStore, char *name, const char *__PROC__)
{
   RDD_FILE file;
   DBT_HEADER hdr;
   int er;
   ...
   if (file.fd == -1)
      goto err;
   if ((er = rdd_write(ClipMemoryStore, &file, 0, sizeof(DBT_HEADER), (char *) &hdr, __PROC__)))
      return er;
   if (close(file.fd) == -1)
      goto err;
   return 0;
...
static int
dbt_zap(ClipMachine * ClipMemoryStore, RDD_MEMO * rm, const char *__PROC__)
{
   DBT_HEADER hdr;
   int er;

   if ((er = rdd_trunc(ClipMemoryStore, &rm->file, sizeof(DBT_HEADER), __PROC__)))
      return er;
   if ((er = rdd_read(ClipMemoryStore, &rm->file, 0, sizeof(DBT_HEADER), &hdr, __PROC__)))
      return er;
   _rdd_put_uint((unsigned char *) hdr.fuu, 1);
   return rdd_write(ClipMemoryStore, &rm->file, 0, sizeof(DBT_HEADER), &hdr, __PROC__);
}

static int
dbt_open(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_MEMO * rm, const char *__PROC__)
{
   rm->blocksize = 512;
   return 0;
}
...
static int
dbt_close(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_MEMO * rm, const char *__PROC__)
{
   destroy_rdd_memo(rm);
   return 0;
}
...
static int
dbt_getvalue(ClipMachine * ClipMemoryStore, RDD_MEMO * rm, int id, ClipVar * vp, const char *__PROC__)
{
	 ...
	 if ((id + count) * DBT_PAGE_SIZE + DBT_PAGE_SIZE > fsize)
	    {
	       ...
	       if ((er = rdd_read(ClipMemoryStore, &rm->file, (id + count++) * DBT_PAGE_SIZE, toread, buf, __PROC__)))
		  return er;
	       buf[toread] = 0x1A;
	       buf[toread + 1] = 0x1A;
	    }
	    ...
	    {
	       if ((er = rdd_read(ClipMemoryStore, &rm->file, (id + count++) * DBT_PAGE_SIZE, DBT_PAGE_SIZE + 1, buf, __PROC__)))
		  return er;
	    }
...
static int
dbt_setvalue(ClipMachine * ClipMemoryStore, RDD_MEMO * rm, int *id, ClipVar * vp, int binary, const char *__PROC__)
{
   ClipVar old;
   int pages, er;
   char *buf;
   ...
   char fuu[4];
	...
	if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != CHARACTER_type_of_ClipVarType)
      return rdd_err(ClipMemoryStore, EG_DATATYPE, 0, __FILE__, __LINE__, __PROC__, "Incompatible types");
   ...
   if ((er = dbt_getvalue(ClipMemoryStore, rm, *id, &old, __PROC__)))
      return er;
   pages = (old.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 2) / DBT_PAGE_SIZE + ((old.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 2) % DBT_PAGE_SIZE !=
0);
   ...
   pages = (vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 2) / DBT_PAGE_SIZE + ((vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 2) % DBT_PAGE_SIZE !=
0);
   ...
   if ((er = rdd_write(ClipMemoryStore, &rm->file, *id * DBT_PAGE_SIZE, pages * DBT_PAGE_SIZE, buf, __PROC__)))
      return er;
   free(buf);
   if (isnew)
      {
...
int
dbt_info(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_MEMO * rm, int cmd, const char *__PROC__)
{
   switch (cmd)
      {
      case DBI_MEMOBLOCKSIZE:
	 {
	    _clip_retni(ClipMemoryStore, DBT_PAGE_SIZE);
	    break;
	 }
      }
   return 0;
