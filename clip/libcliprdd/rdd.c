...
rdd_ii_addkey(ClipMachine * ClipMachineMemory, int h, const char *id, ClipVar * key, const char *__PROC__)
{
   ...
   if ((key->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType && ro->type != 'C') ||
       ...
       (key->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == DATE_type_of_ClipVarType && ro->type != 'D') || (key->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType ==
LOGICAL_type_of_ClipVarType && ro->type != 'L'))
...
_rdd_wildskip(ClipMachine * ClipMachineMemory, RDD_DATA * rd, const char *pattern, int regular, const char *s, int *found, const char *__PROC__)
{
   ...
   if (ro->vtbl->wildskip)
	 ...
	 *(e + 1) = 0;
	 while (!rd->eof && (memcmp(vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, pattern, s - pattern) == 0) &&
