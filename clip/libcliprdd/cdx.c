...
#define _CDX_HEADER(ro,hdr) if((er = rdd_read(ClipMemoryStore,&ro->index->file,ro->header,sizeof(CDX_HEADER),hdr,__PROC__))) return er
#define _CDX_PAGE(ro,pageoffs,page)	if((er = rdd_read(ClipMemoryStore,&ro->index->file,pageoffs,sizeof(CDX_PAGE),page,__PROC__))) return er; \
...
static int cdx_open(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *tag, const char *__PROC__);
static int cdx_close(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__);
static int cdx_create(ClipMachine * ClipMemoryStore, RDD_DATA * rd,
...
static int cdx_reindex(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__);
static int cdx_zap(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__);
static int cdx_gotop(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__);
static int cdx_gobottom(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__);
static int cdx_next(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__);
static int cdx_prev(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__);
static int cdx_seek(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, ClipVar * v, int soft, int last, int *found, const char *__PROC__);
static int cdx_gotokey(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, unsigned int keyno, int *ok, const char *__PROC__);
static int cdx_addkey(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, ClipVar * v, const char *__PROC__);
static int cdx_formatkey(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, ClipVar * var, void *res, const char *__PROC__);
static int cdx_keyno(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, int *keyno, const char *__PROC__);
static int cdx_lastkey(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, int *lastkey, const char *__PROC__);
static int cdx_info(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, int cmd, const char *__PROC__);
static int cdx_keyvalue(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, ClipVar * v, const char *__PROC__);
static int cdx_setscope(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, ClipVar * top, ClipVar * bottom, unsigned int *map, int bytes, int exact, const char *__PROC__);
static int cdx_wildskip(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *pattern, int regular, const char *s, ClipVar * block, int *found, const char *__PROC__);

static int _cdx_addkey(ClipMachine * ClipMemoryStore, int area, DbfLocale * loc, ClipVar * v, RDD_ORDER * ro, void *key, int len, unsigned int recno, const char *__PROC__);
static int __cdx_addkey(ClipMachine * ClipMemoryStore, int area, RDD_ORDER * ro, ClipVar * v, DbfLocale * loc, unsigned int recno, const char *__PROC__);

static int
cdx__rlock(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *__PROC__)
{
   return 0;
}

static int
cdx__wlock(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *__PROC__)
{
   return 0;
}

static int
cdx__ulock(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *__PROC__)
{
   return 0;
}

static int _cdx_prev_rm(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, CDX_LEAF * leaf, int *out, int cache, const char *__PROC__);
static int _cdx_next_rm(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, CDX_LEAF * leaf, int *out, int cache, const char *__PROC__);

static int idx_open(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *tag, const char *__PROC__);
static int idx_create(ClipMachine * ClipMemoryStore, RDD_DATA * rd,
		      RDD_INDEX * ri, RDD_ORDER ** rop, const char *tag, const char *expr, ClipVar * block, int unique, int first, unsigned int header, const char *__PROC__);
static int idx_zap(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__);
static int idx_reindex(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__);
...
int
clip_INIT_CDX(ClipMachine * ClipMemoryStore)
{
   rdd_registerindexdriver(ClipMemoryStore, cdx_vtbl());
   return 0;
}

int
clip_INIT_IDX(ClipMachine * ClipMemoryStore)
{
   rdd_registerindexdriver(ClipMemoryStore, idx_vtbl());
   return 0;
}

static int
_cdx_loadpage(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, unsigned int offs, const char *__PROC__)
{
   if (offs == ro->curoffs)
      return 0;
   ro->curoffs = offs;
   return rdd_read(ClipMemoryStore, &ro->index->file, offs, sizeof(CDX_PAGE), (char *) ro->curpage, __PROC__);
}

static int
_cdx_getfreepage(ClipMachine * ClipMemoryStore, RDD_INDEX * ri, unsigned int *page, const char *__PROC__)
{
   unsigned int fsize;
   char fuu[4];
   int er;

   if ((er = rdd_read(ClipMemoryStore, &ri->file, 4, 4, fuu, __PROC__)))
      return er;
   *page = _rdd_uint((unsigned char *) fuu);
   if (!*page)
	 ...
	 if (delta < 1)
	    delta = 1;
	 *page = fsize;
	 if ((er = rdd_write(ClipMemoryStore, &ri->file, *page + delta * sizeof(CDX_PAGE), 0, "", __PROC__)))
	    return er;
	 if (delta > 1)
	    {
	       int i;
	       ...
	       if ((er = rdd_write(ClipMemoryStore, &ri->file, 4, 4, fuu, __PROC__)))
		  return er;
	       for (i = 1; i < delta; i++)
		  {
		     if (i < delta - 1)
			...
			_rdd_put_uint((unsigned char *) fuu, 0);
		     if ((er = rdd_write(ClipMemoryStore, &ri->file, *page + i * sizeof(CDX_PAGE), 4, fuu, __PROC__)))
			return er;
		  }
	    }
      }
      ...
      {
	 if ((er = rdd_read(ClipMemoryStore, &ri->file, *page, 4, fuu, __PROC__)))
	    return er;
	 if ((er = rdd_write(ClipMemoryStore, &ri->file, 4, 4, fuu, __PROC__)))
	    return er;
      }
...
static int
_cdx_putfreepage(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, unsigned int page, const char *__PROC__)
{
   char fuu[4];
   CDX_PAGE fp;
   int er;
   ...
   if ((er = rdd_read(ClipMemoryStore, &ro->index->file, 4, 4, fuu, __PROC__)))
      return er;
   if ((er = rdd_write(ClipMemoryStore, &ro->index->file, 4, 4, &page, __PROC__)))
      return er;
   _rdd_put_uint((unsigned char *) &fp, _rdd_uint((unsigned char *) fuu));
   if ((er = rdd_write(ClipMemoryStore, &ro->index->file, page, sizeof(CDX_PAGE), &fp, __PROC__)))
      return er;
   return 0;
}

static int
_cdx_init_order(ClipMachine * ClipMemoryStore, int area, RDD_INDEX * ri, unsigned int header, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_HEADER hdr;
   int er;

   if ((er = rdd_read(ClipMemoryStore, &ri->file, header, sizeof(CDX_HEADER), &hdr, __PROC__)))
      return er;
   ...
   if (strlen(ro->expr) == 0)
	 ...
	 if (ro->type == 'X')
	    ...
	    {
	       if ((er = rdd_calc(ClipMemoryStore, area, &ro->block, &v, 0)))
		  return er;
	       vp = _clip_vptr(&v);
...
static int
_cdx_init_rootleaf(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, unsigned int page, const char *__PROC__)
{
   CDX_LEAF leaf;
   int i, bits = 1, er;
   unsigned int mask;
   ...
   if ((er = rdd_write(ClipMemoryStore, &ro->index->file, page, sizeof(CDX_LEAF), &leaf, __PROC__)))
      return er;
   if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->header, 4, &page, __PROC__)))
      return er;
   return 0;
}
...
static int
_cdx_recno(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int level, unsigned int *recno, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
...
static int
_cdx_recno_keyval(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int level, unsigned int *recno, void *keyval, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
...
static int
_cdx_search_tree(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, void *key, int len, unsigned int recno, int *found, int *out, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
	 ...
	 for (i = 0; i < _rdd_ushort(branch->nkeys); i++)
	    {
	       ...
	       if ((r > 0) || ((r == 0) && (recno1 >= recno)) || (i == _rdd_ushort(branch->nkeys) - 1))
		  {
			...
			return rdd_err(ClipMemoryStore, EG_CORRUPTION, 0, __FILE__, __LINE__, __PROC__, er_deepexceeded);
		     if ((er = _cdx_recno(ClipMemoryStore, ro, (ro->level) - 1, &ro->stack[ro->level].page, __PROC__)))
			return er;
		     _CDX_PAGE(ro, ro->stack[ro->level].page, &page);
		     break;
		  }
...
static int
_cdx_back_search_tree(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, void *key, int len, unsigned int recno, int *found, int *out, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
	 ...
	 if (ro->stack[ro->level].pos < 0)
	    {
	       ...
	       if ((er = _cdx_prev_rm(ClipMemoryStore, ro, leaf, &o, 0, __PROC__)))
		  return er;
	       if (out && o)
		  {
		     *out = -1;
	 ...
	 if ((ro->stack[ro->level].pos >= _rdd_ushort(leaf->nkeys)) && !ll)
	    {
	       ...
	       if ((er = _cdx_next_rm(ClipMemoryStore, ro, leaf, &o, 0, __PROC__)))
		  return er;
	       if (out && o)
		  {
		     *out = 1;
...
static int
_cdx_first(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int *out, const char *__PROC__)
{
   CDX_HEADER hdr;
   CDX_PAGE page;
   CDX_LEAF *leaf;
   ...
   *out = 0;
   ...
   if (!(_rdd_ushort(page.attrs) & 0x1))	/* not root page */
      return rdd_err(ClipMemoryStore, EG_CORRUPTION, 0, __FILE__, __LINE__, __PROC__, er_badroot);
...
static int
_cdx_last(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int *out, const char *__PROC__)
{
   CDX_HEADER hdr;
   CDX_PAGE page;
   CDX_LEAF *leaf;
...
static int
_cdx_next_rm(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, CDX_LEAF * leaf, int *out, int cache, const char *__PROC__)
{
   int er;

   *out = 0;
   if (!leaf)
      return rdd_err(ClipMemoryStore, EG_CORRUPTION, 0, __FILE__, __LINE__, __PROC__, er_badleaf);
	 ...
	 if (_rdd_uint(leaf->right) != 0xffffffff)
	    {
	       ...
	       if (cache)
		  {
		     if ((er = _cdx_loadpage(ClipMemoryStore, ro, ro->stack[ro->level].page, __PROC__)))
			return er;
		  }
	       else
		  {
		     if ((er = rdd_read(ClipMemoryStore, &ro->index->file, ro->stack[ro->level].page, sizeof(CDX_PAGE), leaf, __PROC__)))
			return er;
		  }

	       if (_rdd_ushort(leaf->nkeys) == 0)
		  return rdd_err(ClipMemoryStore, EG_CORRUPTION, 0, __FILE__, __LINE__, __PROC__, er_nokeys);
	       ro->stack[ro->level].pos = 0;
	    }
	 else
	    {
...
static int
_cdx_next(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, int *out, unsigned int *rec, const char *__PROC__)
{
   ...
   *out = 0;
   ...
   if ((er = _cdx_next_rm(ClipMemoryStore, ro, leaf, out, 1, __PROC__)))
      return er;
...
static int
_cdx_prev_rm(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, CDX_LEAF * leaf, int *out, int cache, const char *__PROC__)
{
   int er;
...
static int
_cdx_prev(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, int *out, unsigned int *rec, const char *__PROC__)
{
   CDX_LEAF *leaf = ro->curpage;
   unsigned int recno;
   int er;
   ...
   *out = 0;
   ...
   if ((er = _cdx_prev_rm(ClipMemoryStore, ro, leaf, out, 1, __PROC__)))
      return er;
   if (rd->filter && rd->filter->rmap)
      {
	 recno = *(unsigned int *) (leaf->keys + (ro->stack[ro->level].pos) * leaf->bytes) & _rdd_uint(leaf->recmask);
...
static int
_cdx_checkscope(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, void *key, int *ok, const char *__PROC__)
{
   int lok = 1, hok = 1;
...
static int
_cdx_keyno(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, unsigned int npage, int pos, unsigned int *keyno, const char *__PROC__)
{
   CDX_PAGE page;
   int er;
   ...
   if (!ro->descend)
      {
	 if ((er = rdd_read(ClipMemoryStore, &ro->index->file, npage, 12, &page, __PROC__)))
	    return er;
...
static int
_cdx_splitleaf(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, CDX_LEAF * old, CDX_LEAF * new, unsigned int oldpage, unsigned int newpage, void *keys, const char *__PROC__)
{
   ...
   if (_rdd_uint(new->right) != 0xffffffff)
      {
	 if ((er = rdd_write(ClipMemoryStore, &ro->index->file, _rdd_uint(new->right) + 4, 4, &newpage, __PROC__)))
	    goto err;
      }
...
static int
_cdx_addroot(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, unsigned int brpage,
	     void *key1, unsigned int rec1, unsigned int page1, void *key2, unsigned int rec2, unsigned int page2, const char *__PROC__)
{
   ...
   if ((er = rdd_write(ClipMemoryStore, &ro->index->file, brpage, sizeof(CDX_BRANCH), &root, __PROC__)))
      return er;

   _rdd_put_uint((unsigned char *) b, brpage);
   if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->header, sizeof(unsigned int), b, __PROC__)))
      return er;
   return 0;
}

static int
_cdx_adjust_tree(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int level, void *key, unsigned int rec, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
   ...
   if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->stack[level].page, sizeof(CDX_BRANCH), branch, __PROC__)))
      return er;
   if (level && (ro->stack[level].pos == _rdd_ushort(branch->nkeys) - 1))
      {
	 if ((er = _cdx_adjust_tree(ClipMemoryStore, ro, level - 1,
...
static int
_cdx_addstick(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int level, void *key1, unsigned int rec1, unsigned int page1, void *key2, unsigned int rec2, unsigned int page2, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
   ...
   if (fr > ro->bufsize + 8)
	...
	/* add */
	 ...
	 if (level && (i == _rdd_ushort(branch->nkeys) - 1))
	    {
	       if ((er = _cdx_adjust_tree(ClipMemoryStore, ro, level - 1, key2, rec2, __PROC__)))
		  return er;
	    }
      }
   else
	 ...
	 if ((er = _cdx_getfreepage(ClipMemoryStore, ro->index, &newpage, __PROC__)))
	    goto err1;
	 ...
	 if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->stack[level].page, sizeof(CDX_BRANCH), branch, __PROC__)))
	    goto err1;
	 if ((er = rdd_write(ClipMemoryStore, &ro->index->file, newpage, sizeof(CDX_BRANCH), &new, __PROC__)))
	    goto err1;
	 ...
	 if (level == 0)
	    {
	       ...
	       if ((er = _cdx_getfreepage(ClipMemoryStore, ro->index, &brpage, __PROC__)))
		  goto err1;

	       if ((er = _cdx_addroot(ClipMemoryStore, ro, brpage,
	    ...
	    {
	       if ((er = _cdx_addstick(ClipMemoryStore, ro, level - 1,
...
static int
__cdx_addkey(ClipMachine * ClipMemoryStore, int area, RDD_ORDER * ro, ClipVar * v, DbfLocale * loc, unsigned int recno, const char *__PROC__)
{
   CDX_HEADER hdr;
   int found;
   int len = ro->bufsize;
   ...
   if (ro->bforexpr.ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != UNDEF_type_of_ClipVarType)
	 ...
	 if ((er = rdd_calc(ClipMemoryStore, area, &ro->bforexpr, &vv, 0)))
	    return er;
	 vp = _clip_vptr(&vv);
   ...
   if (v)
      {
	 if ((er = cdx_formatkey(ClipMemoryStore, ro, v, newkey, __PROC__)))
	    return er;
	 ...
	 if ((er = rdd_calc(ClipMemoryStore, area, &ro->block, &vv, 0)))
	    goto err1;
	 vp = _clip_vptr(&vv);
	 if ((er = cdx_formatkey(ClipMemoryStore, ro, vp, newkey, __PROC__)))
	    goto err1;
	 _clip_destroy(ClipMemoryStore, &vv);
      }
  ...
  /* Note: _cdx_back_search_tree()'s up ro->levels of ro->stack may be incorrect */
   if (ro->unique)
      {
	 if ((er = _cdx_search_tree(ClipMemoryStore, ro, newkey, ro->bufsize, 0, &found, NULL, __PROC__)))
	    goto err1;
	 if (found)
	    goto cont;
      }
   ro->level = 0;
   if ((er = _cdx_search_tree(ClipMemoryStore, ro, newkey, ro->bufsize, recno, &found, NULL, __PROC__)))
      goto err1;
   if (found)
      goto cont;
   if (!v)
   ...
   er = _cdx_addkey(ClipMemoryStore, area, loc, v, ro, newkey, len, recno, __PROC__);
 err1:
   free(newkey);
   return er;
 cont:
...
static int
_cdx_addkey(ClipMachine * ClipMemoryStore, int area, DbfLocale * loc, ClipVar * v, RDD_ORDER * ro, void *key, int len, unsigned int recno, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
   ...
   if ((fr = _cdx_leaf2split(ro, leaf, keys, ro->stack[ro->level].pos, key, len, recno, &addbytes)) <= 0)
	 ...
	 if ((er = _cdx_splitleaf(ClipMemoryStore, ro, leaf, &newleaf, ro->stack[ro->level].page, newpage, keys, __PROC__)))
	    goto err;
	 if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->stack[ro->level].page, sizeof(CDX_LEAF), leaf, __PROC__)))
	    goto err;
	 if ((er = rdd_write(ClipMemoryStore, &ro->index->file, newpage, sizeof(CDX_LEAF), &newleaf, __PROC__)))
	    goto err;
	 ...
	 if (ro->level == 0)
	    {
	       if ((er = _cdx_getfreepage(ClipMemoryStore, ro->index, &brpage, __PROC__)))
		  goto err1;
	       if ((er = _cdx_addroot(ClipMemoryStore, ro, brpage, key1, rec1, ro->stack[ro->level].page, key2, rec2, newpage, __PROC__)))
		  goto err1;
	    }
	 else
	    {
	       ...
	       if ((er = _cdx_addstick(ClipMemoryStore, ro, ro->level - 1, key1, rec1, ro->stack[ro->level].page, key2, rec2, newpage, __PROC__)))
		  goto err1;
	    }
	 free(key1);
	 free(key2);
	 if ((er = __cdx_addkey(ClipMemoryStore, area, ro, v, loc, recno, __PROC__)))
	    goto err;
	 goto cont;
       err1:
	 free(key1);
	 ...
	 if (ro->level && (ro->stack[ro->level].pos == _rdd_ushort(leaf->nkeys) - 1))
	    {
	       ...
	       if ((er = _cdx_adjust_tree(ClipMemoryStore, ro, ro->level - 1, key, rec, __PROC__)))
		  {
		     free(key);
		     goto err;
		  }
...
static int
_cdx_delstick(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int level, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
   ...
   if (_rdd_ushort(branch->nkeys) == 1)
	 ...
	 if (_rdd_uint(branch->left) != 0xffffffff)
	    {
	       if ((er = rdd_write(ClipMemoryStore, &ro->index->file, _rdd_uint(branch->left) + 8, 4, branch->right, __PROC__)))
		  return er;
	    }
	 if (_rdd_uint(branch->right) != 0xffffffff)
	    {
	       if ((er = rdd_write(ClipMemoryStore, &ro->index->file, _rdd_uint(branch->right) + 4, 4, branch->left, __PROC__)))
		  return er;
	    }
	 if (level > 0)
	    {
	       if ((er = _cdx_putfreepage(ClipMemoryStore, ro, ro->stack[level].page, __PROC__)))
		  return er;
	       if ((er = _cdx_delstick(ClipMemoryStore, ro, level - 1, __PROC__)))
		  return er;
	    }
	 else
	    {
	       if ((er = _cdx_init_rootleaf(ClipMemoryStore, ro, ro->stack[level].page, __PROC__)))
		  return er;
	    }
      }
   else
	 ...
	 if (level && (ro->stack[level].pos == _rdd_ushort(branch->nkeys)))
	    {
	       ...
	       if ((er = _cdx_adjust_tree(ClipMemoryStore, ro, level - 1, branch->keys + s, rec, __PROC__)))
		  return er;
	    }
      }
   return 0;
...
static int
_cdx_delkey(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
   ...
   if ((_rdd_ushort(leaf->nkeys) == 1) && (ro->level > 0))
	 ...
	 if (_rdd_uint(leaf->left) != 0xffffffff)
	    {
	       if ((er = rdd_write(ClipMemoryStore, &ro->index->file, _rdd_uint(leaf->left) + 8, 4, leaf->right, __PROC__)))
		  return er;
	    }
	 if (_rdd_uint(leaf->right) != 0xffffffff)
	    {
	       if ((er = rdd_write(ClipMemoryStore, &ro->index->file, _rdd_uint(leaf->right) + 4, 4, leaf->left, __PROC__)))
		  return er;
	    }
	 if ((er = _cdx_putfreepage(ClipMemoryStore, ro, ro->stack[ro->level].page, __PROC__)))
	    return er;
	 if ((er = _cdx_delstick(ClipMemoryStore, ro, ro->level - 1, __PROC__)))
	    return er;
      }
   else
      {
...
static int
cdx_open(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *tag, const char *__PROC__)
{
   ...
   if ((er = rdd_read(ClipMemoryStore, &ri->file, 0, sizeof(CDX_HEADER), &hdr, __PROC__)))
      goto err;
   if (hdr.options != 0xe0)
      {
	 free(tagbag);
	 return idx_open(ClipMemoryStore, rd, ri, tag, __PROC__);
      }

   if ((er = _cdx_init_order(ClipMemoryStore, rd->area, ri, 0, tagbag, __PROC__)))
      goto err;
   ...
   if (tag)
	 ...
	 while (!(*(--e)))
	    *e = ' ';
	 _clip_upstr(key, 10);
	 if ((er = _cdx_search_tree(ClipMemoryStore, tagbag, key, 10, 0, &found, NULL, __PROC__)))
	    goto err;
	 if (!found)
	    {
	       er = rdd_err(ClipMemoryStore, EG_OPEN, 0, __FILE__, __LINE__, __PROC__, er_notag);
	       goto err;
	    }
	 if ((er = _cdx_recno(ClipMemoryStore, tagbag, tagbag->level, &newheader, __PROC__)))
	    goto err;
	 ro = (RDD_ORDER *) malloc(sizeof(RDD_ORDER));
	 if ((er = _cdx_init_order(ClipMemoryStore, rd->area, ri, newheader, ro, __PROC__)))
	    goto err;
	 ...
	 ro->canadd = 1;
	 if ((er = rdd_read(ClipMemoryStore, &ro->index->file, 8, 4, cntcdx, __PROC__)))
	    goto err;
	 ...
	 if ((er = _cdx_first(ClipMemoryStore, tagbag, &out, __PROC__)))
	    goto err;
	 while (!out)
	    {
	       if ((er = _cdx_recno_keyval(ClipMemoryStore, tagbag, tagbag->level, &header, tagname, __PROC__)))
		  goto err;
	       ro = (RDD_ORDER *) malloc(sizeof(RDD_ORDER));
	       if ((er = _cdx_init_order(ClipMemoryStore, rd->area, ri, header, ro, __PROC__)))
		  goto err;
		  ...
		  *s = 0;
	       ...
	       ri->orders = (RDD_ORDER **) realloc(ri->orders, ri->norders * sizeof(RDD_ORDER *));
	       ri->orders[ri->norders - 1] = ro;
	       if ((er = _cdx_next(ClipMemoryStore, rd, tagbag, &out, NULL, __PROC__)))
		  goto err;
	       if ((er = rdd_read(ClipMemoryStore, &ro->index->file, 8, 4, cntcdx, __PROC__)))
		  goto err;
	       ro->cntcdx = _rdd_backuint((unsigned char *) cntcdx);
   ...
   destroy_rdd_order(ClipMemoryStore, tagbag);
   return 0;
 err:
   if (ro)
      free(ro);
...
static int
cdx_close(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__)
{
   int i;
...
static int
__cdx_sets(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int level, void *key, const char *__PROC__)
{
   CDX_BRANCH branch;
   int er;
   ...
   if (level < 0)
      return 0;
   if ((er = rdd_read(ClipMemoryStore, &ro->index->file, ro->stack[level].page, sizeof(CDX_BRANCH), &branch, __PROC__)))
      return er;
   ...
   if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->stack[level].page, sizeof(CDX_BRANCH), &branch, __PROC__)))
      return er;
   return __cdx_sets(ClipMemoryStore, ro, level - 1, key, __PROC__);
}

static int
__cdx_adds(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int level, void *key, unsigned int *newroot, const char *__PROC__)
{
   CDX_BRANCH branch;
   int er;
   ...
   if (level < 0)
	 ...
	 if ((er = _cdx_getfreepage(ClipMemoryStore, ro->index, newroot, __PROC__)))
	    return er;
	 _cdx_init_branch(ro, &branch, 0xffffffff);

	 if ((er = rdd_read(ClipMemoryStore, &ro->index->file, ro->stack[0].page, sizeof(CDX_BRANCH), &b, __PROC__)))
	    return er;
	 if (!(_rdd_ushort(b.attrs) & 0x02))
	    {
	       unsigned int page = _rdd_uint(b.left);
	       if ((er = rdd_read(ClipMemoryStore, &ro->index->file, page, sizeof(CDX_BRANCH), &b, __PROC__)))
		  return er;
	 ...
	 if ((_rdd_ushort(branch.nkeys) + 1) * (ro->bufsize + 8) > sizeof(branch.keys))
	    {
	       if ((er = _cdx_getfreepage(ClipMemoryStore, ro->index, &newbranch, __PROC__)))
		  return er;
	       if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->stack[level].page + 8, 4, &newbranch, __PROC__)))
		  return er;
	       _cdx_init_branch(ro, &branch, ro->stack[level].page);
	       ro->stack[level].page = newbranch;
	       if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->stack[level].page, sizeof(CDX_BRANCH), &branch, __PROC__)))
		  return er;
	       if ((er = __cdx_adds(ClipMemoryStore, ro, level - 1, key, newroot, __PROC__)))
		  return er;
	    }
      }
   ...
   if ((er = __cdx_sets(ClipMemoryStore, ro, level - 1, key, __PROC__)))
      return er;
   _rdd_put_ushort(branch.nkeys, _rdd_ushort(branch.nkeys) + 1);
   if ((er = rdd_write(ClipMemoryStore, &ro->index->file, (level < 0) ? *newroot : ro->stack[level].page, sizeof(CDX_BRANCH), &branch, __PROC__)))
      return er;
   return 0;
}

static int
_cdx_adds(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, void *key, const char *__PROC__)
{
   unsigned int newroot = 0;
   int er;

   if ((er = __cdx_adds(ClipMemoryStore, ro, ro->level - 1, key, &newroot, __PROC__)))
      return er;
   if (newroot)
      {
	 int i;
	 ...
	 ro->level++;
	 if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->header, sizeof(unsigned int), &newroot, __PROC__)))
	    return er;
      }
   return 0;
}
...
static int
_cdx_savebtree(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, BTREE1 * bt, int lastrec, const char *__PROC__)
{
   CDX_HEADER hdr;
   CDX_PAGE page;
   ...
   while (!eof)
	 ...
	 if (_rdd_uint(leaf->left) != 0xffffffff)
	    {
	       if ((er = rdd_write(ClipMemoryStore, &ro->index->file, _rdd_uint(leaf->left) + 8, 4, &ro->stack[ro->level].page, __PROC__)))
		  goto err;
	    }
	 if ((er = rdd_write(ClipMemoryStore, &ro->index->file, ro->stack[ro->level].page, sizeof(CDX_LEAF), leaf, __PROC__)))
	    goto err;
	 _cdx_init_leaf(ro, leaf, lastrec, ro->stack[ro->level].page);
	 if ((er = _cdx_adds(ClipMemoryStore, ro, bt1_key(bt), __PROC__)))
	    goto err;
	 eof = bt1_next(bt);
	 if (eof)
	    break;
	 if ((er = _cdx_getfreepage(ClipMemoryStore, ro->index, &ro->stack[ro->level].page, __PROC__)))
	    goto err;
      }
   free(buf);
   return 0;
...
#define D20070119
static int
_cdx_create(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, RDD_ORDER ** rop, const char *tag, const char *expr, ClipVar * block, int unique, unsigned int header, const char *__PROC__)
{
   RDD_ORDER *ro = NULL;
   CDX_HEADER hdr;
   ClipVar vv, *vp;
   ...
   for (i = 0; i < ri->norders; i++)
	 ...
	 {
	    destroy_rdd_order(ClipMemoryStore, ro);
	    ro = ri->orders[i];
	    memset(ro, 0, sizeof(RDD_ORDER));
	    break;
	 }
   ...
   *rop = ro;
   ...
   if ((er = rdd_write(ClipMemoryStore, &ri->file, ro->header, sizeof(CDX_HEADER), (char *) &hdr, __PROC__)))
      return er;
   if ((er = _cdx_getfreepage(ClipMemoryStore, ri, &newroot, __PROC__)))
      return er;
   ...
   if ((er = _cdx_init_rootleaf(ClipMemoryStore, ro, newroot, __PROC__)))
      return er;
   ...
   bt = bt1_create(ro->unique, ro->bufsize + sizeof(unsigned int), _cdx_compare, ClipMemoryStore->index_buffer_limit);
//      bt = NULL;
   ...
   if (!rd->os.lCurrent)
	 ...
	 for (rd->recno = s; rd->recno <= e; rd->recno++)
	    {
	       ...
	       if (bt)
		  {
		     ...
		     else
			{
		     ...
		     if ((er = cdx_formatkey(ClipMemoryStore, ro, vp, buf + sizeof(unsigned int), __PROC__)))
			return er;
		     _clip_destroy(ClipMemoryStore, &vv);
		     if (bt1_add(bt, ro, buf))
			{
			   if ((er = _cdx_savebtree(ClipMemoryStore, ro, bt, lastrec, __PROC__)))
			      {
				 bt1_destroy(bt);
				 return er;
			      }
			   bt1_destroy(bt);
			   bt = NULL;
			   if ((er = cdx_addkey(ClipMemoryStore, rd, ro, NULL, __PROC__)))
			      return er;
			}
		  }
	       else
	 ...
	 rd->keysincluded = 0;
      ...
      {
/*		if((er = rdd_gotop(ClipMemoryStore,rd,__PROC__))) return er;*/
	 while (!rd->eof)
	    {
	       rd->eof = 0;
	      /* check WHILE condition */
...
static int
cdx_create(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri,
	   RDD_ORDER ** rop, const char *tag, const char *expr, ClipVar * block, int unique, int first, unsigned int header, const char *__PROC__)
{
   ...
   if (first)
	 ...
	 if ((er = rdd_write(ClipMemoryStore, &ri->file, 0, sizeof(CDX_HEADER), &chdr, __PROC__)))
	    goto err;
	 if ((er = _cdx_getfreepage(ClipMemoryStore, ri, &root, __PROC__)))
	    goto err;
	 if ((er = _cdx_init_order(ClipMemoryStore, rd->area, ri, 0, tagbag, __PROC__)))
	    goto err;
	 if ((er = _cdx_init_rootleaf(ClipMemoryStore, tagbag, root, __PROC__)))
	    goto err;
	 if (strcmp(rd->name, ri->name) == 0)
      ...
      {
	 if ((er = rdd_read(ClipMemoryStore, &ri->file, 0, sizeof(CDX_HEADER), &chdr, __PROC__)))
	    goto err;
	 if ((er = _cdx_init_order(ClipMemoryStore, rd->area, ri, 0, tagbag, __PROC__)))
	    goto err;
      }
   ...
   *(e + 1) = 0;
   ...
   if ((er = _cdx_getfreepage(ClipMemoryStore, ri, &newheader, __PROC__)))
      goto err;
   if ((er = _cdx_getfreepage(ClipMemoryStore, ri, &dummy, __PROC__)))
      goto err;
   ...
   if ((er = _cdx_search_tree(ClipMemoryStore, tagbag, tn, 10, 0, &found, NULL, __PROC__)))
      goto err;
   if (found)
      {
	 if ((er = _cdx_delkey(ClipMemoryStore, tagbag, __PROC__)))
	    goto err;
      }
   oldrn = rd->recno;
   rd->recno = newheader;
   if ((er = cdx_addkey(ClipMemoryStore, rd, tagbag, &v, __PROC__)))
      goto err;
   rd->recno = oldrn;
   if ((er = _cdx_create(ClipMemoryStore, rd, ri, rop, tagname, expr, block, unique, newheader, __PROC__)))
      goto err;
   rd->recno = oldrn;
...
static int
cdx_reindex(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__)
{
   RDD_ORDER *ro;
   int lastrec, i;
   BTREE1 *bt;
   ...
   if ((er = cdx_zap(ClipMemoryStore, rd, ri, __PROC__)))
      return er;
   if ((er = rd->vtbl->lastrec(ClipMemoryStore, rd, &lastrec, __PROC__)))
      return er;
   ...
   rd->tagindexing = NULL;
   if ((er = rd->vtbl->rawgo(ClipMemoryStore, rd, oldrn, 0, __PROC__)))
      return er;
   return 0;
}

static int
cdx_zap(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__)
{
   RDD_ORDER *tagbag = malloc(sizeof(RDD_ORDER));
   int out, i;
   ...
   while (!out)
	 ...
	 ros[ntags - 1] = -1;
	 if ((er = _cdx_recno_keyval(ClipMemoryStore, tagbag, tagbag->level, &recno, tags[ntags - 1], __PROC__)))
	    goto err;
	 for (i = 0; i < ri->norders; i++)
	    {
	 ...
	 if ((er = rdd_read(ClipMemoryStore, &ri->file, recno, sizeof(CDX_HEADER), &hdrs[ntags - 1], __PROC__)))
	    goto err;
	 if ((er = _cdx_next(ClipMemoryStore, rd, tagbag, &out, NULL, __PROC__)))
	    goto err;
      }

   if ((er = rdd_trunc(ClipMemoryStore, &ri->file, sizeof(CDX_HEADER), __PROC__)))
      goto err;
   if ((er = _cdx_init_rootleaf(ClipMemoryStore, tagbag, sizeof(CDX_HEADER), __PROC__)))
      goto err;
   recno = 0;
   if ((er = rdd_write(ClipMemoryStore, &ri->file, 4, 4, &recno, __PROC__)))
      goto err;
   ...
   for (i = 0; i < ntags; i++)
	 ...
	 if ((er = _cdx_getfreepage(ClipMemoryStore, ri, &recno, __PROC__)))
	    goto err;
	 if ((er = _cdx_getfreepage(ClipMemoryStore, ri, &dummy, __PROC__)))
	    goto err;
	 oldrecno = rd->recno;
	 ...
	 if ((er = cdx_addkey(ClipMemoryStore, rd, tagbag, &v, __PROC__)))
	    goto err;
	 rd->recno = oldrecno;
	 if ((er = rdd_write(ClipMemoryStore, &ri->file, recno, sizeof(CDX_HEADER), &hdrs[ros[i]], __PROC__)))
	    goto err;
	 if ((er = _cdx_init_order(ClipMemoryStore, rd->area, ri, recno, ro, __PROC__)))
	    goto err;
	 if ((er = _cdx_getfreepage(ClipMemoryStore, ri, &newroot, __PROC__)))
	    goto err;
	 if ((er = _cdx_init_rootleaf(ClipMemoryStore, ro, newroot, __PROC__)))
...
//              if(ros[i]!=-1)
   ...
   if ((er = rdd_childs(ClipMemoryStore, rd, __PROC__)))
      return er;
   if (tags)
      {
	 for (i = 0; i < ntags; i++)
...
static int
cdx_gotop(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   ...
   if (rd->filter && rd->filter->list)
	 ...
	 if (rd->filter->listlen == 0)
	    {
	       if ((er = rdd_lastrec(ClipMemoryStore, rd, (int *) (&lastrec), __PROC__)))
		  return er;
	 ...
	 else
	    {
		  ...
		  {
		     if ((er = rd->vtbl->rawgo(ClipMemoryStore, rd, rd->filter->list[rd->filter->cursor], 0, __PROC__)))
			return er;
   ...
   if (ro->scopetop)
	 ...
	 if (!ro->descend)
	    {
	       if ((er = _cdx_search_tree(ClipMemoryStore, ro, ro->scopetop, ro->stoplen, 0, &found, &outrange, __PROC__)))
		  return er;
	    }
	 else
	    {
	       if ((er = _cdx_back_search_tree(ClipMemoryStore, ro, ro->scopetop, ro->stoplen, 0, &found, &outrange, __PROC__)))
		  return er;
	 ...
	 if (!outrange)
	    {
	       if ((er = _cdx_recno_keyval(ClipMemoryStore, ro, ro->level, &rd->recno, key, __PROC__)))
		  return er;
	       if ((er = rd->vtbl->rawgo(ClipMemoryStore, rd, rd->recno, 1, __PROC__)))
		  return er;
	       if ((er = _cdx_checkscope(ClipMemoryStore, rd, ro, key, &ok, __PROC__)))
		  return er;
	    }
	 free(key);
	 if (outrange || ok)
	 ...
	 if (!ro->descend)
	    {
	       if ((er = _cdx_first(ClipMemoryStore, ro, &out, __PROC__)))
		  return er;
	    }
	 else
	    {
	       if ((er = _cdx_last(ClipMemoryStore, ro, &out, __PROC__)))
		  return er;
	    }
	 if (out)
	    {
	    ...
	    {
	       if ((er = _cdx_recno(ClipMemoryStore, ro, ro->level, &rd->recno, __PROC__)))
		  return er;
	       if ((er = rd->vtbl->rawgo(ClipMemoryStore, rd, rd->recno, 1, __PROC__)))
		  return er;
	    }
      }
   if ((er = rdd_checkfilter(ClipMemoryStore, rd, &fok, __PROC__)))
      return er;
   if (!fok)
      {
	 if ((er = cdx_next(ClipMemoryStore, rd, ro, __PROC__)))
	    return er;
	 if (rd->eof)
	    {
	       rd->bof = rd->v_bof = 1;
...
static int
cdx_gobottom(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   unsigned int lastrec;
   int out = 0, fok, deleted, er;
   ...
   if (ro->scopebottom)
	 ...
	 if (!ro->descend)
	    {
	       if ((er = _cdx_back_search_tree(ClipMemoryStore, ro, ro->scopebottom, ro->sbotlen, 0, &found, &outrange, __PROC__)))
		  return er;
	       if (outrange > 0 && ro->stack[ro->level].pos > 0)
		  {
		     ro->stack[ro->level].pos--;
	    ...
	    {
	       if ((er = _cdx_search_tree(ClipMemoryStore, ro, ro->scopebottom, ro->sbotlen, 0, &found, &outrange, __PROC__)))
		  return er;
	    }
	 if (!outrange)
	    {
   ...
   if (!fok)
      {
	 if ((er = cdx_prev(ClipMemoryStore, rd, ro, __PROC__)))
	    return er;
	 if (rd->bof)
	    {
	       unsigned int lastrec;
...
static int
cdx_next(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_HEADER hdr;
   int out = 0, found;
   ClipVar v, *vp;
	 ...
	 if (rd->shared || !ro->valid_stack)
	    {
	       ...
	       if (!ro->valid_stack || ro->cntcdx != _rdd_backuint((unsigned char *) cntcdx))
		  {
			...
			{
			   if ((er = rdd_calc(ClipMemoryStore, rd->area, &ro->block, &v, 0)))
			      return er;
			   vp = _clip_vptr(&v);
			   if ((er = cdx_formatkey(ClipMemoryStore, ro, vp, key, __PROC__)))
			      return er;
			   _clip_destroy(ClipMemoryStore, &v);
			}
		     ro->curoffs = 0;
		     ...
		     if (!ro->descend)
			{
			   if ((er = _cdx_search_tree(ClipMemoryStore, ro, key, ro->bufsize, rd->recno, &found, NULL, __PROC__)))
			      return er;
			}
		     else
			{
			   if ((er = _cdx_back_search_tree(ClipMemoryStore, ro, key, ro->bufsize, rd->recno, &found, NULL, __PROC__)))
			      return er;
			}
		    //    _clip_destroy(ClipMemoryStore,&v);
		  }
	 ...
	 while (ok || !fok)
	    {
	       ...
	       if (!ro->descend)
		  {
		     if ((er = _cdx_next(ClipMemoryStore, rd, ro, &out, &recno, __PROC__)))
			return er;
		  }
	       else
		  {
		     if ((er = _cdx_prev(ClipMemoryStore, rd, ro, &out, &recno, __PROC__)))
			return er;
		  }
	       if (out)
		  break;
		  ...
		  return er;
	       if (!ro->scopetop && !ro->scopebottom && !rd->filter && !(ClipMemoryStore->flags & DELETED_FLAG))
		  break;
	       if ((er = _cdx_recno_keyval(ClipMemoryStore, ro, ro->level, &rd->recno, key, __PROC__)))
		  return er;
	       if ((er = _cdx_checkscope(ClipMemoryStore, rd, ro, key, &ok, __PROC__)))
...
static int
cdx_prev(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_HEADER hdr;
   int out, found;
   ClipVar v, *vp;
   ...
   if (rd->eof)
      {
	 rd->eof = 0;
	 if ((er = cdx_gobottom(ClipMemoryStore, rd, ro, __PROC__)))
	    return er;
	 rd->eof = rd->bof;
	 return 0;
      }
   ...
   if (rd->filter && rd->filter->list)
	 ...
	 if (rd->filter->listlen == 0)
	    {
	       rd->eof = 1;
	       if ((er = rdd_lastrec(ClipMemoryStore, rd, &lastrec, __PROC__)))
		  goto err1;
	       if ((er = rd->vtbl->rawgo(ClipMemoryStore, rd, lastrec + 1, 0, __PROC__)))
		  goto err1;
	    }
	 else
	    {
	       ...
	       if ((er = rd->vtbl->rawgo(ClipMemoryStore, rd, rd->filter->list[oldcursor], 0, __PROC__)))
		  goto err1;
	    }
	 goto cont;
      }
...
static int
cdx_seek(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, ClipVar * v, int soft, int last, int *found, const char *__PROC__)
{
   CDX_HEADER hdr;
   void *key = malloc(ro->bufsize);
   ...
   ro->level = 0;
   rd->bof = rd->v_bof = rd->eof = 0;
   if ((er = cdx_formatkey(ClipMemoryStore, ro, v, key, __PROC__)))
      return er;
   ...
   if ((last && !ro->descend) || (!last && ro->descend))
      {
	 if ((er = _cdx_back_search_tree(ClipMemoryStore, ro, key, len, 0, found, &outrange, __PROC__)))
	    return er;
	 if (last && outrange < 0)
	    {
	       outrange = 0;
      ...
      {
	 if ((er = _cdx_search_tree(ClipMemoryStore, ro, key, len, 0, found, &outrange, __PROC__)))
	    return er;
      }
	 ...
	 if (!fok)
	    {
	       ...
	       else
		  {
			...
			return er;
		     vp = _clip_vptr(&v);
		     if ((er = cdx_formatkey(ClipMemoryStore, ro, vp, k, __PROC__)))
			return er;
		     _clip_destroy(ClipMemoryStore, &v);
		  }
...
static int
cdx_gotokey(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, unsigned int keyno, int *ok, const char *__PROC__)
{
   CDX_PAGE page;
   int out, er;
   unsigned int c = 0;
   ...
   *ok = 0;
   if (ro->scopetop)
	 ...
	 if (!ro->descend)
	    {
	       if ((er = _cdx_search_tree(ClipMemoryStore, ro, ro->scopetop, ro->stoplen, 0, &found, NULL, __PROC__)))
		  return er;
	    }
	 else
	    {
	       if ((er = _cdx_back_search_tree(ClipMemoryStore, ro, ro->scopetop, ro->stoplen, 0, &found, NULL, __PROC__)))
		  return er;
	    }
	 if ((er = _cdx_keyno(ClipMemoryStore, ro, ro->stack[ro->level].page, ro->stack[ro->level].pos, &topno, __PROC__)))
	    return er;
	 keyno += topno - 1;
      }
   if (!ro->descend)
	 ...
	 if ((er = rdd_read(ClipMemoryStore, &ro->index->file, ro->stack[ro->level].page, 12, &page, __PROC__)))
	    return er;
	 while (c + _rdd_ushort(page.nkeys) < keyno)
	    {
	       if (_rdd_uint(page.right) == 0xffffffff)
		  {
	       ...
	       if ((er = rdd_read(ClipMemoryStore, &ro->index->file, _rdd_uint(page.right), 12, &page, __PROC__)))
		  return er;
	    }
	 ...
	 while (c + _rdd_ushort(page.nkeys) < keyno)
	    {
	       if (_rdd_uint(page.left) == 0xffffffff)
		  {
	       ...
	       if ((er = rdd_read(ClipMemoryStore, &ro->index->file, _rdd_uint(page.left), 12, &page, __PROC__)))
		  return er;
	    }
   ...
   if (ro->scopetop || ro->scopebottom)
	 ...
	 if ((ro->type == 'C' || (ro->type == 'X' && ((char *) key)[0] == type_weight(CHARACTER_type_of_ClipVarType))) && !ro->binary)
	    loc_write(rd->loc, key, ro->bufsize);
	 if ((er = _cdx_checkscope(ClipMemoryStore, rd, ro, key, &ook, __PROC__)))
	    return er;
	 free(key);
	 if (ook)
	    {
...
static int
cdx_descend(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, int descend, int *res, const char *__PROC__)
{
   *res = ro->descend;
   if (descend != -1)
      ro->descend = descend;
...
static int
cdx_addkey(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, ClipVar * v, const char *__PROC__)
{
   int r = __cdx_addkey(ClipMemoryStore, rd->area, ro, v, rd->loc, rd->recno, __PROC__);
   int er;
   char cntcdx[4];
   ...
   if (!r)
	 ...
	 if ((er = rdd_write(ClipMemoryStore, &ro->index->file, 8, 4, cntcdx, __PROC__)))
	    return er;
      }
   return r;
}
...
static int
cdx_delkey(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_HEADER hdr;
   int found, er;
   ClipVar vv, *vp;
   ...
   if ((er = _cdx_delkey(ClipMemoryStore, ro, __PROC__)))
      goto err;
   ro->valid_stack = 0;
...
static int
cdx_formatkey(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, ClipVar * var, void *res, const char *__PROC__)
{
...
static int
cdx_keyno(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, int *keyno, const char *__PROC__)
{
   CDX_HEADER hdr;
   int found, ok, er;
   ClipVar v, *vp;
   ...
   *keyno = 0;
   ...
   if (ok)
      return 0;
   if ((er = _cdx_keyno(ClipMemoryStore, ro, ro->stack[ro->level].page, ro->stack[ro->level].pos, (unsigned int *) keyno, __PROC__)))
      return er;
   if (ro->scopetop)
      {
	 unsigned int topno;
	 ...
	 if ((er = _cdx_keyno(ClipMemoryStore, ro, ro->stack[ro->level].page, ro->stack[ro->level].pos, (unsigned int *) &topno, __PROC__)))
	    return er;
	 *keyno -= topno - 1;
      }
   return 0;
...
static int
cdx_lastkey(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, int *lastkey, const char *__PROC__)
{
   ...
   if (ro->scopetop)
	 ...
	 if ((er = _cdx_keyno(ClipMemoryStore, ro, ro->stack[ro->level].page, ro->stack[ro->level].pos, &b, __PROC__)))
	    return er;
      }
   ...
   if (ro->scopebottom)
	 ...
	 if (!ro->descend)
	    {
	       if ((er = _cdx_back_search_tree(ClipMemoryStore, ro, ro->scopebottom, ro->sbotlen, 0, &found, NULL, __PROC__)))
		  return er;
	    }
	 else
	    {
	       if ((er = _cdx_search_tree(ClipMemoryStore, ro, ro->scopebottom, ro->sbotlen, 0, &found, NULL, __PROC__)))
		  return er;
	    }
	 if ((er = _cdx_keyno(ClipMemoryStore, ro, ro->stack[ro->level].page, ro->stack[ro->level].pos, &e, __PROC__)))
	    return er;
      }
   else
      {
...
static int
cdx_info(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, int cmd, const char *__PROC__)
{
   switch (cmd)
      {
      case DBOI_CONDITION:
	 {
	    if (ro->cforexpr)
	       _clip_retc(ClipMemoryStore, ro->cforexpr);
	    else
      ...
      case DBOI_HPLOCKING:
	 {
      ...
      case DBOI_ISCOND:
	 {
	    _clip_retl(ClipMemoryStore, ro->bforexpr.ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != UNDEF_type_of_ClipVarType);
	    break;
	 }
      case DBOI_ISDESC:
	 {
	    _clip_retl(ClipMemoryStore, ro->descend);
	    break;
	 }
      case DBOI_ORDERCOUNT:
	 {
	    _clip_retni(ClipMemoryStore, ro->index->norders);
	    break;
	 }
...
static int
cdx_keyvalue(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, ClipVar * v, const char *__PROC__)
{
   CDX_HEADER hdr;
   ClipVar vv, *vp;
   int found, er;
...
static int
cdx_setscope(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, ClipVar * top, ClipVar * bottom, unsigned int *map, int bytes, int exact, const char *__PROC__)
{
   void *t = NULL;
   void *b = NULL;
   ...
   if (top)
      {
	 t = malloc(ro->bufsize);
	 if ((er = cdx_formatkey(ClipMemoryStore, ro, top, t, __PROC__)))
	    goto err;
   ...
   if (bottom)
      {
	 b = malloc(ro->bufsize);
	 if ((er = cdx_formatkey(ClipMemoryStore, ro, bottom, b, __PROC__)))
	    goto err;
   ...
   if (b)
      {
	 ro->level = 0;
	 if ((er = _cdx_back_search_tree(ClipMemoryStore, ro, b, blen, 0, &found, &bout, __PROC__)))
	    goto err;
	 if (bout < 0)
	    goto e;
	 if ((er = _cdx_recno(ClipMemoryStore, ro, ro->level, &lastrecno, __PROC__)))
	    goto err;
	 if (!found && !bout)
	    {
	       if ((er = rdd_read(ClipMemoryStore, &ro->index->file, ro->stack[ro->level].page, sizeof(CDX_LEAF), &leaf, __PROC__)))
		  goto err;
	       if ((er = _cdx_next_rm(ClipMemoryStore, ro, &leaf, &bout, 0, __PROC__)))
		  goto err;
	       if ((er = _cdx_recno(ClipMemoryStore, ro, ro->level, &lastrecno1, __PROC__)))
		  goto err;
	    }
      }
   if (t)
      {
	 ro->level = 0;
	 if ((er = _cdx_search_tree(ClipMemoryStore, ro, t, tlen, 0, &found, &tout, __PROC__)))
	    goto err;
	 if (tout > 0)
	    goto e;
	 if ((er = _cdx_recno(ClipMemoryStore, ro, ro->level, &firstrecno, __PROC__)))
	    goto err;
      }
   else
      {
...
/* --------------------------- */
   ...
   while (!out && recno != lastrecno)
	 ...
	 if ((er = _cdx_next_rm(ClipMemoryStore, ro, &leaf, &out, 0, __PROC__)))
	    goto err;
	 recno = *(unsigned int *) (leaf.keys + (ro->stack[ro->level].pos) * leaf.bytes) & _rdd_uint(leaf.recmask);
      }
...
static int
cdx_destroy(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_HEADER chdr;
   RDD_ORDER *tagbag = (RDD_ORDER *) malloc(sizeof(RDD_ORDER));
   char tn[11];
   ...
   tagbag->level = 0;
   if ((er = rdd_read(ClipMemoryStore, &ro->index->file, 0, sizeof(CDX_HEADER), &chdr, __PROC__)))
      goto err;
   if ((er = _cdx_init_order(ClipMemoryStore, rd->area, ro->index, 0, tagbag, __PROC__)))
      goto err;
   strcpy(tn, ro->name);
   memset(tn + strlen(tn), 0x20, 10 - strlen(tn));
   tn[10] = 0;
...
static int
cdx_ii_create(ClipMachine * ClipMemoryStore, RDD_INDEX * ri, const char *__PROC__)
{
   CDX_HEADER chdr;
   RDD_ORDER *tagbag = malloc(sizeof(RDD_ORDER));
   unsigned int root;
   ...
   ri->loc = dbf_get_locale(ClipMemoryStore);
   root = sizeof(CDX_HEADER);
   memset(&chdr, 0, sizeof(CDX_HEADER));
   _rdd_put_uint(chdr.root, sizeof(CDX_HEADER));
   _rdd_put_ushort(chdr.keysize, 10);
   ...
   if ((er = _cdx_getfreepage(ClipMemoryStore, ri, &root, __PROC__)))
      goto err;
   if ((er = _cdx_init_order(ClipMemoryStore, -1, ri, 0, tagbag, __PROC__)))
      goto err;
   if ((er = _cdx_init_rootleaf(ClipMemoryStore, tagbag, root, __PROC__)))
      goto err;
...
static int
cdx_ii_createtag(ClipMachine * ClipMemoryStore, RDD_INDEX * ri, const char *tag, const char *expr, RDD_ORDER ** rop, const char *__PROC__)
{
   CDX_HEADER chdr;
   RDD_ORDER *tagbag = malloc(sizeof(RDD_ORDER));
   char tagname[11];
   ...
   if ((er = __cdx_addkey(ClipMemoryStore, -1, tagbag, &v, ri->loc, newheader, __PROC__)))
      goto err;
   ...
   strcpy(ro->expr, expr);
   {
      ...
      if ((er = _clip_eval_macro(ClipMemoryStore, e, strlen(e), &block)))
	 return er;
   }
   if ((er = rdd_calc(ClipMemoryStore, -1, &block, &vv, 0)))
      return er;
   vp = _clip_vptr(&vv);
   ...
   *rop = ro;
   ...
   _clip_destroy(ClipMemoryStore, &vv);
   _clip_destroy(ClipMemoryStore, &block);
   destroy_rdd_order(ClipMemoryStore, tagbag);
   return 0;
 err:
   _clip_destroy(ClipMemoryStore, &vv);
...
static int
cdx_ii_open(ClipMachine * ClipMemoryStore, RDD_INDEX * ri, const char *__PROC__)
{
   ri->loc = dbf_get_locale(ClipMemoryStore);
   ri->orders = malloc(0);
   ri->norders = 0;
...
static int
cdx_ii_opentag(ClipMachine * ClipMemoryStore, RDD_INDEX * ri, const char *tag, RDD_ORDER ** rop, const char *__PROC__)
{
   CDX_HEADER chdr;
   CDX_PAGE page;
   CDX_LEAF *leaf;
   ...
   if ((er = _cdx_recno_keyval(ClipMemoryStore, tagbag, tagbag->level, &header, tn, __PROC__)))
      return er;

   if (!found)
      return rdd_err(ClipMemoryStore, EG_OPEN, 0, __FILE__, __LINE__, __PROC__, er_notag);

   ro = (RDD_ORDER *) malloc(sizeof(RDD_ORDER));
   if ((er = _cdx_init_order(ClipMemoryStore, -1, ri, header, ro, __PROC__)))
      return er;
...
static int
cdx_ii_addkey(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *cid, ClipVar * key, const char *__PROC__)
{
   unsigned int id = *(unsigned int *) cid;
   int er;

   if ((er = __cdx_addkey(ClipMemoryStore, -1, ro, key, ro->index->loc, id, __PROC__)))
      return er;
...
static int
cdx_ii_delkey(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_HEADER hdr;
   int found, er;
   ...
   if ((er = _cdx_search_tree(ClipMemoryStore, ro, ro->iikey, ro->bufsize, *(unsigned int *) ro->id, &found, NULL, __PROC__)))
      return er;
   if (!found)
      return 0;

   return _cdx_delkey(ClipMemoryStore, ro, __PROC__);
}

static int
cdx_ii_gotop(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *__PROC__)
{
   int out, er;
   ...
   if ((er = _cdx_recno_keyval(ClipMemoryStore, ro, ro->level, (unsigned int *) ro->id, ro->iikey, __PROC__)))
      return er;
   if ((ro->type == 'C' || (ro->type == 'X' && ((char *) ro->iikey)[0] == type_weight(CHARACTER_type_of_ClipVarType))) && !ro->binary)
      loc_read(ro->index->loc, (unsigned char *) ro->iikey, ro->bufsize);
   return 0;
...
static int
cdx_ii_gobottom(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *__PROC__)
{
   int out, er;
...
static int
cdx_ii_id(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, ClipVar * v, const char *__PROC__)
{
   v->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType = CHARACTER_type_of_ClipVarType;
   v->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType = F_NONE_ClipFlags;
...
static int
cdx_ii_key(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, ClipVar * v, const char *__PROC__)
{
   memset(v, 0, sizeof(ClipVar));
   if (ro->eof)
      {
...
static int
cdx_ii_next(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
   ...
   ro->level = 0;
   if (ro->eof)
      return cdx_ii_gobottom(ClipMemoryStore, ro, __PROC__);
   ro->bof = 0;
   _CDX_HEADER(ro, &hdr);
   ro->stack[0].page = _rdd_uint(hdr.root);

   if ((er = _cdx_search_tree(ClipMemoryStore, ro, ro->iikey, ro->bufsize, *(unsigned int *) ro->id, &found, &out, __PROC__)))
      return er;
   if (!found)
      {
	 if (out)
	    {
	       if ((er = cdx_ii_gobottom(ClipMemoryStore, ro, __PROC__)))
		  return er;
	       if ((er = cdx_ii_next(ClipMemoryStore, ro, __PROC__)))
		  return er;
	       return 0;
	    }
	 if ((er = _cdx_recno_keyval(ClipMemoryStore, ro, ro->level, (unsigned int *) ro->id, ro->iikey, __PROC__)))
...
static int
cdx_ii_prev(ClipMachine * ClipMemoryStore, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_PAGE page;
   CDX_LEAF *leaf;
   CDX_BRANCH *branch;
   ...
   if (!found && out)
      {
	 if ((er = cdx_ii_gotop(ClipMemoryStore, ro, __PROC__)))
	    return er;
	 if ((er = cdx_ii_prev(ClipMemoryStore, ro, __PROC__)))
	    return er;
      }
   _CDX_PAGE(ro, ro->stack[ro->level].page, &page);
   if (!leaf)
   ...
   if ((er = _cdx_prev_rm(ClipMemoryStore, ro, leaf, &out, 0, __PROC__)))
      return er;
   if (!out)
      {
	 if ((er = _cdx_recno_keyval(ClipMemoryStore, ro, ro->level, (unsigned int *) ro->id, ro->iikey, __PROC__)))
...
int
cdx_calcfiltlist(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, RDD_FILTER * fp, const char *__PROC__)
{
   int i, er;
   BTREE *bt;
   ...
   if (fp->list)
	 ...
	 for (i = 0; i < fp->listlen; i++)
	    {
	       if ((er = rd->vtbl->rawgo(ClipMemoryStore, rd, fp->list[i], 0, __PROC__)))
		  return er;
	       if (ro->simpexpr)
		  {
		     if ((er = rd->vtbl->getvalue(ClipMemoryStore, rd, ro->simpfno, &vv, __PROC__)))
	       ...
	       else
		  {
	       ...
	       if ((er = cdx_formatkey(ClipMemoryStore, ro, vp, key + sizeof(unsigned int), __PROC__)))
		  return er;
	       _clip_destroy(ClipMemoryStore, &vv);
	       bt_add(bt, ro, key);
	    }
	 ...
	 for (i = 1; i <= fp->size; i++)
	    {
	       if (_rm_getbit(fp->rmap, fp->size, i))
		  {
		     if ((er = rd->vtbl->rawgo(ClipMemoryStore, rd, i, 0, __PROC__)))
			return er;
		     if (ro->simpexpr)
			{
			   if ((er = rd->vtbl->getvalue(ClipMemoryStore, rd, ro->simpfno, &vv, __PROC__)))
...
static int
cdx_buildtree(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *__PROC__)
{
   CDX_HEADER hdr;
   ClipVar v, *vp;
   int found, er;
   ...
   if (!rd->shared && !rd->eof)
	 ...
	 _clip_destroy(ClipMemoryStore, &v);
/*		if(!found){
			return rdd_err(ClipMemoryStore,EG_CORRUPTION,0,__FILE__,__LINE__,__PROC__,
				er_nokey);
		}*/
	 ro->valid_stack = 1;
      }
...
static int
idx_create(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri,
	   RDD_ORDER ** rop, const char *tag, const char *expr, ClipVar * block, int unique, int first, unsigned int header, const char *__PROC__)
{
   int er;

   if ((er = _cdx_create(ClipMemoryStore, rd, ri, rop, ri->name, expr, block, unique, 0, __PROC__)))
      goto err;
...
static int
idx_open(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *tag, const char *__PROC__)
{
   RDD_ORDER *ro = NULL;
   int er;
   unsigned int newheader;
...
static int
idx_zap(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__)
{
   CDX_HEADER chdr;
   ...
   if ((er = rdd_trunc(ClipMemoryStore, &ri->file, sizeof(CDX_HEADER), __PROC__)))
      goto err;
   if ((er = _cdx_init_rootleaf(ClipMemoryStore, ro, sizeof(CDX_HEADER), __PROC__)))
      goto err;
   if ((er = rdd_read(ClipMemoryStore, &ri->file, 0, sizeof(CDX_HEADER), &chdr, __PROC__)))
      goto err;
...
static int
idx_reindex(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_INDEX * ri, const char *__PROC__)
{
   RDD_ORDER *ro;
   int lastrec;
   BTREE1 *bt;
   ...
   if ((er = idx_zap(ClipMemoryStore, rd, ri, __PROC__)))
      return er;
   if ((er = rd->vtbl->lastrec(ClipMemoryStore, rd, &lastrec, __PROC__)))
      return er;
...
static int
cdx_wildskip(ClipMachine * ClipMemoryStore, RDD_DATA * rd, RDD_ORDER * ro, const char *pattern, int regular, const char *s, ClipVar * block, int *found, const char *__PROC__)
{
   CDX_HEADER hdr;
   CDX_PAGE page;
   CDX_LEAF *leaf = (CDX_LEAF *) & page;
	 ...
	 if (!ro->valid_stack || ro->cntcdx != _rdd_backuint((unsigned char *) cntcdx))
	    {
		  ...
		  goto err;
	       ...
	       if (!ro->descend)
		  {
		     if ((er = _cdx_search_tree(ClipMemoryStore, ro, key, ro->bufsize, rd->recno, &fnd, NULL, __PROC__)))
			goto err;
		  }
	       else
		  {
		     if ((er = _cdx_back_search_tree(ClipMemoryStore, ro, key, ro->bufsize, rd->recno, &fnd, NULL, __PROC__)))
			goto err;
		  }
	       _clip_destroy(ClipMemoryStore, &v);
	    }
	 ...
	 if (!block)
	    {
	       ...
	       else
		  {
	       ...
	       if (*found)
		  {
		     if ((er = rd->vtbl->rawgo(ClipMemoryStore, rd, rec, 1, __PROC__)))
			goto err;
		     if (!ro->scopetop && !ro->scopebottom && !rd->filter && !(ClipMemoryStore->flags & DELETED_FLAG))
			break;
		     if ((er = _cdx_checkscope(ClipMemoryStore, rd, ro, key, &ok, __PROC__)))
	 ...
	 else
	    {
	       ...
	       if (!ok && fok)
		  {
		     ...
		     if ((er = _clip_eval(ClipMemoryStore, block, 1, &v, &r)))
			goto err;
