       ...
       if (fp1)
	{
	  /* we have a range; expect the word "idem" or a Unicode range of the
	   ...
	   if (!strncmp(p, "idem", 4))
		...
		{
		   fprintf(stderr,
			   "line %d: Corresponding to a range of font positions, " "there should be a Unicode range\n", line);
		   goto ret;
		}
	       p++;
	       un1 = getunicode(&p);
		...
		{
		   fprintf(stderr,
			   "line %d: Bad Unicode range corresponding to " "font position range 0x%x-0x%x\n", line, fp0, fp1);
		   goto ret;
		}
	       if (un1 - un0 != fp1 - fp0)
		{
...
int
load_charset_tables(char *bname, unsigned char *cmptbl, unsigned char *uptbl,
		    unsigned char *lowtbl, unsigned char *alphatbl, unsigned char *pgtbl)
{
   char      buf[256];
