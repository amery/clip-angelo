...
clip_SYSCMD(ClipMachine * ClipMachineMemory)
{
   ...
   if (out
       && (out->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != CHARACTER_type_of_ClipVarType
	   || out->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType == F_MSTAT_ClipFlags))
      return EG_ARG;

   if (err
       && (err->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != CHARACTER_type_of_ClipVarType
	   || err->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType == F_MSTAT_ClipFlags))
      return EG_ARG;
   ...
   if (cenv && cenv->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == ARRAY_type_of_ClipVarType)
       ...
       for (i = 0; i < cenv->ClipArrVar_a_of_ClipVar.count_of_ClipArrVar; i++)
	{
	   ...
	   if (e1 || e2 || l.ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != CHARACTER_type_of_ClipVarType
	       || r.ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != CHARACTER_type_of_ClipVarType)
	      continue;
	   s =
	    calloc(1,
		   l.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf +
		   r.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 2);
	   memcpy(s, l.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		  l.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
	   ...
	   memcpy(s + l.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 1,
		  ...
		  r.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
	   env[i] = s;
   ...
   for (;;)
    {
	   ...
	   && (outfd < 0 || !FD_ISSET(outfd, &r_fds)) && (!have_err || errfd < 0 || !FD_ISSET(errfd, &r_fds)))
	{
	   break;
	}
