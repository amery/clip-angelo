...
_clip_creat(ClipMachine * ClipMachineMemory, char *file, int flags, mode_t mode, int exclusive)
{
   ...
   */
   ...
   h = CreateFile(wp, GENERIC_READ | GENERIC_WRITE,
		  exclusive ? 0 : (FILE_SHARE_READ | FILE_SHARE_WRITE), NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
_clip_open(ClipMachine * ClipMachineMemory, char *file, int flags, mode_t mode, int exclusive)
{
   ...
   */
   ...
   h = CreateFile(wp, access,
		  exclusive ? 0 : (FILE_SHARE_READ | FILE_SHARE_WRITE), NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
   setmode((int) h, O_BINARY);
   if (h == INVALID_HANDLE_VALUE)
    {
       if (GetLastError())
...
int
clip_FTYPE(ClipMachine * ClipMachineMemory)
{
   int       fd = _clip_parni(ClipMachineMemory, 1), *err = _clip_fetch_item(ClipMachineMemory, HASH_ferror);
   ...
   if (cf == NULL)
    ...
    {
       *err = 0;
       _clip_retni(ClipMachineMemory, cf->type_of_C_FILE);
    }
   return 0;
}
...
int
clip_FREADREADY(ClipMachine * ClipMachineMemory)
{
   return _clip_fready(ClipMachineMemory, 1);
}
...
clip___FILELOCK(ClipMachine * ClipMachineMemory)
{
   ...
   *err = 0;
   ...
   if (_clip_setlock(ClipMachineMemory, cf->hash_of_C_FILE, cf->fileno_of_C_FILE, 1000000000 + 1,
...
int
clip_ISPRINTER(ClipMachine * ClipMachineMemory)
{
   _clip_retl(ClipMachineMemory, 1);
   return 0;
}
...
int
clip_OS(ClipMachine * ClipMachineMemory)
{
   _clip_retc(ClipMachineMemory, OSNAME);
   return 0;
}
...
int
clip_MEMOWRITE(ClipMachine * ClipMachineMemory)
{
   return clip_MEMOWRIT(ClipMachineMemory);
}
...
clip_DIRECTORY(ClipMachine * ClipMachineMemory)
{
   ...
   while (dirp != NULL)
       ...
       if (S_ISDIR(statbuf.st_mode) && !flag_dir)
	{
       ...
       var.ClipDateVar_d_of_ClipVar.julian_of_ClipDateVar =
	_clip_jdate(sysTime->tm_mday, sysTime->tm_mon + 1, sysTime->tm_year + 1900);
       vect[1] = 2;
       _clip_aset(ClipMachineMemory, rp, &var, 2, vect);
