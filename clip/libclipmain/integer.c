...
integer_sadd(integer * d1, integer * d2)
{
   ...
   for (k = 0; k < i; k++)
      {
	 sum += ((INTEGER_FULL_TYPE) (d1->vec[k])) + ((INTEGER_FULL_TYPE) (d2->vec[k]));
	 d1->vec[k] = EXTRACT(sum);
	 sum >>= INTEGER_HALF;
      }
   for (; sum != 0; k++)
...
/**********************************************/
integer *
integer_ora(integer * d1, integer * d2)
{
   int i;
   if (d1->len < d2->len)
...
/**********************************************/
integer *
integer_anda(integer * d1, integer * d2)
{
   int i;
   if (d1->len > d2->len)
      integer_resize(d1, d2->len);
