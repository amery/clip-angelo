...
static void get_static(ClipMachine * ClipMemoryStore, ClipFile * file, ClipVar * statics, char *modbeg, int no, ClipVar ** dest);
static void get_func(char *modbeg, int no, char **fp);
static int run_vm(ClipMachine * ClipMemoryStore, ClipBlock * bp);
...
static int
load_pobj(ClipMachine * ClipMemoryStore, struct ClipFile *file, const char *name, int mallocMem)
{
   struct stat st;
   int fd;
   long len;
   ...
   if (stat(name, &st) != 0)
      {
	 _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "cannot stat file '%s': %s", name, strerror(errno));
	 return _clip_call_errblock(ClipMemoryStore, 1);
      }

   if (!S_ISREG(st.st_mode))
      {
	 _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "is not regular file '%s'", name);
	 return _clip_call_errblock(ClipMemoryStore, 1);
      }
   ...
   if (len < 38)
      {
	 _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "is not pobj file '%s'", name);
	 return _clip_call_errblock(ClipMemoryStore, 1);
      }
   ...
   if (fd < 0)
      {
	 _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "open file '%s' error: %s", name, strerror(errno));
	 return _clip_call_errblock(ClipMemoryStore, 1);
      }
...
if (file->body_of_ClipFile == (char *) -1)
	    {
	       _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "mmap file '%s' error: %s", name, strerror(errno));
	       return _clip_call_errblock(ClipMemoryStore, -1);
	    }
      }
   else
	 ...
	 for (s = file->body_of_ClipFile, readed = 0; readed < len; readed += r, s += r)
	    {
	       ...
	       if (r < portion)
		  {
		     ...
		     _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "read file '%s' error: %s", name, strerror(errno));
		     return _clip_call_errblock(ClipMemoryStore, -1);
		  }
	       close(fd);
...
file->body_of_ClipFile = sp;
   ...
   if (memcmp(sp, "!<pobj>\n", 8))
      {
	 _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, " '%s' is not valid pobj file: invalid magic", name);
	 destroy_ClipFile(ClipMemoryStore, file);
	 return _clip_call_errblock(ClipMemoryStore, 1);
      }
  ...
  /*
     ...
     {
     _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, " '%s' is not valid pobj file: invalid length", name);
     destroy_ClipFile(ClipMemoryStore, file);
     return 1;
     }
   */
...
void
_clip_hash_buckets(ClipMachine * ClipMemoryStore, ClipFile * file)
{
   char *hp = 0;
   int len = 0;
   int count = 0;
	 ...
	 if (_clip_debuglevel)
	    {
	       ...
	       if (!s)
		  {
		     ...
		     s = strdup(str);
		     HashTable_store(ClipMemoryStore->hashnames, s, hash);
		  }
	    }
      }
...
static int
load_dll(ClipMachine * ClipMemoryStore, const char *name, struct Coll *names, ClipVar * resp)
{
   void *hp;
   char buf[256], *s, *e;
   char uname[128];
   ...
   if (!hp)
      {
	 _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "shared loading problem: %s: file %s", dlerror(), buf);
	 return _clip_call_errblock(ClipMemoryStore, 1);
      }
 ...
 */
   ...
   if (!entry)
      {
	 _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "shared '%s' fetch name '%s' problem: %s", name, buf, dlerror());
	 return _clip_call_errblock(ClipMemoryStore, 1);
      }

for (np = entry->ClipNameEntry_cfunctions_of_ClipModule; np && np->ClipFunction_function_of_ClipNameEntry; ++np)
	_clip_register_hash(ClipMemoryStore, np->ClipFunction_function_of_ClipNameEntry, np->hash_of_ClipNameEntry);

for (fpp = entry->ClipFunction_inits_of_ClipModule; fpp && *fpp; ++fpp)
      _clip_main_func(ClipMemoryStore, *fpp, _clip_argc, _clip_argv, _clip_envp);

for (fpp = entry->ClipFunction_exits_of_ClipModule; fpp && *fpp; ++fpp)
      {
	 ClipMemoryStore->cexits = (ClipFunction **) realloc(ClipMemoryStore->cexits, (ClipMemoryStore->ncexits + 1) * sizeof(ClipFunction *));
	 ClipMemoryStore->cexits[ClipMemoryStore->ncexits] = *fpp;
	 ++ClipMemoryStore->ncexits;
      }

for (spp = entry->pfunctions_of_ClipModule; spp && *spp; ++spp)
      if (_clip_load(ClipMemoryStore, *spp, 0, 0))
	 ++ret;
/*
	if (entry->cpfile && _clip_register_file(ClipMemoryStore, entry->cpfile))
		++ret;
*/
for (cpp = entry->cpfiles_of_ClipModule; cpp && *cpp; ++cpp)
      if (_clip_register_file(ClipMemoryStore, *cpp))
	 ++ret;

for (dpp = entry->dbdrivers_of_ClipModule; dpp && *dpp; ++dpp)
      if (_clip_register_driver(ClipMemoryStore, *dpp))
	 ++ret;

   add_ClipVect(&ClipMemoryStore->dlls, hp);

   return ret;
}

CLIP_DLLEXPORT int
_clip_load_inits(ClipMachine * ClipMemoryStore, ClipFile * file)
{
  ...
  /*file->end = file->pos + 2 * num; */
   for (i = 0; i < initnum; ++i)
	...
	file->pos_of_ClipFile += 2;
	 _clip_main_code(ClipMemoryStore, c.union_of_ClipCodeVar.ClipBlock_block_of_ClipCodeVar, _clip_argc, _clip_argv, _clip_envp);
      }
   for (i = 0; i < exitnum; ++i)
      {
	 make_func(file, &c, &hash);
	file->pos_of_ClipFile += 2;
	 ClipMemoryStore->exits = (ClipBlock *) realloc(ClipMemoryStore->exits, (ClipMemoryStore->nexits + 1) * sizeof(ClipBlock));
	 ClipMemoryStore->exits[ClipMemoryStore->nexits] = *c.union_of_ClipCodeVar.ClipBlock_block_of_ClipCodeVar;
	 ++file->refCount_of_ClipFile;
	 ++ClipMemoryStore->nexits;
      }
   return 0;
}

CLIP_DLLEXPORT int
_clip_load(ClipMachine * ClipMemoryStore, const char *name, struct Coll *names, ClipVar * resp)
{
   ClipFile *file = 0;
   int r = 0;
   ClipBlock b;
   ...
   if (e && (!s || s < e))
	 ...
	 if (!strcmp(e, ".po") || !strcmp(e, ".pa"))
	    {
	       file = NEW(ClipFile);
	       r = load_pobj(ClipMemoryStore, file, name, 0);
	    }
	 else if (!strcasecmp(e, DLLREALSUFF))
	    {
	       r = load_dll(ClipMemoryStore, name, names, resp);
	       return r;
	    }
	 else
	    {
	       _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "unknown file type: '%s'", name);
	       return _clip_call_errblock(ClipMemoryStore, 1);
	    }
      }
   else
	 ...
	 if (!access(path, R_OK))
	    {
	       file = NEW(ClipFile);
	       r = load_pobj(ClipMemoryStore, file, path, 0);
	    }
	 else
	    {
	       ...
	       else
		  {
		     ...
		     if (!access(path, R_OK))
			{
			   r = load_dll(ClipMemoryStore, path, names, resp);
			   return r;
			}
		     else
			{
			   _clip_trap_printf(ClipMemoryStore, __FILE__, __LINE__, "cannot load file: '%s'", name);
			   return _clip_call_errblock(ClipMemoryStore, 1);
			}
		  }
	    }
	 ...
	 if (c.ClipType_type_of_ClipCodeVar.ClipVartype_type_of_ClipType == CCODE_type_of_ClipVarType)
	    {
		  ...
		  {
		     if (_clip_register_hash(ClipMemoryStore, c.union_of_ClipCodeVar.ClipFunction_function_of_ClipCodeVar, hash))
				  ++file->refCount_of_ClipFile;
		  }
	    }
	 else if (c.ClipType_type_of_ClipCodeVar.ClipVartype_type_of_ClipType == PCODE_type_of_ClipVarType)
	    {
		  ...
		  {
		     if (_clip_register_block(ClipMemoryStore, *c.union_of_ClipCodeVar.ClipBlock_block_of_ClipCodeVar, hash))
			{
			   char *name;
...
++file->refCount_of_ClipFile;
   ...
   delete_ClipFile(ClipMemoryStore, file);

   return 0;
}

void
delete_ClipFile(ClipMachine * ClipMemoryStore, ClipFile * fp)
{
   if (destroy_ClipFile(ClipMemoryStore, fp))
      free(fp);
}

int
destroy_ClipFile(ClipMachine * ClipMemoryStore, ClipFile * fp)
{
   int i;
	 ...
	 break;
	   ...
	   if (fp->fd_of_ClipFile > 2)
		...
		for (i = 0; i < fp->nstatics_of_ClipFile; ++i)
			_clip_destroy(ClipMemoryStore, fp->ClipVar_statics_of_ClipFile + i);
...
if (fp->hash_names_of_ClipFile)
      {
			...
			/*free(fp->hash_names_of_ClipFile->mem_of_ClipHashNames); */
...
void
_clip_init_Block(ClipMachine * ClipMemoryStore, ClipBlock * dest, struct ClipFile *file, char *func)
{
...
void
destroy_Block(ClipMachine * ClipMemoryStore, ClipBlock * bp)
{
   delete_ClipFile(ClipMemoryStore, bp->file_of_ClipBlock);
}

void
_clip_dup_Block(ClipMachine * ClipMemoryStore, ClipBlock * src, ClipBlock * dest)
{
   *dest = *src;
...
static void
get_static(ClipMachine * ClipMemoryStore, ClipFile * file, ClipVar * statics, char *modbeg, int no, ClipVar ** dest)
{
	...
	statics->ClipType_t_of_ClipVar.count_of_ClipType = 1;
	 ...
	 _clip_code_func(ClipMemoryStore, &init, 0, 0, 0);
      }
   *dest = statics + no + 1;
}
...
static int
do_code(ClipMachine * ClipMemoryStore, struct ClipBlock *cp, int argc, int isProc, int rest, ClipVarFrame * uplocals)
{
...
CLIP_DLLEXPORT int
_clip_code_func(ClipMachine * ClipMemoryStore, struct ClipBlock *cp, int argc, int rest, ClipVarFrame * uplocals)
{
   return do_code(ClipMemoryStore, cp, argc, 0, rest, uplocals);
}

CLIP_DLLEXPORT int
_clip_code_proc(struct ClipMachine *ClipMemoryStore, struct ClipBlock *cp, int argc, int rest, ClipVarFrame * uplocals)
{
   return do_code(ClipMemoryStore, cp, argc, 1, rest, uplocals);
}
...
static int
run_vm(ClipMachine * ClipMemoryStore, ClipBlock * bp)
{
  ...
  file->hash_names_of_ClipFile, procname, maxdeep, 0
   ...
   if (!nlocals && !nreflocals)
      {
			reflocals = frame.ClipVarFrame_localvars_of_ClipFrame = ClipMemoryStore->fp->ClipVarFrame_localvars_of_ClipFrame;
   ...
   while (pp >= beg && pp < end)
	 ...
	 switch ((code = *pp++))
	    ...
	    case CLIP_PUSH_STR:
	       ...
	       get_str(modbeg, s, &(frame.sp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf), &(frame.sp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len));
	      /*
	         if ( !(ClipMemoryStore->flags1 & NOEXPAND_MACRO_FLAG) )
	         {
	       */
	       if (strchr(frame.sp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf, '&'))
		  _clip_expand_var(ClipMemoryStore, frame.sp);
	      /*} */
	       ++frame.sp;
	       break;
#endif
	    case CLIP_PUSH_STRDUP:
	       ...
	       */
			if (strchr(frame.ClipVar_sp_of_ClipFrame->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, '&'))
				_clip_expand_var(ClipMemoryStore, frame.ClipVar_sp_of_ClipFrame);
	      /*} */
			++frame.ClipVar_sp_of_ClipFrame;
	       break;
	    case CLIP_PUSH_NIL:
	    ...
	    case CLIP_MEMVAR_PUBLIC:
	       l = GET_LONG(pp);
	       _clip_memvar_public(ClipMemoryStore, l);
	       break;
	    case CLIP_MEMVAR_SPACE:
	       l = GET_LONG(pp);
	       l1 = GET_LONG(pp);
	       _clip_memvar_space(ClipMemoryStore, _clip_space(ClipMemoryStore, l), l1, 1);
	       break;
	    case CLIP_MEMVAR_PRIVATE:
	       l = GET_LONG(pp);
	       _clip_memvar_private(ClipMemoryStore, l);
	       break;
	    case CLIP_MEMVAR_PUBLIC_POP:
	       l = _clip_pop_hash(ClipMemoryStore);
	       _clip_memvar_public(ClipMemoryStore, l);
	       break;
	    case CLIP_MEMVAR_SPACE_POP:
	       l = GET_LONG(pp);
	       l1 = _clip_pop_hash(ClipMemoryStore);
	       _clip_memvar_space(ClipMemoryStore, _clip_space(ClipMemoryStore, l), l1, 1);
	       break;
	    case CLIP_MEMVAR_PRIVATE_POP:
	       l = _clip_pop_hash(ClipMemoryStore);
/*_clip_memvar_private(ClipMemoryStore, l);*/
	       _clip_add_private(ClipMemoryStore, l);
	       break;
	    case CLIP_MEMVAR_PARAM:
	       l = GET_LONG(pp);
	       c = GET_BYTE(pp);
	       _clip_memvar_param(ClipMemoryStore, l, c);
	       break;
	    case CLIP_PUSH_PARAM:
	       c = GET_BYTE(pp);
	       s = GET_SHORT(pp);
	       _clip_param(ClipMemoryStore, c, s);
	       break;
	    case CLIP_PUSH_LOCAL:
	       s = GET_SHORT(pp);
	       if ((ret = _clip_push_local(ClipMemoryStore, s)))
		  goto _trap;
	       break;
	    case CLIP_PUSH_REF_LOCAL:
	       s = GET_SHORT(pp);
#if 0
frame.sp->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar = _clip_ref_local(ClipMemoryStore, s);
#else
	       {
		  ClipVar *vp1 = _clip_ref_local(ClipMemoryStore, s);
	    ...
	    case CLIP_PUSH_STATIC:
	       s = GET_SHORT(pp);
	       get_static(ClipMemoryStore, file, statics, modbeg, s, &vp);
	       if ((ret = _clip_push_static(ClipMemoryStore, vp)))
		  goto _trap;
	       break;
	    ...
	    case CLIP_PUSH_MEMVAR:
	       l = GET_LONG(pp);
	       if ((ret = _clip_memvar(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_PUSH_FMEMVAR:
	       l = GET_LONG(pp);
	       if ((ret = _clip_fmemvar(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_PUSH_MEMVARF:
	       l = GET_LONG(pp);
	       if ((ret = _clip_memvarf(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_REF_FMEMVAR:
	       l = GET_LONG(pp);
	       if ((ret = _clip_ref_fmemvar(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_PUSH_REF_MEMVAR:
	       l = GET_LONG(pp);
#if 0
frame.sp->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar = _clip_ref_memvar(ClipMemoryStore, l);
#else
	       {
		  ClipVar *vp1 = _clip_ref_memvar(ClipMemoryStore, l);
		  frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar = vp1;
	       }
#endif
	       break;
	    case CLIP_PUSH_REF_MEMVAR_NOADD:
	       l = GET_LONG(pp);
	       {
		  ClipVar *vp1 = _clip_ref_memvar_noadd(ClipMemoryStore, l);
		  frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar = vp1;
	       }
	       break;
	    case CLIP_PUSH_PUBLIC:
	       l = GET_LONG(pp);
	       if ((ret = _clip_public(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_PUSH_REF_PUBLIC:
	       l = GET_LONG(pp);
#if 0
frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar = _clip_ref_public(ClipMemoryStore, l);
#else
	       {
		  ClipVar *vp1 = _clip_ref_public(ClipMemoryStore, l);
		  frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar = vp1;
	       }
#endif
	       break;
	    case CLIP_REFMACRO:
	       if ((ret = _clip_refmacro(ClipMemoryStore)))
		  goto _trap;
	       break;
	    case CLIP_MAKE_REF:
	       c = GET_BYTE(pp);
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_ref(ClipMemoryStore, vp, c)))
		  goto _trap;
	       break;
	    case CLIP_UNREF_ARR:
	       _clip_unref_arr(ClipMemoryStore);
	       break;
	    case CLIP_FIELD:
	       l = GET_LONG(pp);
	       l1 = GET_LONG(pp);
	       if ((ret = _clip_field(ClipMemoryStore, l, l1)))
		  goto _trap;
	       break;
	    case CLIP_FIELD_POP:
	       l = GET_LONG(pp);
	    ...
	    case CLIP_FIELD_POP_NAME:
	       _clip_pop_fieldhash(ClipMemoryStore, &l1, &l);
	       if ((ret = _clip_field(ClipMemoryStore, l, l1)))
		  goto _trap;
	       break;
	    case CLIP_PUSH_AREA:
	       l = GET_LONG(pp);
	       if ((ret = _clip_push_area(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_PUSH_AREA_POP:
		  ...
		  goto _trap;
	       break;
	    case CLIP_POP_AREA:
	       if ((ret = _clip_pop_area(ClipMemoryStore)))
		  goto _trap;
	       break;
	    case CLIP_FUNC:
	       c = GET_BYTE(pp);
	       l = GET_LONG(pp);
	       if ((ret = _clip_func_hash(ClipMemoryStore, l, c, 0, reflocals)))
		  goto _trap;
	       break;
	    case CLIP_FUNCR:
	       c = GET_BYTE(pp);
	       l = GET_LONG(pp);
	       if ((ret = _clip_func_hash(ClipMemoryStore, l, c, 1, reflocals)))
		  goto _trap;
	       break;
	    case CLIP_PROC:
	       c = GET_BYTE(pp);
	       l = GET_LONG(pp);
	       if ((ret = _clip_proc_hash(ClipMemoryStore, l, c, 0, reflocals)))
		  goto _trap;
	       break;
	    case CLIP_PROCR:
	       c = GET_BYTE(pp);
	       l = GET_LONG(pp);
	       if ((ret = _clip_proc_hash(ClipMemoryStore, l, c, 1, reflocals)))
		  goto _trap;
	       break;
	    case CLIP_SFUNC:
	       c = GET_BYTE(pp);
	       ...
	       if ((ret = _clip_code_func(ClipMemoryStore, &bl, c, 0, reflocals)))
		  goto _trap;
	       break;
	    case CLIP_SFUNCR:
	       c = GET_BYTE(pp);
	       ...
	       bl.file_of_ClipBlock = file;
	       get_func(modbeg, s, &bl.function_of_ClipBlock);
	       if ((ret = _clip_code_func(ClipMemoryStore, &bl, c, 1, reflocals)))
		  goto _trap;
	       break;
	    case CLIP_SPROC:
	       c = GET_BYTE(pp);
	       ...
	       bl.file_of_ClipBlock = file;
	       get_func(modbeg, s, &bl.function_of_ClipBlock);
	       if ((ret = _clip_code_proc(ClipMemoryStore, &bl, c, 0, reflocals)))
		  goto _trap;
	       break;
	    case CLIP_SPROCR:
	       c = GET_BYTE(pp);
	       ...
	       bl.file_of_ClipBlock = file;
	       get_func(modbeg, s, &bl.function_of_ClipBlock);
	       if ((ret = _clip_code_proc(ClipMemoryStore, &bl, c, 1, reflocals)))
		  goto _trap;
	       break;
	    case CLIP_ASSIGN:
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_assign(ClipMemoryStore, vp)))
		  goto _trap;
	       break;
	    ...
	    case CLIP_MACROASSIGN:
	       c = GET_BYTE(pp);
	       c1 = GET_BYTE(pp);
	       if ((ret = _clip_macroassign(ClipMemoryStore, c, c1 & 1, c1 & 2)))
		  goto _trap;
	       break;
	    case CLIP_REFASSIGN:
	       c = GET_BYTE(pp);
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_refassign(ClipMemoryStore, vp, c)))
		  goto _trap;
	       break;
	    case CLIP_UNREF:
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	    ...
	    case CLIP_IASSIGN:
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_iassign(ClipMemoryStore, vp)))
		  goto _trap;
	       break;
	    case CLIP_OPASSIGN:
	       c = GET_BYTE(pp);
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_opassign(ClipMemoryStore, vp, c)))
		  goto _trap;
	       break;
	    case CLIP_OPIASSIGN:
	       c = GET_BYTE(pp);
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_opiassign(ClipMemoryStore, vp, c)))
		  goto _trap;
	       break;
	    case CLIP_ASSIGN_FIELD:
	       l = GET_LONG(pp);
	       l1 = GET_LONG(pp);
	       if ((ret = _clip_assign_field(ClipMemoryStore, l, l1)))
		  goto _trap;
	       break;
	    case CLIP_IASSIGN_FIELD:
	       l = GET_LONG(pp);
	       l1 = GET_LONG(pp);
	       if ((ret = _clip_iassign_field(ClipMemoryStore, l, l1)))
		  goto _trap;
	       break;
	    ...
	    case CLIP_FM_ASSIGN:
	       l = GET_LONG(pp);
	       if ((ret = _clip_fm_assign(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_FM_IASSIGN:
	       l = GET_LONG(pp);
	       if ((ret = _clip_fm_iassign(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_ARRAY:
	       l = GET_LONG(pp);
	       _clip_sarray(ClipMemoryStore, l);
	       break;
	    case CLIP_DIMARRAY:
	       l = GET_LONG(pp);
	       _clip_dimarray(ClipMemoryStore, l);
	       break;
	    case CLIP_NEWARRAY:
	       l = GET_LONG(pp);
	       _clip_vnewarray(ClipMemoryStore, l, (long *) pp);
	       pp += l * sizeof(long);

	       break;
	    case CLIP_GET:
	       l = GET_LONG(pp);
	       if ((ret = _clip_get(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_SET:
	       l = GET_LONG(pp);
	       if ((ret = _clip_set(ClipMemoryStore, l)))
		  goto _trap;
	       break;
	    case CLIP_RETURN_POP:
	       _clip_return(ClipMemoryStore);
	    case CLIP_RETURN:
	       goto _return;
	    case CLIP_STORE:
	       c1 = GET_BYTE(pp);
	       c = GET_BYTE(pp);
	       if ((ret = _clip_store(ClipMemoryStore, c, c1)))
		  goto _trap;
	       break;
	    case CLIP_ISTORE:
	       c1 = GET_BYTE(pp);
	       c = GET_BYTE(pp);
	       if ((ret = _clip_istore(ClipMemoryStore, c, c1)))
		  goto _trap;
	       break;
	    case CLIP_OPSTORE:
	       c = GET_BYTE(pp);
	       c1 = GET_BYTE(pp);
	       if ((ret = _clip_opstore(ClipMemoryStore, c1, c)))
		  goto _trap;
	       break;
	    case CLIP_OPISTORE:
	       c = GET_BYTE(pp);
	       c1 = GET_BYTE(pp);
	       if ((ret = _clip_opistore(ClipMemoryStore, c1, c)))
		  goto _trap;
	       break;
	    case CLIP_FETCH:
	       c = GET_BYTE(pp);
	       if ((ret = _clip_fetch(ClipMemoryStore, c)))
		  goto _trap;
	       break;
	    case CLIP_FETCHREF:
	       c = GET_BYTE(pp);
#if 0
frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar = _clip_fetchref(ClipMemoryStore, c);
#else
	       {
		  ClipVar *vp1 = _clip_fetchref(ClipMemoryStore, c);
		 /*printf("%p\n", vp1); */
		 frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar = vp1;
	       }
#endif
if (!frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar)
	    ...
	    case CLIP_IFETCH:
	       c = GET_BYTE(pp);
	       if ((ret = _clip_ifetch(ClipMemoryStore, c)))
		  goto _trap;
	       break;
	    case CLIP_CALL:
	       c = GET_BYTE(pp);
	       l = GET_LONG(pp);
	       if ((ret = _clip_call(ClipMemoryStore, c, l)))
		  goto _trap;
	       break;
	    case CLIP_PUSH_CODE:
	       s = GET_SHORT(pp);
		  ...
		  file->refCount_of_ClipFile++;
	       ...
	       break;
	    case CLIP_MACRO:
	       if ((ret = _clip_macro(ClipMemoryStore)))
		  goto _trap;
	       break;
	    case CLIP_PCOUNT:
	       ...
	       break;
	    case CLIP_PSHIFT:
	       _clip_pshift(ClipMemoryStore);
	       break;
	    case CLIP_PARN:
	       if ((ret = _clip_parn(ClipMemoryStore)))
		  goto _trap;
	       break;
	    case CLIP_FUNC_NAME:
	       c = GET_BYTE(pp);
	       if ((ret = _clip_func_name(ClipMemoryStore, c)))
		  goto _trap;
	       break;
	    case CLIP_INCR:
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_incr(ClipMemoryStore, vp, 1, 0)))
		  goto _trap;
	       break;
	    case CLIP_INCR_POST:
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_incr(ClipMemoryStore, vp, 1, 1)))
		  goto _trap;
	       break;
	    case CLIP_DECR:
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_incr(ClipMemoryStore, vp, 0, 0)))
		  goto _trap;
	       break;
	    case CLIP_DECR_POST:
			 vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
	       if ((ret = _clip_incr(ClipMemoryStore, vp, 0, 1)))
		  goto _trap;
	       break;
	    case CLIP_OP:
	       c = GET_BYTE(pp);
	       if ((ret = _clip_op(ClipMemoryStore, c)))
		  goto _trap;
	       break;
	    case CLIP_NOT:
	       if ((ret = _clip_not(ClipMemoryStore)))
		  goto _trap;
	       break;
	    case CLIP_COND:
	       s = GET_SHORT(pp);
	       if ((ret = _clip_cond(ClipMemoryStore, &i)))
		  goto _trap;
	       if (!i)
		  pp += s;
	       break;
	    case CLIP_TCOND:
	       s = GET_SHORT(pp);
	       if ((ret = _clip_tcond(ClipMemoryStore, &i)))
		  goto _trap;
	       if (!i)
		  pp += s;
	       break;
	    ...
	    case CLIP_FORSTEP:
	       s = GET_SHORT(pp);
	       if ((ret = _clip_forstep(ClipMemoryStore, &i)))
		  goto _trap;
	       if (!i)
		  pp += s;
	       break;
	    case CLIP_MAP_FIRST:
	       c = GET_BYTE(pp);
	       s = GET_SHORT(pp);
	       if ((ret = _clip_map_first(ClipMemoryStore, c, &i)))
		  goto _trap;
	       if (!i)
		  pp += s;
	       break;
	    case CLIP_MAP_NEXT:
	       c = GET_BYTE(pp);
	       s = GET_SHORT(pp);
	       if ((ret = _clip_map_next(ClipMemoryStore, c, &i)))
		  goto _trap;
	       if (!i)
		  pp += s;
	       break;
	    case CLIP_MINUS:
	       if ((ret = _clip_minus(ClipMemoryStore)))
		  goto _trap;
	       break;
	    case CLIP_RESETTRAP:
	       trap_pp = 0;
	    ...
	    case CLIP_RECOVER:
			 while (frame.ClipVar_sp_of_ClipFrame > frame.ClipVar_stack_of_ClipFrame)
				 _clip_destroy(ClipMemoryStore, --frame.ClipVar_sp_of_ClipFrame);
	       ret = 0;
	       break;
	    case CLIP_USING:
	       if (ClipMemoryStore->trapVar)
		  {
			  vp = frame.ClipVar_sp_of_ClipFrame->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar;
			  *frame.ClipVar_sp_of_ClipFrame = *ClipMemoryStore->trapVar;
			  ++frame.ClipVar_sp_of_ClipFrame;
		     free(ClipMemoryStore->trapVar);
		     ClipMemoryStore->trapVar = 0;
		     if ((ret = _clip_assign(ClipMemoryStore, vp)))
			goto _trap;
		  }
	       break;
	    case CLIP_BREAK:
#if 0
			/*_clip_trap_str(ClipMemoryStore, filename, frame.line, "BREAK");*/
#else
	       vp = NEW(ClipVar);
			 _clip_trap_var(ClipMemoryStore, filename, frame.line_of_ClipFrame, vp);
#endif
	       ret = -1;
	       goto _trap;
	    ...
	    case CLIP_PUSH_HASH:
	       ...
	       break;
	    case CLIP_CALC_HASH:
	       _clip_calc_hash(ClipMemoryStore);
	       break;
	    case CLIP_CALC_HASH2:
	       _clip_calc_hash2(ClipMemoryStore, 1);
	       break;
	    case CLIP_PUSH_LOCALE:
	       _clip_push_locale(ClipMemoryStore);
	       break;
	    case CLIP_RDDFIELD:
	       s = GET_SHORT(pp);
	       s1 = GET_SHORT(pp);
	       _clip_rddfield(ClipMemoryStore, s, s1);
	       break;
	    case CLIP_CATSTR:
	       s = GET_SHORT(pp);
	       _clip_catstr(ClipMemoryStore, s);
	       break;
	    case CLIP_QUOT:
	       _clip_quot(ClipMemoryStore);
	       break;
	    case CLIP_SWITCH:
	       s = GET_SHORT(pp);	/* label num */
	       l = _clip_pop_shash(ClipMemoryStore);	/* hash */
	       {
		  ...
		  if (n < 0)
	       ...
	       break;
	    default:
			 _clip_trap_printf(ClipMemoryStore, filename, frame.line_of_ClipFrame, "invalid bytecode %d", code);
	       _clip_call_errblock(ClipMemoryStore, 1);
	       ret = 1;
	       goto _trap;
	    }
      }
 ...
 _trap:
   if (trap_pp /*&& ret > 0 */ )
	 ...
	 goto cont;
	   ...
	   _clip_trap(ClipMemoryStore, filename, frame.line_of_ClipFrame);
  /*ret = 1; */
 _return:
   if (_clip_profiler)
      _clip_stop_profiler(ClipMemoryStore);
   if (local_locals)
      _clip_destroy_locals(ClipMemoryStore);
   _clip_resume(ClipMemoryStore, nlocals, nreflocals);
#if 0
   _clip_vresume(ClipMemoryStore, nlocals, locals);
#endif
#if 0
   _clip_vresume(ClipMemoryStore, nparams, params);
#endif
#if 0
   if (nreflocals)
      _clip_vresume(ClipMemoryStore, nreflocals, reflocals);
#endif
/*_clip_vremove_privates(ClipMemoryStore, nprivates, privates);*/
