...
_clip_debug(ClipMachine * ClipMachineMemory)
{
   ...
   /* process commands */
	while (dbg_commands->count_of_Coll)
   {
		char *cmd = (char *) dbg_commands->items_of_Coll[0];
   ...
   if (!in_sigdebug)
   {
	 ...
	 return 0;
      ...
      if (next_line && !_clip_debugnext && ClipMachineMemory->fp->line_of_ClipFrame && ClipMachineMemory->fp->line_of_ClipFrame != next_line)
      {
	 next_line = 0;
	 ...
	 return 0;
	  ...
	  && next_file == ClipMachineMemory->fp->filename_of_ClipFrame && next_proc && next_proc == ClipMachineMemory->fp->procname_of_ClipFrame)
      {
	 _clip_debugnext = 0;
	 next_line = 0;
	 next_file = 0;
	 ...
	 return 0;
	  ...
	  && (!until_file || (ClipMachineMemory->fp->filename_of_ClipFrame && !strcasecmp(until_file, ClipMachineMemory->fp->filename_of_ClipFrame))))
      {
	 until_line = 0;
	 if (until_file)
	 {
	 ...
	 bp->line_of_ClipBreakPoint = ClipMachineMemory->fp->line_of_ClipFrame;
      ...
      {
			nbp = (ClipBreakPoint *) sbreakpoints.items_of_Coll[ind];
	 if (nbp == prev_bp)
	    return 0;
	 if (!bp_in_codeblock && ClipMachineMemory->fp->procname_of_ClipFrame && !memcmp(ClipMachineMemory->fp->procname_of_ClipFrame, "___code_", 8))
	    return 0;
	 ...
	 if (watchs.count_of_Coll)
	 {
		 ...
		 for (i = 0; i < watchs.count_of_Coll; i++)
	    {
	       ClipVar res;
			 ...
			 char *expr = (char *) watchs.items_of_Coll[i];
		  ...
		  fprintf(_clip_dbg_out,
			  ...
			  expr, nullstr(ClipMachineMemory->fp->filename_of_ClipFrame), ClipMachineMemory->fp->line_of_ClipFrame, nullstr(ClipMachineMemory->fp->procname_of_ClipFrame));
		  fprintf(_clip_dbg_out, "\n.\n");
		  fflush(_clip_dbg_out);
		  _clip_logg(1,
