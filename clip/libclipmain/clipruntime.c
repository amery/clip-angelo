...
/* [ ClipBuf */
#include "clipruntime/destroy_ClipBuf.h"
#include "clipruntime/setLen_ClipBuf.h"
/*
...
typedef struct
{
   long      hash_of_FuncEntry;	/* hash */
   ClipFunction *ClipFunction_f_of_FuncEntry;	/* f */
   ClipBlock ClipBlock_block_of_FuncEntry;	/* block */
}
FuncEntry;
...
_clip_init_tty(ClipMachine * ret)
{
   ...
   if (ret->screen)
    {
       ret->wboard.bottom_of_ClipRect = ret->windows->ClipRect_format_of_ClipWindow.bottom_of_ClipRect =
	ret->screen->base->Lines - 1;
       ret->wboard.right_of_ClipRect = ret->windows->ClipRect_format_of_ClipWindow.right_of_ClipRect =
	ret->screen->base->Columns - 1;
...
*/
   ...
   *(ret->kbdptr) = ret->kbdbuf;
   ...
   if (ret->screen)
    {
       ret->wboard.bottom_of_ClipRect = ret->windows->ClipRect_rect_of_ClipWindow.bottom_of_ClipRect =
	ret->screen->base->Lines - 1;
       ret->wboard.right_of_ClipRect = ret->windows->ClipRect_rect_of_ClipWindow.right_of_ClipRect =
	ret->screen->base->Columns - 1;
       ret->windows->ClipRect_format_of_ClipWindow = ret->windows->ClipRect_format_of_ClipWindow;
    }
   ret->wshadow = -1;
...
_clip_logg(int level, const char *fmt, ...)
{
   ...
   fprintf(logg, "%02d:%02d:%02d %s:%lu:%d ", tp->tm_hour, tp->tm_min,
	   tp->tm_sec, _clip_progname, (unsigned long) getpid(), level);
   va_start(ap, fmt);
...
delete_ClipMachine(ClipMachine * ClipMachineMemory)
{
   ...
   if (ClipMachineMemory == first_mp)
   ...
   else
    {
       ...
       for (ClipMachineMemoryp = first_mp; ClipMachineMemoryp && ClipMachineMemoryp->next;
	    ClipMachineMemoryp = ClipMachineMemoryp->next)
	{
...
_clip_log_call(ClipMachine * ClipMachineMemory)
{
   ...
   if (log_level >= 5)
    {
       _clip_logg(5, "C call: proc '%s' file '%s' line %d",
...
_clip_errorblock_res(ClipMachine * ClipMachineMemory, ClipVar * vp, int genCode, ClipVar * res)
{
  ...
  /* default err handler */
   if (!ClipMachineMemory->errorblock
       || (_clip_type(ClipMachineMemory->errorblock) != CCODE_type_of_ClipVarType
	   && _clip_type(ClipMachineMemory->errorblock) != PCODE_type_of_ClipVarType))
    {
       char     *msg = 0;
...
do_call(ClipMachine * ClipMachineMemory, ClipFunction f, int argc, int isProc, int rest, ClipVarFrame * uplocals)
{
   ...
   if (log_level >= 6)
    {
       _clip_logg(6, "C call from: proc '%s' file '%s' line %d",
		  fp->procname_of_ClipFrame ? fp->procname_of_ClipFrame : "unknown", fp->filename_of_ClipFrame,
		  fp->line_of_ClipFrame);
    }
...
{
   long      h1 = ((ProfileBucket *) p1)->hash_of_VarEntry;

   long      h2 = ((ProfileBucket *) p2)->hash_of_VarEntry;
   ...
   for (i = 0; i < coll.count; i++)
    {
	  ...
	  continue;
       ...
       fprintf(out, "%18s %-13s %8ld calls %8lu.%03lu ms\n", bp->procname_of_ProfileBucket,
	       bp->filename_of_ProfileBucket, bp->callno_of_ProfileBucket, ms, us);
    }
...
_clip_eval(ClipMachine * ClipMachineMemory, ClipVar * blockp, int argc, ClipVar * stackp, ClipVar * retp)
{
   ...
   if (type == CCODE_type_of_ClipVarType)
      r =
       _clip_func(ClipMachineMemory, blockp->ClipCodeVar_c_of_ClipVar.u.ClipFunction_function_of_ClipCodeVar, argc, 0,
		  blockp->ClipCodeVar_c_of_ClipVar.ClipVarFrame_uplocals_of_ClipCodeVar);
   else
      r =
       _clip_code_func(ClipMachineMemory, blockp->ClipCodeVar_c_of_ClipVar.u.ClipBlock_block_of_ClipCodeVar, argc, 0,
		       blockp->ClipCodeVar_c_of_ClipVar.ClipVarFrame_uplocals_of_ClipCodeVar);
...
_clip_push_func(struct ClipMachine *ClipMachineMemory, ClipFunction f, int nlocals, int nolocals)
{
   ...
   vp->ClipCodeVar_c_of_ClipVar.u.ClipFunction_function_of_ClipCodeVar = f;
   ...
   else if (!nolocals && fp->ClipVarFrame_localvars_of_ClipFrame
	    && fp->ClipVarFrame_localvars_of_ClipFrame->refcount_of_ClipVarFrame > 0)
    {
...
_clip_func_name(struct ClipMachine *ClipMachineMemory, int argc)
{
   ...
   if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType)
      hash =
       _clip_casehashbytes(0, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
    ...
    {
       if (vp->ClipType_t_of_ClipVar.memo_of_ClipType)
	  hash = (long) rational_toDouble(vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
       else
	  hash = (long) vp->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar;
    }
   else
    {
       _clip_trap_str(ClipMachineMemory, __file__, __LINE__,
		      "call by name not from CHARACTER_type_of_ClipVarType or NUMERIC_type_of_ClipVarType value");
       return _clip_call_errblock(ClipMachineMemory, 1);
    }
   ...
   if (!_clip_get_function(ClipMachineMemory, hash, &fp, &bp))
	       ...
	       || vep->ClipVar_var_of_VarEntry.ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CCODE_type_of_ClipVarType))
	{
	  /* if function not found - call codeblock from memvar */
	   ...
	   ClipMachineMemory->bp = fp->ClipVar_sp_of_ClipFrame;
	   r =
	    _clip_eval(ClipMachineMemory, &(vep->ClipVar_var_of_VarEntry), ClipMachineMemory->argc,
		       ARGPTR(ClipMachineMemory, 1), &res);
   ...
   if (ClipMachineMemory->fp->ClipVarFrame_localvars_of_ClipFrame)
       ...
       if (ClipMachineMemory->fp->ClipVarFrame_localvars_of_ClipFrame->refcount_of_ClipVarFrame == 0)
	{
	   ...
	   for (i = 0; i < ClipMachineMemory->fp->ClipVarFrame_localvars_of_ClipFrame->size_of_ClipVarFrame; i++)
	      _clip_destroy(ClipMachineMemory,
			    ClipMachineMemory->fp->ClipVarFrame_localvars_of_ClipFrame->ClipVar_vars_of_ClipVarFrame + i);
	}
    }
}
...
#if 0
static void
resume_mp(ClipMachine * ClipMachineMemory)
{
   ClipFrame *fp = ClipMachineMemory->fp;
   ...
   for (; fp->sp > fp->stack;)
    {
       --(fp->sp);
       _clip_destroy(ClipMachineMemory, fp->sp);
    }
}
#endif
...
_clip_add_tempvar(ClipMachine * ClipMachineMemory, ClipVar * vp)
{
   ...
   vvp->ClipVar_items_of_ClipVarVect =
    (ClipVar *) realloc(vvp->ClipVar_items_of_ClipVarVect, (vvp->count_of_ClipVarVect + 1) * sizeof(ClipVar));
...
*/
	   ...
	   if ((vp->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType & F_MPTR_ClipFlags)
	       && vp->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar->ClipType_t_of_ClipVar.count_of_ClipType == 1
	       && (calc_loopcount(ClipMachineMemory, vp, vfp, 0) == vfp->refcount_of_ClipVarFrame - 1))
	      _clip_destroy(ClipMachineMemory, vp);
#else
	   if ((vp->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType & F_MPTR_ClipFlags)
...
_clip_destroy(ClipMachine * ClipMachineMemory, ClipVar * vp)
{
   ...
   switch (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
    ...
    case NUMERIC_type_of_ClipVarType:
       if (vp->ClipType_t_of_ClipVar.memo_of_ClipType)
	{
	   rational_destroy(vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
	}
       break;
    case ARRAY_type_of_ClipVarType:
       {
    ...
    case MAP_type_of_ClipVarType:
	 ...
	 /* try run `destroy` member */
	  if (search_map
	      (vp->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar, vp->ClipMapVar_m_of_ClipVar.count_of_ClipMapVar,
	       HASH_destroy, &ind))
	   {
	      ClipVar  *ep;
	  ...
	  for (i = vp->ClipMapVar_m_of_ClipVar.count_of_ClipMapVar - 1; i >= 0; --i)
	     _clip_destroy(ClipMachineMemory,
			   &(vp->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar[i].ClipVar_var_of_ClipVarEl));
    ...
    case PCODE_type_of_ClipVarType:
       ...
       delete_ClipVarFrame(ClipMachineMemory, vp->ClipCodeVar_c_of_ClipVar.ClipVarFrame_uplocals_of_ClipCodeVar);
       destroy_Block(ClipMachineMemory, vp->ClipCodeVar_c_of_ClipVar.u.ClipBlock_block_of_ClipCodeVar);
       free(vp->ClipCodeVar_c_of_ClipVar.u.ClipBlock_block_of_ClipCodeVar);
   ...
   if (bytype && ltype != rtype)
	...
	{			/* SET EXACT OFF - empty rstr, len must match for equal */
	   if (rrlen == 0)
	   ...
	   else			/* if ( rrlen > lllen ) */
	    {
		  ...
		  lllen = rrlen;
	       l =
		_clip_strnncmp(Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			       Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, lllen, rrlen);
	    }
#else
	   else if (llen < rrlen)
	    {
	       l =
		_clip_strncmp(Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			      Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, llen);
	       if (!l)
		  l = -1;
	    }
	   else
	      l =
	       _clip_strncmp(Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			     Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, rrlen);
#endif
	}
       else
	 /* SET EXACT ON - all but trailing spaces */
	{
	   if (llen == 0 && rlen > 0 /* llen > rlen */ )
	    ...
	    {
	       for (s = Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf + rlen - 1; rlen && *s == ' ';
		    --rlen, --s)
		  ;
	       for (s = Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf + llen - 1; llen && *s == ' ';
		    --llen, --s)
		  ;
	       if (rlen == 0 && llen > 0)
		  l = 1;
		  ...
		  l =
		   _clip_strnncmp(Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
				  Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, llen, rlen);
	    }
	}
    }
    ...
    {
       l =
	(Lval->ClipDateVar_d_of_ClipVar.julian_of_ClipDateVar <
	 Rval->ClipDateVar_d_of_ClipVar.julian_of_ClipDateVar ? -1 : (Lval->ClipDateVar_d_of_ClipVar.julian_of_ClipDateVar >
								      Rval->ClipDateVar_d_of_ClipVar.
								      julian_of_ClipDateVar ? 1 : 0));
    }
	...
	(Lval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar <
	 Rval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar ? -1 : (Lval->ClipDateTimeVar_dt_of_ClipVar.
									       julian_of_ClipDateTimeVar >
									       Rval->ClipDateTimeVar_dt_of_ClipVar.
									       julian_of_ClipDateTimeVar ? 1 : 0));
       if (l == 0)
	  l =
	   (Lval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar <
	    Rval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar ? -1 : (Lval->ClipDateTimeVar_dt_of_ClipVar.
										time_of_ClipDateTimeVar >
										Rval->ClipDateTimeVar_dt_of_ClipVar.
										time_of_ClipDateTimeVar ? 1 : 0));
    }
       ...
       else
	{
	   if (Lval->ClipType_t_of_ClipVar.memo_of_ClipType && Rval->ClipType_t_of_ClipVar.memo_of_ClipType)
	      l =
	       rational_cmp(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar,
			    Rval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
	   else
	    {
	       rational *tmp;
	       ...
	       if (Lval->ClipType_t_of_ClipVar.memo_of_ClipType)
		{
		   ...
		   l = rational_cmp(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar, tmp);
		}
	       else
		{
		   tmp = rational_double_init(Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar);
		   l = rational_cmp(tmp, Rval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
		}
    ...
    {
       l =
	(Lval->ClipCodeVar_c_of_ClipVar.u.ClipFunction_function_of_ClipCodeVar !=
	 Rval->ClipCodeVar_c_of_ClipVar.u.ClipFunction_function_of_ClipCodeVar);
    }
   else if (ltype == PCODE_type_of_ClipVarType && rtype == PCODE_type_of_ClipVarType)
    {
       l =
	(Lval->ClipCodeVar_c_of_ClipVar.u.ClipBlock_block_of_ClipCodeVar !=
	 Rval->ClipCodeVar_c_of_ClipVar.u.ClipBlock_block_of_ClipCodeVar);
    }
   else
    if ((ltype == PCODE_type_of_ClipVarType && rtype == CCODE_type_of_ClipVarType)
	|| (ltype == CCODE_type_of_ClipVarType && rtype == PCODE_type_of_ClipVarType))
    {
       l = 0;
    }
   ...
   switch (op)
	   ...
	   if (Lval->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType == F_MSTAT_ClipFlags)
	    {
	       ...
	       memcpy(s, Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		      Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
	      ...
	      Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf =
	       (char *) realloc(Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, len + 1);
       ...
       else if (ltype == DATE_type_of_ClipVarType && rtype == NUMERIC_type_of_ClipVarType)
	{
	   if (Rval->ClipType_t_of_ClipVar.memo_of_ClipType)
	      Lval->ClipDateVar_d_of_ClipVar.julian_of_ClipDateVar +=
	       rational_toDouble(Rval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
	   else
       ...
       else if (ltype == NUMERIC_type_of_ClipVarType && rtype == DATE_type_of_ClipVarType)
	{
	   ...
	   if (Lval->ClipType_t_of_ClipVar.memo_of_ClipType)
	      tmp = rational_toDouble(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
	   else
       ...
       else if (ltype == DATETIME_type_of_ClipVarType && rtype == NUMERIC_type_of_ClipVarType)
	{
	   ...
	   if (Rval->ClipType_t_of_ClipVar.memo_of_ClipType)
	      tmp = (long) rational_toDouble(Rval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
	   else
       ...
       else if (ltype == NUMERIC_type_of_ClipVarType && rtype == DATETIME_type_of_ClipVarType)
	{
	   ...
	   if (Lval->ClipType_t_of_ClipVar.memo_of_ClipType)
	      tmp = (long) rational_toDouble(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
	   else
	   ...
	   Lval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar =
	    Rval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar + tmp / (24 * 60 * 60);
	   Lval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar =
	    Rval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar + (tmp % (24 * 60 * 60)) * 1000;
	}
       else
	{
	   return _clip_trap_operation(ClipMachineMemory, __file__, __LINE__, "+", Lval);
       ...
       if (ltype == CHARACTER_type_of_ClipVarType && rtype == CHARACTER_type_of_ClipVarType)
	{
	   ...
	   for (s = Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf + llen - 1; llen;
		--llen, --s, ++slen)
	      if (*s != ' ' && *s != '\t')
		 break;
	   ...
	   if (Lval->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType == F_MSTAT_ClipFlags)
	    {
	      ...
	      Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf =
	       (char *) realloc(Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, len + slen + 1);
       ...
       else if (ltype == DATETIME_type_of_ClipVarType && rtype == DATETIME_type_of_ClipVarType)
	{
	   long      tmp =
	    (Lval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar -
	     Rval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar) * 24 * 60 * 60;

	   tmp +=
	    (Lval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar -
	     Rval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar) / 1000;
	   ...
	   if (Lval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar < 0)
	    {
	       Lval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar--;
	       Lval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar =
		(24 * 60 * 60 * 1000) + Lval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar;
	    }
	}
       else if (ltype == NUMERIC_type_of_ClipVarType && rtype == DATE_type_of_ClipVarType)
	{
	...
	{
	   Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar =
	    Lval->ClipDateVar_d_of_ClipVar.julian_of_ClipDateVar - Rval->ClipDateVar_d_of_ClipVar.julian_of_ClipDateVar;
	   Lval->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType = NUMERIC_type_of_ClipVarType;
       ...
       if (ltype == NUMERIC_type_of_ClipVarType && rtype == NUMERIC_type_of_ClipVarType)
	{
	   if (!Lval->ClipType_t_of_ClipVar.memo_of_ClipType && !Rval->ClipType_t_of_ClipVar.memo_of_ClipType)
	    {
	       Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar =
		fmod(Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar, Rval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar);
	       Lval->ClipType_t_of_ClipVar.len_of_ClipType = 0;
	       Lval->ClipType_t_of_ClipVar.dec_of_ClipType = 0;
	       Lval->ClipType_t_of_ClipVar.memo_of_ClipType = 0;
	    }
		...
		{
		   tmp =
		    rational_fmod(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar,
				  Rval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
		}
	       else
		{
		   if (Lval->ClipType_t_of_ClipVar.memo_of_ClipType)
		    {
		       ...
		       tmp = rational_fmod(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar, tmp1);
		   ...
		   else
		    {
		       ...
		       tmp = rational_fmod(tmp1, Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
		       rational_destroy(tmp1);
		    }
		}
	       rational_destroy(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
	       Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar = tmp;
	    }
	   ...
	   if (Lval->ClipType_t_of_ClipVar.memo_of_ClipType)
	    {
		  ...
		  tmp = (long) Rval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar;
	       rational_powa(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar, tmp);
	    }
	   else
	    {
	       Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar =
		pow(Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar, Rval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar);
	...
	{
	   const char *s =
	    _clip_strstr(Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			 Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf,
			 Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			 Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
       ...
       else if (rtype == MAP_type_of_ClipVarType)
	{
	   ...
	   switch (ltype)
	    ...
	    case NUMERIC_type_of_ClipVarType:
	       if (Lval->ClipType_t_of_ClipVar.memo_of_ClipType)
		  no = rational_toDouble(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
	       else
	    ...
	    case CHARACTER_type_of_ClipVarType:
	      /*no = _clip_casehashbytes(0, Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf); */
	       no =
		_clip_hashbytes(0, Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
				Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
	       break;
	    default:
	       goto inv_in_arg;
	   ...
	   r =
	    search_map(Rval->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar,
		       Rval->ClipMapVar_m_of_ClipVar.count_of_ClipMapVar, no, &ind);
	   _clip_destroy(ClipMachineMemory, lval);
	 ...
	 /*l = !strcmp(Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf, Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf); */
	{
	   ...
	   if (ll != lr)
	      ...
	      l =
	       !memcmp(Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		       Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, ll);
	}
#else
/*Rust begin */
	{
	   int       len =
	    ...
	    Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len ? Lval->ClipStrVar_s_of_ClipVar.
	    ClipBuf_str_of_ClipStrVar.len_of_ClipBuf : Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf;

	   l =
	    !_clip_strncmp(Lval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			   Rval->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, len);
	}
/*Rust end */
#endif
       else
	...
	{
	   if (Lval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar <
	       Rval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar)
	      l = -1;
	   else if (Lval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar >
		    Rval->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar)
	      l = 1;
	   else
	      l = 0;
	   if (l == 0)
	    {
	       if (Lval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar <
		   Rval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar)
		  l = -1;
	       else if (Lval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar >
			Rval->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar)
		  l = 1;
	       else
		  l = 0;
	   ...
	   if (!Lval->ClipType_t_of_ClipVar.memo_of_ClipType && !Rval->ClipType_t_of_ClipVar.memo_of_ClipType)
	    {
	       ...
	       diff =
		fabs(Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar - Rval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar);
	       if (diff < (Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar * MIN_NUMERIC_DIFF))
		  l = 0;
	       else if (Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar < Rval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar)
		  l = -1;
	...
	{
	   l =
	    Lval->ClipObjVar_o_of_ClipVar.rtti_of_ClipObjVar->cmp_of_ClipObjRtti(ClipMachineMemory,
										 Lval->ClipObjVar_o_of_ClipVar.
										 obj_of_ClipObjVar,
										 Lval->ClipObjVar_o_of_ClipVar.
										 rtti_of_ClipObjVar, Rval);
	   _clip_destroy(ClipMachineMemory, lval);
	}
       else
	{
...
_clip_minus(ClipMachine * ClipMachineMemory)
{
   ...
   if (vp->ClipType_t_of_ClipVar.memo_of_ClipType)
      rational_inverse(vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
   else
   ...
   if (lval)
    {
	   ...
	   && lval->ClipType_t_of_ClipVar.field_of_ClipType && lval->ClipRefVar_p_of_ClipVar.ClipFieldDef_of_ClipRefVar)
	{
       ...
       else
	{
	   if (!(lval->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType & F_MPTR_ClipFlags)
	       ...
	       && !(rval->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == ARRAY_type_of_ClipVarType
		    || rval->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == MAP_type_of_ClipVarType))
	    {
	       _clip_dup(ClipMachineMemory, lval, _clip_vptr(rval));
	    }
#if 0
	   else if (lval->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == ARRAY_type_of_ClipVarType
		    || lval->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == MAP_type_of_ClipVarType)
	    {
	       _clip_destroy(ClipMachineMemory, lval);
   ...
   if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != CHARACTER_type_of_ClipVarType)
    {
       _clip_trap_printf(ClipMachineMemory, __file__, __LINE__,
			 "macro assign with non-character argument (%s)", _clip_typename(vp));
       return _clip_call_errblock(ClipMachineMemory, 1);
    }

   if (op == '=')
      r =
       _clip_nameassign(ClipMachineMemory, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf, rval, fieldprec);
   else
    {
       lval =
	find_lval(ClipMachineMemory, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		  vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);

       if (!lval)
	{
	   _clip_trap_printf(ClipMachineMemory, __file__, __LINE__,
			     "no variable name: '%.*s'", vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf,
   ...
   switch (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
    ...
    case CHARACTER_type_of_ClipVarType:
	  ...
	  _clip_parse_name(ClipMachineMemory, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			   vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf, &aname, &al, &fname, &fl, 0,
			   0);
	  if (aname)
    ...
    case OBJECT_type_of_ClipVarType:
       *field =
	vp->ClipObjVar_o_of_ClipVar.rtti_of_ClipObjVar->hash_of_ClipObjRtti(ClipMachineMemory,
...
{
   if (dest->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType == F_MREF_ClipFlags
       && dest->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar->ClipType_t_of_ClipVar.count_of_ClipType > 1)
    {
       ClipVar  *dp, *srcp;
	...
	case F_MPTR_ClipFlags:
	   if (src->ClipType_t_of_ClipVar.field_of_ClipType && src->ClipRefVar_p_of_ClipVar.ClipFieldDef_of_ClipRefVar)
	    {
	       dest->ClipRefVar_p_of_ClipVar.ClipFieldDef_of_ClipRefVar =
		src->ClipRefVar_p_of_ClipVar.ClipFieldDef_of_ClipRefVar;
...
_clip_clone(struct ClipMachine *ClipMachineMemory, ClipVar * dest, ClipVar * src)
{
   ...
   switch (src->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType)
	   ...
	   if (src->ClipType_t_of_ClipVar.memo_of_ClipType)
	    {
	       ...
	       dest->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar =
		rational_copy(src->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
	    }
	   else
	      *dest = *src;
	   break;
	...
	case PCODE_type_of_ClipVarType:
	   *dest = *src;
	   if (src->ClipCodeVar_c_of_ClipVar.ClipVarFrame_uplocals_of_ClipCodeVar
	...
	case CHARACTER_type_of_ClipVarType:
	   {
	      ...
	      dest->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf =
	       (char *) _clip_memdup(src->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, len);
   ...
   switch (src->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
    ...
    case PCODE_type_of_ClipVarType:
	  ...
	  if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == PCODE_type_of_ClipVarType
	      && vp->ClipCodeVar_c_of_ClipVar.u.block->file)
	     vp->ClipCodeVar_c_of_ClipVar.u.block->file->refCount++;
#if 0
	  if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == PCODE_type_of_ClipVarType)
	   {
	      ClipBlock *bp;
	      ...
	      bp = NEW(ClipBlock);
	      *bp = *(src->ClipCodeVar_c_of_ClipVar.u.block);
	      vp->ClipCodeVar_c_of_ClipVar.u.block = bp;
	     /*bp->file->refCount++; */
	   }
	  else
	     vp->ClipCodeVar_c_of_ClipVar.u.func = src->ClipCodeVar_c_of_ClipVar.u.func;
#endif
#endif
       }
#if 0
...
_clip_push(ClipMachine * ClipMachineMemory, void *vpp)
{
   ...
   */
   if (!ret)
    {
       if (sp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType
	   && sp->ClipType_t_of_ClipVar.len_of_ClipType)
...
CLIP_DLLEXPORT void
_clip_push_false(ClipMachine * ClipMachineMemory)
{
   ClipFrame *fp = ClipMachineMemory->fp;
...
_clip_push_static(ClipMachine * ClipMachineMemory, void *vpp)
{
   ...
   if (!r)
	...
	{
	   if (!vp->ClipCodeVar_c_of_ClipVar.ClipVarFrame_uplocals_of_ClipCodeVar
	       && ClipMachineMemory->fp->ClipVarFrame_localvars_of_ClipFrame)
	    {
	       ClipMachineMemory->fp->ClipVarFrame_localvars_of_ClipFrame->refcount_of_ClipVarFrame++;
	       vp->ClipCodeVar_c_of_ClipVar.ClipVarFrame_uplocals_of_ClipCodeVar =
		ClipMachineMemory->fp->ClipVarFrame_localvars_of_ClipFrame;
	    }
	}
    }
...
_clip_hash(ClipMachine * ClipMachineMemory, ClipVar * vp)
{
   ...
   switch (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
    {
    case CHARACTER_type_of_ClipVarType:
       return _clip_hashbytes(0, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			      vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
    case NUMERIC_type_of_ClipVarType:
    ...
    case OBJECT_type_of_ClipVarType:
       return vp->ClipObjVar_o_of_ClipVar.rtti_of_ClipObjVar->hash_of_ClipObjRtti(ClipMachineMemory,
										  vp->ClipObjVar_o_of_ClipVar.obj_of_ClipObjVar,
										  vp->ClipObjVar_o_of_ClipVar.
										  rtti_of_ClipObjVar);
    default:
       break;
    }
   ...
   if (no < ClipMachineMemory->argc)
       ...
       if (vp->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType == F_MREF_ClipFlags)
	{
	  ...
	  _clip_mclone(ClipMachineMemory, param, vp);
       if (param->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType & F_MPTR_ClipFlags
	   && param->ClipType_t_of_ClipVar.field_of_ClipType)
	{
...
CLIP_DLLEXPORT void
_clip_add_private(ClipMachine * ClipMachineMemory, long hash)
{
   ...
   if (cp)
    {
       ...
       if ((vp->ClipVar_var_of_VarEntry.ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType & F_MPTR_ClipFlags)
...
CLIP_DLLEXPORT void
_clip_vremove_privates(ClipMachine * ClipMachineMemory, int num, long *vect)
{
   remove_private_vect(ClipMachineMemory, num, vect);
}
...
_clip_adel(ClipMachine * ClipMachineMemory, ClipVar * ap, int ndim, long *dims)
{
   ...
   memmove(ap->ClipArrVar_a_of_ClipVar.ClipVar_items_of_ClipArrVar + ind,
	   ap->ClipArrVar_a_of_ClipVar.ClipVar_items_of_ClipArrVar + ind + 1, (c - ind - 1) * sizeof(ClipVar));
...
_clip_asize(ClipMachine * ClipMachineMemory, ClipVar * ap, int ndim, long *dims)
{
   ...
   if (ndim > 1)
	...
	{
	   _clip_trap_printf(ClipMachineMemory, __file__, __LINE__,
			     "index for asize is out-of range: ind=%d, count=%d", ind + 1, c);
	   return _clip_call_errblock(ClipMachineMemory, 1);
	}
       app = ap->ClipArrVar_a_of_ClipVar.ClipVar_items_of_ClipArrVar + ind;
   ...
   ap->ClipArrVar_a_of_ClipVar.ClipVar_items_of_ClipArrVar =
    (ClipVar *) realloc(ap->ClipArrVar_a_of_ClipVar.ClipVar_items_of_ClipArrVar, sizeof(ClipVar) * ind);
   ...
   if (c > 0)
    {
       ...
       memmove(ap->ClipArrVar_a_of_ClipVar.ClipVar_items_of_ClipArrVar + ind + 1,
	       ap->ClipArrVar_a_of_ClipVar.ClipVar_items_of_ClipArrVar + ind, (c - ind - 1) * sizeof(ClipVar));
    }
...
_clip_aref(ClipMachine * ClipMachineMemory, ClipVar * ap, int ndim, long *dims)
{
   ...
   if ((ap->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != ARRAY_type_of_ClipVarType
	&& ap->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != MAP_type_of_ClipVarType) || !ndim)
    {
   ...
   if (ap->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == MAP_type_of_ClipVarType)
	...
	{
	   _clip_trap_printf(ClipMachineMemory, __file__, __LINE__,
			     "index for aget is out-of range: ind=%d, count=%d", ind + 1, c);
	   return 0;
	}
...
_clip_aset(ClipMachine * ClipMachineMemory, ClipVar * ap, ClipVar * vp, int ndim, long *dims)
{
   ...
   if (ap->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == MAP_type_of_ClipVarType)
	...
	{
	   _clip_trap_printf(ClipMachineMemory, __file__, __LINE__,
			     "index for aset is out-of range: ind=%d, count=%d", ind + 1, c);
	   return _clip_call_errblock(ClipMachineMemory, 1);
	}
...
CLIP_DLLEXPORT int
_clip_mputl(ClipMachine * ClipMachineMemory, ClipVar * ap, long no, int l)
{
   ClipVar   v;
...
_clip_madd(ClipMachine * ClipMachineMemory, ClipVar * ap, long no, ClipVar * vp)
{
   ...
   if (search_map(ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar, c, no, &ind))
    ...
    {
       ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar =
	(ClipVarEl *) realloc(ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar, (c + 1) * sizeof(ClipVarEl));
       if (ind < c)
	  memmove(ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar + ind + 1,
		  ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar + ind, (c - ind /* -1 */ ) * sizeof(ClipVarEl));
...
_clip_mdel(ClipMachine * ClipMachineMemory, ClipVar * ap, long no)
{
   ...
   if (search_map
       (ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar, ap->ClipMapVar_m_of_ClipVar.count_of_ClipMapVar, no, &ind))
    {
       _clip_destroy(ClipMachineMemory,
		     &((ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar + ind)->ClipVar_var_of_ClipVarEl));
	  ...
	  memmove(ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar + ind,
		  ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar + ind + 1,
		  (ap->ClipMapVar_m_of_ClipVar.count_of_ClipMapVar - ind) * sizeof(ClipVarEl));
    }
   return 0;
}
...
_clip_mind(ClipMachine * ClipMachineMemory, ClipVar * ap, long no, int *ind)
{
   ...
   return !search_map(ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar,
		      ap->ClipMapVar_m_of_ClipVar.count_of_ClipMapVar, no, ind);
}
...
test_maponerror(ClipMachine * ClipMachineMemory, ClipVar * map, char *member)
{
  ...
  /* try run `onerror` member */
   if (search_map
       (map->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar, map->ClipMapVar_m_of_ClipVar.count_of_ClipMapVar,
	HASH_onerror, &ind))
    {
       ClipVar  *ep;
       ...
       if (ep->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == PCODE_type_of_ClipVarType
	   || ep->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CCODE_type_of_ClipVarType)
	{
	   int       i;
...
static ClipVar *
fetch_arr(ClipMachine * ClipMachineMemory, ClipVar * ap, int dim, long *vect,
	  int Dim, long *Vect, int store, ClipVar ** mapp, long *hashp)
{
   long      no;
   ...
   if (type != ARRAY_type_of_ClipVarType && !isMap)
    {
       ...
       _clip_trap_printf(ClipMachineMemory, ClipMachineMemory->fp->filename_of_ClipFrame,
			 ...
			 "try fetch element from non-array object%s%s", buf[0] ? " " : "", buf);
   ...
   if (isMap)
       ...
       else
	{
	   if ((ClipMachineMemory->flags1 & MAPERR_FLAG))
		...
		{
		   _clip_trap_printf(ClipMachineMemory, ClipMachineMemory->fp->filename_of_ClipFrame,
				     ClipMachineMemory->fp->line_of_ClipFrame, "map fetch error: index %s", buf);
		   _clip_call_errblock(ClipMachineMemory, EG_BOUND);
		   return 0;
		}
	       else
   ...
   else if (no < 1 || no > ap->ClipArrVar_a_of_ClipVar.count_of_ClipArrVar)
    {
       ...
       _clip_trap_printf(ClipMachineMemory, ClipMachineMemory->fp->filename_of_ClipFrame,
			 ClipMachineMemory->fp->line_of_ClipFrame,
			 "out-of-bounds array %s, index=%d, count=%d", buf,
			 no, ap->ClipArrVar_a_of_ClipVar.count_of_ClipArrVar);
       _clip_call_errblock(ClipMachineMemory, EG_BOUND);
       return NULL;
    }
   if (isMap)
    {
      ...
      vp =
       fetch_arr(ClipMachineMemory, ap->ClipArrVar_a_of_ClipVar.ClipVar_items_of_ClipArrVar + no - 1, dim, vect, Dim, Vect,
		 store, mapp, hashp);
   ...
   if (vp && vp->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType & F_MPTR_ClipFlags)
    {
       count =
	vp->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar->ClipType_t_of_ClipVar.count_of_ClipType +
	clip_count(vp->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar);
    }
   else
      count = 0;
   return count;
   ...
   if (!vp)
    {
       _clip_trap_str(ClipMachineMemory, ClipMachineMemory->fp->filename_of_ClipFrame,
		      ClipMachineMemory->fp->line_of_ClipFrame, "map/array fetch error");
       _clip_call_errblock(ClipMachineMemory, EG_BOUND);
    }
...
_clip_log(ClipVar * vp)
{
   ...
   switch (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
    ...
    case NUMERIC_type_of_ClipVarType:
       if (vp->ClipType_t_of_ClipVar.memo_of_ClipType)
	  return rational_empty(vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
       else
...
_clip_long(ClipVar * vp)
{
   ...
   switch (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
    ...
    case NUMERIC_type_of_ClipVarType:
       if (vp->ClipType_t_of_ClipVar.memo_of_ClipType)
	  return (long) rational_toDouble(vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
       else
...
_clip_double(ClipVar * vp)
{
   ...
   switch (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
    ...
    case NUMERIC_type_of_ClipVarType:
       if (vp->ClipType_t_of_ClipVar.memo_of_ClipType)
	  return rational_toDouble(vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
       else
   ...
   for (; find_macro(s, e, &beg, &end, &next, &dot); s = next)
       ...
       if (vp && !(ClipMachineMemory->flags1 & NOEXPAND_MACRO_FLAG))
	{
	   ...
	   switch (vpp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
	    {
	    case CHARACTER_type_of_ClipVarType:
	       putBuf_Buf(&buf, vpp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			  vpp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
	       break;
	    case NUMERIC_type_of_ClipVarType:
	       {
		  if (vpp->ClipType_t_of_ClipVar.memo_of_ClipType)
		   {
			 ...
			 bf =
			  rational_toString(vpp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar, 10,
					    vpp->ClipType_t_of_ClipVar.dec_of_ClipType, 0);
		      else
			 bf =
			  rational_toString(vpp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar, 10,
		  ...
		  else
		   {
		      ...
		      if (vpp->ClipType_t_of_ClipVar.len_of_ClipType)
			 snprintf(bf, sizeof(bf), "%*.*f", vpp->ClipType_t_of_ClipVar.len_of_ClipType,
				  vpp->ClipType_t_of_ClipVar.dec_of_ClipType,
	    ...
	    case DATETIME_type_of_ClipVarType:
		  ...
		  char     *r =
		   _clip_ttoc(ClipMachineMemory, vpp->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar,
...
_clip_macro(ClipMachine * ClipMachineMemory)
{
   ...
   if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType)
    ...
    {
       _clip_trap_printf(ClipMachineMemory, __file__, __LINE__,
			 "macro operation with non-character argument (%s)", _clip_typename(vp));
       return _clip_call_errblock(ClipMachineMemory, 1);
    }
}
...
obj_call(struct ClipMachine *ClipMachineMemory, int argc, long hash, int call)
{
   ...
   switch (op->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
	...
	case CALL_CALL:
	   ret =
	    op->ClipObjVar_o_of_ClipVar.rtti_of_ClipObjVar->call_of_ClipObjRtti(ClipMachineMemory,
										op->ClipObjVar_o_of_ClipVar.obj_of_ClipObjVar,
										op->ClipObjVar_o_of_ClipVar.rtti_of_ClipObjVar,
										hash);
	   break;
	case CALL_GET:
	   ret =
	    op->ClipObjVar_o_of_ClipVar.rtti_of_ClipObjVar->get_of_ClipObjRtti(ClipMachineMemory,
									       op->ClipObjVar_o_of_ClipVar.obj_of_ClipObjVar,
									       op->ClipObjVar_o_of_ClipVar.rtti_of_ClipObjVar,
									       hash);
	   break;
	case CALL_SET:
	   ret =
	    op->ClipObjVar_o_of_ClipVar.rtti_of_ClipObjVar->set_of_ClipObjRtti(ClipMachineMemory,
									       op->ClipObjVar_o_of_ClipVar.obj_of_ClipObjVar,
									       op->ClipObjVar_o_of_ClipVar.rtti_of_ClipObjVar,
									       hash);
	   break;
    ...
    case MAP_type_of_ClipVarType:
	  ...
	  if (type == CCODE_type_of_ClipVarType)
	     ret =
	      _clip_func(ClipMachineMemory, vpp->ClipCodeVar_c_of_ClipVar.u.ClipFunction_function_of_ClipCodeVar, argc, 0,
			 vpp->ClipCodeVar_c_of_ClipVar.ClipVarFrame_uplocals_of_ClipCodeVar);
	  else
	     ret =
	      _clip_code_func(ClipMachineMemory, vpp->ClipCodeVar_c_of_ClipVar.u.ClipBlock_block_of_ClipCodeVar, argc, 0,
			      vpp->ClipCodeVar_c_of_ClipVar.ClipVarFrame_uplocals_of_ClipCodeVar);
...
CLIP_DLLEXPORT int
_clip_parni(ClipMachine * ClipMachineMemory, int num)
{
   return (int) _clip_parnd(ClipMachineMemory, num);
}

CLIP_DLLEXPORT long
_clip_parnl(ClipMachine * ClipMachineMemory, int num)
{
   return (long) _clip_parnd(ClipMachineMemory, num);
}
   ...
   if (num > 0 && num <= ClipMachineMemory->argc)
    {
       ...
       if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == NUMERIC_type_of_ClipVarType
	   && !vp->ClipType_t_of_ClipVar.memo_of_ClipType)
	{
	   return vp->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar;
	}
       if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == NUMERIC_type_of_ClipVarType
...
CLIP_DLLEXPORT char *
_clip_parc(ClipMachine * ClipMachineMemory, int num)
{
   return _clip_parcl(ClipMachineMemory, num, 0);
}
...
_clip_retndp(ClipMachine * ClipMachineMemory, double n, int len, int dec)
{
   ...
   if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType
       && vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf != NULL)
      free(vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf);
...
CLIP_DLLEXPORT void
_clip_retni(ClipMachine * ClipMachineMemory, int n)
{
   _clip_retndp(ClipMachineMemory, n, 0, 0);
}

CLIP_DLLEXPORT void
_clip_retnl(ClipMachine * ClipMachineMemory, long n)
{
   _clip_retndp(ClipMachineMemory, n, 0, 0);
}

CLIP_DLLEXPORT void
_clip_retnd(ClipMachine * ClipMachineMemory, double n)
{
   _clip_retndp(ClipMachineMemory, n, 0, 0);
}
...
_clip_retnr(ClipMachine * ClipMachineMemory, rational * r, int len, int dec)
{
   ...
   vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar = r;
}
...
CLIP_DLLEXPORT int
_clip_storclen(ClipMachine * ClipMachineMemory, const char *str, int len, int num, int ind)
{
#if 1
   ClipVar  *vp = _clip_spar(ClipMachineMemory, num);
   ...
   if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == ARRAY_type_of_ClipVarType)
    {
       if (ind < 0 || ind >= vp->ClipArrVar_a_of_ClipVar.count_of_ClipArrVar)
	  return 0;
       vp = _clip_vptr(vp->ClipArrVar_a_of_ClipVar.items_of_ClipVect + ind);
    }
...
CLIP_DLLEXPORT int
_clip_storni(ClipMachine * ClipMachineMemory, int d, int num, int ind)
{
   return _clip_stornd(ClipMachineMemory, d, num, ind);
}

CLIP_DLLEXPORT int
_clip_stornl(ClipMachine * ClipMachineMemory, long d, int num, int ind)
{
   return _clip_stornd(ClipMachineMemory, d, num, ind);
}
...
incr_var(ClipMachine * ClipMachineMemory, ClipVar * vp, int isIncr)
{
   ...
   switch (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType)
       ...
       if (vp->ClipType_t_of_ClipVar.memo_of_ClipType)
	{
	   ...
	   rational_adda(vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar, tmp);
	   rational_destroy(tmp);
	}
       else
...
_clip_hash_name(ClipMachine * ClipMachineMemory, long hash, char *buf, int buflen)
{
   ...
   for (fp = ClipMachineMemory->fp; fp; fp = fp->ClipFrame_up_of_ClipFrame)
       ...
       if (fp->names_of_ClipFrame)
	{
	   ...
	   bp = (ClipHashBucket *) bsearch(&hash, fp->names_of_ClipFrame->buckets_of_ClipHashNames,
					   fp->names_of_ClipFrame->num_of_ClipHashNames,
					   sizeof(ClipHashBucket), cmp_hash_bucket);
	   if (bp)
	    {
...
_clip_fetch_c_item(ClipMachine * ClipMachineMemory, int key, int type)
{
   ...
   ci =
    bsearch(&citem, ClipMachineMemory->container->ContainerItem_items_of_Container,
	    ClipMachineMemory->container->len_of_Container, sizeof(ContainerItem), _clip_compare_c_item);
...
_clip_fetch_c_item_type(ClipMachine * ClipMachineMemory, int type, int *key)
{
   ...
   ci =
    bsearch(&citem, ClipMachineMemory->container->ContainerItem_items_of_Container,
	    ClipMachineMemory->container->len_of_Container, sizeof(ContainerItem), _clip_compare_c_item_type);
   if (ci && key)
...
_clip_destroy_c_item(ClipMachine * ClipMachineMemory, int key, int type)
{
   ...
   citem.key_of_ContainerItem = key;
   ci =
    bsearch(&citem, cn->ContainerItem_items_of_Container, cn->len_of_Container, sizeof(ContainerItem), _clip_compare_c_item);
...
_clip_store_c_item(ClipMachine * ClipMachineMemory, void *item, int type, void (*destroy) (void *))
{
  ...
  /* search for first unused key */
   ...
   cn->ContainerItem_items_of_Container =
    (ContainerItem *) realloc(cn->ContainerItem_items_of_Container, (cn->len_of_Container + 1) * sizeof(ContainerItem));
...
CLIP_DLLEXPORT int
_clip_trap_err(ClipMachine * ClipMachineMemory, int genCode, int canDefault,
	       int canRetry, const char *subSystem, int subCode, const char *operation)
{
   ClipVar  *vp;

   vp =
    _clip_new_error(ClipMachineMemory, _clip_errname(genCode), 2, genCode, 0,
		    canDefault, canRetry, 0, 1, subSystem, subCode, operation);
   ...
   if (!_clip_mind(ClipMachineMemory, vp, no, &ind))
    {
       ...
       ap = _clip_vptr(vp);
       return ap->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar[ind].ClipVar_var_of_ClipVarEl.ClipType_t_of_ClipVar.
   ...
   if (vvp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == NUMERIC_type_of_ClipVarType)
    {
	  ...
	  hash = (long) rational_toDouble(vvp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
       else
      ...
      hash =
       _clip_casehashbytes(0, vvp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
			   vvp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf);
   ...
   if (ClipMachineMemory->flags & RATIONAL_FLAG)
    {
	  ...
	  tmp1 = Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar;
       if (!Rval->ClipType_t_of_ClipVar.memo_of_ClipType)
	  tmp2 = rational_double_init(Rval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar);
       else
	  tmp2 = Rval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar;
       switch (op)
	{
	case '-':
       ...
       if (!Lval->ClipType_t_of_ClipVar.memo_of_ClipType)
	{
	   ...
	   Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar = tmp1;
	}
       if (!Rval->ClipType_t_of_ClipVar.memo_of_ClipType)
    ...
    {
       if (Lval->ClipType_t_of_ClipVar.memo_of_ClipType)
	  d1 = rational_toDouble(Lval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
       else
	  d1 = Lval->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar;
       if (Rval->ClipType_t_of_ClipVar.memo_of_ClipType)
	  d2 = rational_toDouble(Rval->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar);
       else
...
_clip_forstep(ClipMachine * ClipMachineMemory, int *_if)
{
   ...
   if (step->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != NUMERIC_type_of_ClipVarType
       ...
       || (val->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != NUMERIC_type_of_ClipVarType
	   && val->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != DATE_type_of_ClipVarType))
      return EG_ARG;
...
CLIP_DLLEXPORT int
_clip_map_next(ClipMachine * ClipMachineMemory, int keys, int *_if)
{
   ClipVar  *rp = ClipMachineMemory->fp->ClipVar_sp_of_ClipFrame - 1;
   ...
   *_if = 0;
   ...
   if (arr->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == MAP_type_of_ClipVarType)
	...
	{
	   no =
	    arr->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar[arr->ClipMapVar_m_of_ClipVar.current_of_ClipMapVar].
	    no_of_ClipVarEl;
	   vp =
	    &(arr->ClipMapVar_m_of_ClipVar.ClipVarEl_items_of_ClipMapVar[arr->ClipMapVar_m_of_ClipVar.current_of_ClipMapVar].
	      ClipVar_var_of_ClipVarEl);
	   *_if = 1;
	}
    }
   else if (arr->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == ARRAY_type_of_ClipVarType)
...
_clip_swap(ClipMachine * ClipMachineMemory, int offset)
{
   ...
   memmove(ClipMachineMemory->fp->ClipVar_sp_of_ClipFrame - offset, ClipMachineMemory->fp->ClipVar_sp_of_ClipFrame - offset - 1,
	   offset * sizeof(ClipVar));
...
local_ref(ClipMachine * ClipMachineMemory, int no)
{
   ...
   if (lp && no < lp->size_of_ClipVarFrame)
   ...
   else
    {
       ...
       if (lp)
	{
	   memset(localvars->ClipVar_vars_of_ClipVarFrame + no, 0,
		  (nlocals - localvars->size_of_ClipVarFrame) * sizeof(ClipVar));
	}
       else
	{
   ...
   if (vp->ClipType_t_of_ClipVar.ClipFlags_flags_of_ClipType & F_MPTR_ClipFlags
       && vp->ClipRefVar_p_of_ClipVar.ClipVar_of_ClipRefVar->ClipType_t_of_ClipVar.
       ClipFlags_flags_of_ClipType & F_MPTR_ClipFlags)
    {
#if 1
...
_clip_refmacro(ClipMachine * ClipMachineMemory)
{
   ...
   if (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != CHARACTER_type_of_ClipVarType)
    {
       _clip_trap_printf(ClipMachineMemory, __file__, __LINE__,
			 "macro reference with non-character argument (%s)", _clip_typename(vp));
       return 0;
    }

   r =
    _clip_parse_name(ClipMachineMemory, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
		     vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf, &aname, &al, &fname, &fl, &dim,
		     &ndim);
   ...
   for (i = 0; i < num; i++)
	...
	case NUMERIC_type_of_ClipVarType:
	   ...
	   if (!vp->ClipType_t_of_ClipVar.memo_of_ClipType)
	    ...
	    {
	       if (len)
		  s1 = rational_toString(vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar, 10, dec, 0);
	       else
		{
		   s1 = rational_toString(vp->ClipRationalVar_r_of_ClipVar.rational_of_ClipRationalVar, len, dec, 0);
...
_clip_quot(ClipMachine * ClipMachineMemory)
{
   ...
   if (rp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType)
    ...
    {
       s =
	_clip_ttoc(ClipMachineMemory, rp->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar,
		   rp->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar, &l, ClipMachineMemory->date_format,
		   ClipMachineMemory->hours, ClipMachineMemory->seconds);
    }
   ...
   if (fp && fp->ClipVar_sp_of_ClipFrame > (fp->ClipVar_stack_of_ClipFrame + fp->stklen_of_ClipFrame))
    {
       ...
       _clip_logg(0, "stack check failed at line %d file %d", fp->line_of_ClipFrame,
		  fp->filename_of_ClipFrame ? fp->filename_of_ClipFrame : "unknown");
       exit(1);
    }
}
