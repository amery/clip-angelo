...
clip_SQLCREATEROWSET(ClipMachine * ClipMachineMemory)
{
   ...
   if (!stmt)
    {
	 ...
	 if (orders && orders->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != ARRAY_type_of_ClipVarType
		&& orders->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != UNDEF_type_of_ClipVarType)
    {
   ...
   rowset->stmt = stmt;
       ...
       for (i = 0; i < orders->ClipArrVar_a_of_ClipVar.count_of_ClipArrVar; i++)
	{
	   ...
	   dims[0] = 2;
	   if ((olen = _clip_aref(ClipMachineMemory, el, 1, dims))
			&& olen->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType != NUMERIC_type_of_ClipVarType)
	    {
	       _clip_trap_err(ClipMachineMemory, 0, 0, 0, subsys, EG_ARG, er_badorders);
	       return 1;
	    }
	   if (sql_createorder
	       (ClipMachineMemory, rmap, rowset, oname->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf, oexpr,
		(olen ? olen->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar : 0)))
	      return 1;
	}
    }
