...
#		# sleep $SleepTime
		...
		while [[ $varX = [nN] ]] ; do
			...
			if [ -z "$Found" ] ; then
				...
				read -n 1 -p "Is it Ok [y/n/s] : " varX 	>&0
				OnScreen 2
				if [[ $varX = [yY] ]] ; then
...
function Config_Init ()
	{
	   ...
	   if [ $? != 0 ] ; then
		...
		cp --remove-destination -puf$V ../configure.ini ./
		cp --remove-destination -puf$V ../Makefile.ini ./
...
function GetArch ()
	{
		...
		while [ $yz -eq -1 ] ; do
			...
			rm -Rf$V $Clip_H_Dir/clip32 	>&0
			rm -Rf$V $Clip_H_Dir/clip64 	>&0
			varX=0
