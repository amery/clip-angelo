...
if [ -f /opt/interbase/include/ibase.h ] ; then
	SourceDir="/opt"
elif [ -f /usr/local/interbase/include/ibase.h ] ; then
	SourceDir="/usr/local"
else
	SourceDir=""
fi
...
if [ -n $SourceDir ] ; then
	cp -sf$V $SourceDir/interbase/include/* 		$PWD/usr/include/

	cp -sf$V $SourceDir/interbase/lib/* 			$PWD/usr/lib$EXE/
