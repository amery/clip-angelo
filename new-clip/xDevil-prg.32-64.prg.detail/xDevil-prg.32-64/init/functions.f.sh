...
############################################################################################################
##                                                                                                        ##
##   this script CANNOT be run alone, it MUST be called from ../configure or ../menu.sh              ##
##                                                                                                        ##
############################################################################################################
############################################################################################################
#ProgName=""
...
function banner ()
	{
		if [[ "$Bban" != 0 ]] ; then
			export Bban=0
			Check_Bin banner xyz xDevil yes /usr/bin banner-xx-$ARCH.rpm
		fi
...
#			el
			if [ -x /usr/bin/mplayer ] ; then
				/usr/bin/mplayer -vo null $xDevil_M_Dir/sounds/Kopete_Received.ogg 1>/dev/null 2>/dev/null
				echo " beep mplayer beep" 	>/dev/null
				echo "" 					>&0
			elif [ -x /usr/bin/play ] ; then
				/usr/bin/play -ao alsa $xDevil_M_Dir/sounds/Kopete_Received.ogg 1>/dev/null 2>/dev/null
...
function Check_Bin ()
	{
		...
		#
		local LogFile="$xDevil_T_Dir/askbin.log"
		local WhatToFind
		local Extension
		local RequestedBy
		local Important
		...
		IdxToFind=0
		if [ -f "$xDevil_S_Dir/$ARCH/$WhatToFind/.set" ] ; then
			CurIdx="$xDevil_S_Dir/$ARCH/$WhatToFind/.set"
			lList="$(cat $CurIdx)"
			# sleep $SleepTime
			for fName in $lList ; do
				if [[ x"$fName" != x"" ]] ; then
					...
					echo $fName >"$xDevil_S_Dir/$ARCH/$WhatToFind/.set"
...
#		# sleep $SleepTime
		...
		while [[ $varX = [nN] ]] ; do
			...
			if [ -z "$Found" ] ; then
				...
				OnScreen 2
				if [[ $varX = [yY] ]] ; then
					mkdir -p$V "$xDevil_S_Dir/$ARCH/$WhatToFind/" >&0
					echo $Found >"$xDevil_S_Dir/$ARCH/$WhatToFind/.set"
					# sleep $SleepTime
					# OnScreen 0
...
function Check_Include ()
	{
		...
		#
		local LogFile="$xDevil_T_Dir/askinclude.log"
		local WhatToFind
		local Extension
		local RequestedBy
...
function Check_Library ()
	{
		...
		#
		local LogFile="$xDevil_T_Dir/asklibrary.log"
		local WhatToFind
		local Extension
		local RequestedBy
		local Important
		...
		if [ -f "$xDevil_S_Dir/$ARCH/$WhatToFind/.set" ] ; then
			...
			for fName in $lList ; do
				if [[ x"$fName" != x"" ]] ; then
					OnScreen 2 "Found : $WhatToFind requested by : $RequestedBy found in : $fName"
					mkdir -p$V "$xDevil_S_Dir/$ARCH/$WhatToFind/"
					echo $fName >"$xDevil_S_Dir/$ARCH/$WhatToFind/.set"
					ArrayToFind[$IdxToFind]=$fName
...
function Config_Init ()
	{
	   ...
	   if [ $? != 0 ] ; then
		...
		[ -d include ] 	|| [ -L include ] 	|| ln -sf$V $xDevil_I_Dir 			./include
		[ -d locale.po ] 	|| [ -L locale.po ] 	|| ln -sf$V $xDevilROOT/locale.po 	./locale.po
		[ -d charsets ] 	|| [ -L charsets ] 	|| ln -sf$V $xDevilROOT/charsets 	./charsets
...
function CreateAllDirs ()
	{
		xDevil_C_Dir="$xDevil_M_Dir/config"
		xDevil_S_Dir="$xDevil_C_Dir/setup"
		xDevil_T_Dir="$xDevil_M_Dir/temp"
		xDevil_D_Dir="$xDevilROOT/doc"
		Hash_Dir="$xDevil_M_Dir/hasher/hashed"
		[ -d $xDevil_S_Dir ] 	|| mkdir -p$V $xDevil_S_Dir
		[ -d $xDevil_T_Dir ] 	|| mkdir -p$V $xDevil_T_Dir
		[ -d $xDevil_B_Dir ] 	|| mkdir -p$V $xDevil_B_Dir
		[ -d $xDevil_D_Dir ] 	|| mkdir -p$V $xDevil_D_Dir
		[ -d $xDevil_I_Dir ] 	|| mkdir -p$V $xDevil_I_Dir
		[ -d $xDevil_L_Dir ] 	|| mkdir -p$V $xDevil_L_Dir
...
function deception ()
	{
		if [ -x /usr/bin/mplayer ] ; then
			mplayer -vo null $xDevil_M_Dir/sounds/k3b_error1.wav 1>/dev/null 2>/dev/null
			echo " pitty mplayer pitty" 	>&0
			echo "" 								>&0
		elif [ -x /usr/bin/play ] ; then
			play -ao alsa $xDevil_M_Dir/sounds/k3b_error1.wav 1>/dev/null 2>/dev/null
...
function error ()
	{
		...
		#
		if [ -x /usr/bin/mplayer ] ; then
			/usr/bin/mplayer -vo null $xDevil_M_Dir/sounds/k3b_error1.wav 1>/dev/null 2>/dev/null
			echo " pitty mplayer pitty" 	>&0
			echo "" 								>&0
		elif [ -x /usr/bin/play ] ; then
			/usr/bin/play -ao alsa $xDevil_M_Dir/sounds/k3b_error1.wav 1>/dev/null 2>/dev/null
			echo " pitty play pitty" 		>&0
			echo "" 								>&0
		else
			beep_on
...
function GetArch ()
	{
		...
		if [ -f $xDevil_S_Dir/ARCH.setup.ini ] ; then
			ARCH=$(cat "$xDevil_S_Dir/ARCH.setup.ini")
		fi
		for nii in $ARCH ; do
			ARCH=$nii
		done
		...
		while [ $yz -eq -1 ] ; do
			...
			[ -d $xDevil_S_Dir ] || mkdir -p$V $xDevil_S_Dir
			echo $ARCH >$xDevil_S_Dir/ARCH.setup.ini
		done
		if [ $yz -eq 2 ] && [[ $C64 = "x86_64" ]] ; then
			ARCH=x86_64
		else
...
function MakeDocDir ()
	{
		if [ -n $* ] ; then
			# echo "xDevil_D_Dir=$xDevil_D_Dir"
			[ -d $xDevil_D_Dir/$1 ] 			|| mkdir -p$V $xDevil_D_Dir/$1
			[ -d $xDevil_D_Dir/example/$1 ] || mkdir -p$V $xDevil_D_Dir/example/$1
		fi
	}
function oneof ()
	{
...
function temp_file ()
	{
		Temp_File_Name=$xDevil_T_Dir/temp.$Temp_File_Number
...
function trumpet ()
	{
		if [ -x /usr/bin/mplayer ] ; then
			/usr/bin/mplayer -vo null $xDevil_M_Dir/sounds/k3b_success1.wav 1>/dev/null 2>/dev/null
			echo " successful mplayer" 	>&0
			echo "" 								>&0
		elif [ -x /usr/bin/play ] ; then
			/usr/bin/play -ao alsa $xDevil_M_Dir/sounds/k3b_success1.wav 1>/dev/null 2>/dev/null
