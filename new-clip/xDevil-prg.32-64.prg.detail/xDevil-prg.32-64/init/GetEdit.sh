...
if [[ -f "$xDevil_S_Dir/Edit.setup.ini" ]] ; then
	export Edit=$(cat "$xDevil_S_Dir/Edit.setup.ini" )
else
	if [ -x /usr/bin/kwrite ] ; then
		export Edit="/usr/bin/kwrite"
		echo "$Edit" >"$xDevil_S_Dir/Edit.setup.ini"
		echo "Y" >"$xDevil_S_Dir/MultiFile.setup.ini"
	fi
fi
...
if [[ 1"" = 1"$Edit" ]] || [[ 1"" = 1"$MultiFile" ]]; then
	...
	if [[ $MultiFile = [Nn] ]] ; then
		echo "N" >"$xDevil_S_Dir/MultiFile.setup.ini"
	else
		echo "Y" >"$xDevil_S_Dir/MultiFile.setup.ini"
	fi
fi
