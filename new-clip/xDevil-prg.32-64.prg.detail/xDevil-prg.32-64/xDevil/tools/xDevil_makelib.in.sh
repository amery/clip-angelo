...
if [ -f xDevil_makelib.ini ] ; then
	printf "configure: creating xDevil_makelib$SCRIPTSUFF .."
	exec 																3>xDevil_makelib$SCRIPTSUFF
	echo '#!/bin/sh -u' 											>&3
	...
	echo "CC=$CC" 													>&3
	cat xDevil_makelib.ini 										>&3
	exec 																3>&-
	chmod +x xDevil_makelib$SCRIPTSUFF
	echo ". done."
fi
