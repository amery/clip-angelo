...
local file_menu, action_menu, doc_menu
...
i := file_menu:add( UIImage("icons/journal_bank_pp.xpm"),"&Payment orders", {|| qout("Payment orders") } )
file_menu:disable(i)
i := file_menu:add( UIImage("icons/doc_bank_pp.xpm"),"&Create payment order", {|| qout("Create payment order") } )
file_menu:add(,"Submenu",help_menu)
help_menu:add(UIImage("icons/journal_bank_pp.xpm"),"Item",{|| qout("Item") })
help_menu:add(,"Item2",{|| qout("Item2") })
file_menu:setKey(i,"F5")
file_menu:addSeparator()
file_menu:add(,"&Exit", @quit())
