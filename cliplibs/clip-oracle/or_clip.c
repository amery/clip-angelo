		   ...
		   if (rowset->data[i][j])
		    {
			   ...
			   rowset->fields[j].type == SQLT_BLOB || rowset->fields[j].type == SQLT_FILE)
			{
...
or_oracletype(dvoid ** ordata, ClipVar cldata, double *type, sb4 * len, OR_STMT * stmt, ClipMachine * mp, sb2 * ind)
{
   ...
   if ((cldata.ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType)
   ...
   if (*type == SQLT_RDD || *type == SQLT_RID)
   ...
   else if (*type == SQLT_CLOB || *type == SQLT_BLOB)
    {
       CHECKENV(stmt->conn, ER_RDBMSERROR, -1,
		res = OCIDescriptorAlloc((dvoid *) stmt->conn->env, (dvoid **) & lob, OCI_DTYPE_LOB, 0, NULL)) if (*len == 0)
	{
	CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
		    res = OCIAttrSet((dvoid *) lob, OCI_DTYPE_LOB, &lobempty, 0, OCI_ATTR_LOBEMPTY, stmt->conn->err))}
       else
	{
	   CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
		    OCILobAssign(stmt->conn->env, stmt->conn->err,
				 *(OCILobLocator **) cldata.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
				 &lob)) lob =
	    *(OCILobLocator **) cldata.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf;
	   CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
		    OCILobCharSetForm(stmt->conn->env, stmt->conn->err, lob, &csfrm)) CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
											       OCILobCharSetId(stmt->conn->env,
													       stmt->conn->err,
													       lob,
													       &csid))
	    CHECKERR(stmt->conn, ER_RDBMSERROR, -1, OCILobTrim(stmt->conn->context, stmt->conn->err, lob, 0));
	   CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
		    OCILobGetChunkSize(stmt->conn->context, stmt->conn->err, lob, &chunksize)) towrite =
	    min(cldata.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf - 4, chunksize);
	   CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
		    OCILobOpen(stmt->conn->context, stmt->conn->err, lob,
			       OCI_LOB_READWRITE)) while (wrote <
							  cldata.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.
							  len_of_ClipBuf - 4)
	    {
	       OCILobWrite(stmt->conn->context, stmt->conn->err, lob,
			   &towrite, 1 + wrote,
			   cldata.ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf + 4 + wrote, towrite,
...
or_createconn(ClipMachine * mp)
{
   ...
   CHECKENV(conn, ER_CONNECT, -1,
    ...
    CHECKERR(conn, ER_CONNECT, -1,
	     OCIAttrSet((dvoid *) context, OCI_HTYPE_SVCCTX, (dvoid *) trans, 0, OCI_ATTR_TRANS, err)) conn->serv = serv;
   conn->sess = sess;
   ...
   CHECKENV(((OR_CONN *) conn), ER_RDBMSERROR, -1,
    ...
    CHECKERR(((OR_CONN *) conn), ER_BADSTATEMENT, -1,
	     OCIStmtPrepare(stmt->stmt,
			    ...
			    OCI_DEFAULT)) return stmt->stmt_item;
}

static int
or_bindpars(ClipMachine * mp, OR_STMT * stmt, dvoid *** ordata, sb2 ** ind, ClipVar * ap, int *isrowid, int *islong,
	    int *longlen, int *longfield)
{
   char      msg[256];
       ...
       && (ap->ClipArrVar_a_of_ClipVar.count_of_ClipArrVar))
    {
       CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
		OCIAttrGet((dvoid *) stmt->stmt, OCI_HTYPE_STMT, (dvoid *) & stmttype,
			   0, OCI_ATTR_STMT_TYPE, stmt->conn->err)) if (isrowid)
	  *isrowid = 0;
       if (islong)
	  *islong = 0;
       *ordata = malloc(0);
       ...
       for (i = 0; i < ap->ClipArrVar_a_of_ClipVar.count_of_ClipArrVar; i++)
	{
	   *ordata = realloc(*ordata, sizeof(dvoid *) * (i + 1));
	   ...
	   if (or_oracletype(&((*ordata)[i]), *pv,
			     &pl->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar, &ordatalen, stmt, mp, &(*ind)[i]))
	    {
	       return -1;
	    }
	   ...
	   if ((stmttype == OCI_STMT_INSERT || stmttype == OCI_STMT_UPDATE) &&
	       (pl->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar == SQLT_LNG
		|| pl->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar == SQLT_LBI
		|| pl->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar == SQLT_LVC
		|| pl->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar == SQLT_LVB))
	    {
	       if (islong)
		  *islong = 1;
	       if (longlen)
		  ...
		  *longfield = i;
	       CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
			OCIBindByName(stmt->stmt, &bind, stmt->conn->err,
				      pn->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
				      strlen(pn->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf), NULL,
				      *longlen, (ub2) pl->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar, &(*ind)[i], NULL, NULL,
				      0, NULL, OCI_DATA_AT_EXEC)) continue;
	    }
	CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
		    OCIBindByName(stmt->stmt, &bind, stmt->conn->err,
				     pn->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf,
				     strlen(pn->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf),
				     (*ordata)[i], ordatalen, (ub2) pl->ClipNumVar_n_of_ClipVar.double_of_ClipNumVar,
...
or_command(ClipMachine * mp, SQLSTMT * sqlstmt, ClipVar * ap)
{
   ...
   CHECKERR(conn, ER_RDBMSERROR, -1,
	    OCIAttrGet((dvoid *) stmt->stmt, OCI_HTYPE_STMT, (dvoid *) & stmttype,
		       0, OCI_ATTR_STMT_TYPE, stmt->conn->err)) if (stmttype == OCI_STMT_SELECT)
    {
   ...
   if (stmttype == OCI_STMT_INSERT && isrowid)
    ...
    CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
		OCIBindByName(stmt->stmt, &bind, stmt->conn->err,
				 "ID___", 5, &stmt->conn->gRowid, 18, SQLT_CHR, NULL, NULL, NULL, 0, 0, OCI_DEFAULT))}
   if (islong)
    {
    ...
    CHECKERR(stmt->conn, ER_BADSTATEMENT, -1,
		OCIStmtExecute(stmt->conn->context, stmt->stmt, stmt->conn->err, 1, 0, NULL, NULL, OCI_DEFAULT))}
   ...
   CHECKERR(stmt->conn, ER_RDBMSERROR, -1,
	    OCIAttrGet((dvoid *) stmt->stmt, OCI_HTYPE_STMT, (dvoid *) & rows,
		       0, OCI_ATTR_ROW_COUNT, stmt->conn->err)) if (!conn->at)
    {
...
or_createrowset(ClipMachine * mp, SQLROWSET * rs, ClipVar * ap, ClipVar * idname, const char *gen_idSQL)
{
   ...
   CHECKERR(conn, ER_RDBMSERROR, 1,
	    OCIAttrGet((dvoid *) stmt->stmt, OCI_HTYPE_STMT, (dvoid *) & stmttype,
		       0, OCI_ATTR_STMT_TYPE, conn->err)) if (stmttype != OCI_STMT_SELECT)
    {
   ...
   CHECKERR(conn, ER_BADSELECT, 1,
    ...
    or_freebinds(&ordata, &indin, ap->ClipArrVar_a_of_ClipVar.count_of_ClipArrVar);
   ...
   CHECKERR(conn, ER_RDBMSERROR, 1,
	    res = OCIParamGet(stmt->stmt, OCI_HTYPE_STMT, conn->err,
			      (dvoid **) & param, (ub4) i + 1)) while (res == OCI_SUCCESS)
    {
	...
	CHECKERR(conn, ER_RDBMSERROR, 1,
		 OCIAttrGet((dvoid *) param, OCI_DTYPE_PARAM,
			    ...
			    conn->err)) strncpy(rowset->fields[i].name, col_name, MAXFIELDNAME);
   ...
   for (i = 0; i < rowset->nfields; i++)
    {
	   ...
	   rowset->fields[i].type == SQLT_UIN || rowset->fields[i].type == SQLT_FLT || rowset->fields[i].type == SQLT_VNU)
	{
	...
	CHECKERR(conn, ER_RDBMSERROR, 1,
		    OCIDefineByPos(stmt->stmt, &stmt->define, conn->err, i + 1, stmt->defbuf[i],
				      18, SQLT_CHR, (dvoid *) & stmt->inds[i], &stmt->rlen[i], NULL, OCI_DEFAULT))}
       else if (rowset->fields[i].type == SQLT_LNG ||
		rowset->fields[i].type == SQLT_LBI || rowset->fields[i].type == SQLT_LVC || rowset->fields[i].type == SQLT_LVB)
	{
	   CHECKERR(conn, ER_RDBMSERROR, 1,
		    OCIDefineByPos(stmt->stmt, &stmt->define, conn->err, i + 1, NULL,
				   0x7fffffff, rowset->fields[i].type, (dvoid *) & stmt->inds[i],
				   &stmt->rlen[i], NULL, OCI_DYNAMIC_FETCH)) stmt->longfield = i;
	}
       else if (rowset->fields[i].type == SQLT_CLOB ||
		rowset->fields[i].type == SQLT_BLOB || rowset->fields[i].type == SQLT_FILE)
	{
   ...
   for (i = 0; i < rowset->nfields; i++)
	...
	CHECKERR(conn, ER_RDBMSERROR, -1,
		    OCIDefineByPos(stmt->stmt, &define, conn->err, i + 1, defbuf[i],
				      sizeof(double), SQLT_FLT, (dvoid *) & (inds[i]), &rlen[i], NULL, OCI_DEFAULT))}
       else if (rowset->fields[i].type == SQLT_RDD || rowset->fields[i].type == SQLT_RID)
	{
	   defbuf[i] = malloc(18);
	CHECKERR(conn, ER_RDBMSERROR, -1,
		    OCIDefineByPos(stmt->stmt, &define, conn->err, i + 1, defbuf[i],
				      18, SQLT_CHR, (dvoid *) & inds[i], &rlen[i], NULL, OCI_DEFAULT))}
       else if (rowset->fields[i].type == SQLT_LNG ||
		rowset->fields[i].type == SQLT_LBI || rowset->fields[i].type == SQLT_LVC || rowset->fields[i].type == SQLT_LVB)
	{
	   CHECKERR(conn, ER_RDBMSERROR, -1,
		    OCIDefineByPos(stmt->stmt, &define, conn->err, i + 1, NULL,
				   ...
				   &rlen[i], NULL, OCI_DYNAMIC_FETCH)) longfield = i;
	}
       else if (rowset->fields[i].type == SQLT_CLOB ||
		rowset->fields[i].type == SQLT_BLOB || rowset->fields[i].type == SQLT_FILE)
	{
	   ...
	   while (res)
	    {
	       CHECKERR(conn, ER_RDBMSERROR, -1,
			OCIStmtGetPieceInfo(stmt->stmt, conn->err, (dvoid **) & define, &type,
					    &in_out, &iter, &idx, &piece)) piecelen = PIECEBUFLEN;
       ...
       for (i = 0; i < rowset->nfields; i++)
	{
	   if (rowset->fields[i].type == SQLT_RDD || rowset->fields[i].type == SQLT_RID)
		    ...
		    rowset->fields[i].type == SQLT_BLOB || rowset->fields[i].type == SQLT_FILE)
	    {
	       ...
	       CHECKERR(conn, ER_RDBMSERROR, -1,
			OCILobLocatorIsInit(conn->env, conn->err, *(OCILobLocator **) defbuf[i], &inited)) if (inited)
		{
		   if (rowset->fields[i].type == SQLT_FILE)
		    {
		    CHECKERR(conn, ER_RDBMSERROR, -1,
				OCILobFileOpen(conn->context, conn->err, *(OCILobLocator **) defbuf[i], OCI_FILE_READONLY))}
		   else
		    {
		    CHECKERR(conn, ER_RDBMSERROR, -1,
				OCILobOpen(conn->context, conn->err, *(OCILobLocator **) defbuf[i], OCI_LOB_READONLY))}
		   CHECKERR(conn, ER_RDBMSERROR, -1,
			    OCILobGetChunkSize(conn->context, conn->err,
					       *(OCILobLocator **) defbuf[i], &chunksize))
		    CHECKERR(conn, ER_RDBMSERROR, -1,
			     OCILobGetLength(conn->context, conn->err,
					     *(OCILobLocator **) defbuf[i], &loblen)) rec[i] = malloc(loblen + 8);
		   ...
		   *((OCILobLocator **) ((char *) rec[i] + 4)) = *(OCILobLocator **) defbuf[i];
		   ...
		   if (rowset->fields[i].type == SQLT_FILE)
		    {
		    CHECKERR(conn, ER_RDBMSERROR, -1,
				OCILobFileClose(conn->context, conn->err, *(OCILobLocator **) defbuf[i]))}
		   else
		    {
   ...
   if (((OR_ROWSET *) rowset)->fields[fieldno].type == SQLT_CLOB || ((OR_ROWSET *) rowset)->fields[fieldno].type == SQLT_BLOB)
    {
       if (((OR_ROWSET *) rowset)->data[((OR_ROWSET *) rowset)->recno - 1][fieldno])
	{
	   ((OR_ROWSET *) rowset)->data[((OR_ROWSET *) rowset)->recno - 1][fieldno] =
	    realloc(((OR_ROWSET *) rowset)->data[((OR_ROWSET *) rowset)->recno - 1][fieldno], len + 8);
	}
       else
	{
       ...
       for (i = 0; i < rowset->nfields; i++)
	{
	   if (rowset->fields[i].type == SQLT_RDD || rowset->fields[i].type == SQLT_RID)
		    ...
		    rowset->fields[i].type == SQLT_BLOB || rowset->fields[i].type == SQLT_FILE)
	    {
	       ...
	       CHECKERR(conn, ER_RDBMSERROR, 1,
			OCILobLocatorIsInit(conn->env, conn->err, *(OCILobLocator **) stmt->defbuf[i], &inited)) if (inited)
		{
		   if (rowset->fields[i].type == SQLT_FILE)
		    {
		    CHECKERR(conn, ER_RDBMSERROR, 1,
		    ...
		    {
		    CHECKERR(conn, ER_RDBMSERROR, 1,
				OCILobOpen(conn->context, conn->err, *(OCILobLocator **) stmt->defbuf[i], OCI_LOB_READONLY))}
		   CHECKERR(conn, ER_RDBMSERROR, 1,
			    OCILobGetChunkSize(conn->context, conn->err,
					       *(OCILobLocator **) stmt->defbuf[i], &chunksize))
		    CHECKERR(conn, ER_RDBMSERROR, 1,
			     OCILobGetLength(conn->context, conn->err,
					     *(OCILobLocator **) stmt->defbuf[i], &loblen)) rec[i] = malloc(loblen + 8);
		   *((unsigned int *) (rec[i])) = loblen;
		   *((OCILobLocator **) ((char *) rec[i] + 4)) = *(OCILobLocator **) stmt->defbuf[i];
		   ...
		   if (rowset->fields[i].type == SQLT_FILE)
		    {
		    CHECKERR(conn, ER_RDBMSERROR, 1,
				OCILobFileClose(conn->context, conn->err, *(OCILobLocator **) stmt->defbuf[i]))}
		   else
		    {
		    CHECKERR(conn, ER_RDBMSERROR, 1,
				OCILobClose(conn->context, conn->err, *(OCILobLocator **) stmt->defbuf[i]))}
