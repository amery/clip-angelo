...
#define CHECKERR(s,c,m,f,r) \
	if((s)[0] == 1 && (s)[1]){ \
		...
		_clip_trap_err(ClipMachineMemory,0,0,0,subsys,(c),err); \
		if(f); \
		return (r); \
	}
...
clip_INIT_FIREBIRD(ClipMachine * ClipMachineMemory)
{
   ...
   *ClipMachineMemory->sqldrivers =
    realloc(*ClipMachineMemory->sqldrivers, sizeof(SQLDriver) * (*ClipMachineMemory->nsqldrivers));
	 ...
	 strcpy((*ClipMachineMemory->sqldrivers)[*ClipMachineMemory->nsqldrivers - 1].desc_of_SQLDriver,
...
ib_createconn(ClipMachine * ClipMachineMemory)
{
   ...
   *dpb++ = isc_dpb_version1;
   ...
   if (!trpars)
      trpars = _clip_fetch_item(ClipMachineMemory, _clip_hashstr("IB_ISOLATION_LEVEL"));
   if (!trpars)
      trpars = _clip_fetch_item(ClipMachineMemory, _clip_hashstr("SQL_ISOLATION_LEVEL"));
   if (trpars)
      conn->tpb = _ib_tran_pars(trpars, &conn->tpblen);

   return _clip_store_c_item(ClipMachineMemory, (void *) conn, _C_ITEM_TYPE_SQL, destroy_ib_conn);
}
...
ib_prepare(ClipMachine * ClipMachineMemory, SQLCONN * c, char *sql)
{
   ...
   isc_dsql_prepare(conn->status, &conn->tr, &stmt->res, 0, stmt->parsed_sql, SQL_DIALECT_CURRENT, NULL);
   CHECKERR(conn->status, ER_INTERNAL, er_internal, (free(stmt->sql), free(stmt->parsed_sql), free(stmt), 0), -1);

   stmt->conn = conn;

   stmt->stmt_item = _clip_store_c_item(ClipMachineMemory, stmt, _C_ITEM_TYPE_SQL, destroy_ib_stmt);
...
ib_createrowset(ClipMachine * ClipMachineMemory, SQLROWSET * rs, ClipVar * ap, ClipVar * idname, const char *gen_idSQL)
{
   ...
   rowset->rowset_item = _clip_store_c_item(ClipMachineMemory, rowset, _C_ITEM_TYPE_SQL, destroy_ib_rowset);
...
void
ib_setvalue(SQLROWSET * rs, int fieldno, char *value, int len)
{
   IB_ROWSET *rowset = (IB_ROWSET *) rs;
...
int
clip_IB_OUT_INT64(ClipMachine * ClipMachineMemory)
{
   long long ll = (long long) _clip_parnd(ClipMachineMemory, 1);

   _clip_retcn(ClipMachineMemory, (char *) &ll, sizeof(long long));
   return 0;
}
...
clip_IB_OUT_TIME(ClipMachine * ClipMachineMemory)
{
   ...
   t *= 10;
   _clip_retcn(ClipMachineMemory, (char *) &t, sizeof(long));
   return 0;
}
...
clip_IB_OUT_DATETIME(ClipMachine * ClipMachineMemory)
{
   ...
   dt[0] -= 2400001;
   dt[1] *= 10;
   _clip_retcn(ClipMachineMemory, (char *) dt, 2 * sizeof(long));
   return 0;
}
