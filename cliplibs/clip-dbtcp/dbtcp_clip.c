...
void
dbtcp_bindpars(DBTCP_STMT * stmt, ClipVar * ap)
{
   char     *sql = stmt->sql;
   ...
   for (i = 0; i < ap->a.count; i++)
       ...
       while ((b = strstr(b, parnamebuf)))
	{
	   ...
	   if (e)
		...
		{
		   len += (vp->t.ClipVartype_type_of_ClipType == CHARACTER_t) ? strlen(vp->s.str.buf) : 4 - (e - b);
		}
	    }
	   else
	    {
	       len += (vp->t.ClipVartype_type_of_ClipType == CHARACTER_t) ? strlen(vp->s.str.buf) : 4 - (initlen - (b - sql));
	    }
	   b++;
	}
    }
   ...
   while ((e = strchr(b, ':')))
       ...
       for (i = 0; i < ap->a.count; i++)
	{
	   ...
	   vp = _clip_vptr(&tp->a.items[1]);
	   tp = _clip_vptr(&tp->a.items[0]);
	   if (vp->t.ClipVartype_type_of_ClipType == CHARACTER_t)
	    {
	       if (strncmp(e + 1, tp->s.str.buf, b - e - 1) == 0)
		{
		   if (b - e - 1 == strlen(tp->s.str.buf))
		      break;
		}
	    }
	}
...
int
clip_INIT_DBTCP(ClipMachine * mp)
{
   (*mp->nsqldrivers)++;
...
void
dbtcp_error(ClipMachine * cm, SQLLocale * loc, int code, dbftp_result * res, const char *str)
{
   char     *err, *c, *e;
...
void
dbtcp_destroyconn(SQLCONN * c)
{
}

int
dbtcp_prepare(ClipMachine * cm, SQLCONN * c, char *sql)
{
   DBTCP_CONN *conn = (DBTCP_CONN *) c;
   ...
   stmt->stmt_item = _clip_store_c_item(cm, stmt, _C_ITEM_TYPE_SQL, destroy_dbtcp_stmt);
...
int
dbtcp_command(ClipMachine * cm, SQLSTMT * s, ClipVar * ap)
{
   DBTCP_STMT *stmt = (DBTCP_STMT *) s;
   ...
   if (dbftp_sql(conn->conn, stmt->sql) != OK)
      goto err;
   return 0;
 err:
...
int
dbtcp_createrowset(ClipMachine * cm, SQLROWSET * rs, ClipVar * ap, ClipVar * idname, const char *gen_idSQL)
{
   ...
   dbtcp_bindpars(stmt, ap);
   rowset->rowset_item = _clip_store_c_item(cm, rowset, _C_ITEM_TYPE_SQL, destroy_dbtcp_rowset);
   ...
   if (dbftp_fetch_fields(conn->conn) != OK)
      goto err;
...
char     *
dbtcp_testparser(ClipMachine * mp, char *sql, ClipVar * ap)
{
   DBTCP_STMT stmt;
...
char     *
dbtcp_getvalue(SQLROWSET * rs, int fieldno, int *len)
{
   DBTCP_ROWSET *rowset = (DBTCP_ROWSET *) rs;
...
void
dbtcp_setvalue(SQLROWSET * rs, int fieldno, char *value, int len)
{
   DBTCP_ROWSET *rowset = (DBTCP_ROWSET *) rs;
...
void
dbtcp_newid(ClipMachine * cm, SQLSTMT * stmt)
{
}

int
dbtcp_refresh(ClipMachine * cm, SQLROWSET * rs, SQLSTMT * s, ClipVar * ap)
{
   DBTCP_ROWSET *rowset = (DBTCP_ROWSET *) rs;
...
int
dbtcp_fetch(ClipMachine * cm, SQLROWSET * rs, int recs, ClipVar * eval, int every, ClipVar * ors)
{
   DBTCP_ROWSET *rowset = (DBTCP_ROWSET *) rs;
...
int
dbtcp_start(ClipMachine * cm, SQLCONN * c, const char *p1, const char *p2)
{
   DBTCP_CONN *conn = (DBTCP_CONN *) c;
...
int
dbtcp_commit(ClipMachine * cm, SQLCONN * c)
{
   DBTCP_CONN *conn = (DBTCP_CONN *) c;
...
int
dbtcp_rollback(ClipMachine * cm, SQLCONN * c)
{
   DBTCP_CONN *conn = (DBTCP_CONN *) c;
