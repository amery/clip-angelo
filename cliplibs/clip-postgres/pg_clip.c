...
int
clip_INIT_POSTGRES(ClipMachine * mp)
{
   (*mp->nsqldrivers)++;
   *mp->sqldrivers = realloc(*mp->sqldrivers, sizeof(SQLDriver) * (*mp->nsqldrivers));
	strcpy((*mp->sqldrivers)[*mp->nsqldrivers - 1].id_of_SQLDriver, "PG");
	strcpy((*mp->sqldrivers)[*mp->nsqldrivers - 1].name_of_SQLDriver, "PostgreSQL");
	strcpy((*mp->sqldrivers)[*mp->nsqldrivers - 1].desc_of_SQLDriver, "Generic PostgreSQL for CLIP driver v.1.0");
   (*mp->sqldrivers)[*mp->nsqldrivers - 1].connect = pg_createconn;
   return 0;
}

static void
destroy_pg_stmt(void *s)
{
...
void
pg_bindpars(PG_STMT * stmt, ClipVar * ap)
{
   char *sql = stmt->sql;
...
ARRAY_type_of_ClipVarType))
	 ...
	 while ((b = strstr(b, parnamebuf)))
	    {
	       if (!(strchr(delims, *(b + tp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 1)) || !(*(b + tp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.len_of_ClipBuf + 1))))
		  {
		     b++;
		     continue;
		  }
	       ...
	       if (e)
		  {
			...
			{
			   len += (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType) ? strlen(vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf) : 4 - (e -
b);
			}
		  }
	       else
		  {
		     len += (vp->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType) ? strlen(vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf) : 4 -
(initlen - (b - sql));
		  }
	       b++;
	    }
...
pg_createconn(ClipMachine * mp)
{
   ...
   if (PQresultStatus(res) != PGRES_COMMAND_OK)
      {
	 _clip_trap_err(mp, 0, 0, 0, subsys, ER_BADSTATEMENT, PQresultErrorMessage(res));
	 return -1;
      }
...
void
pg_destroyconn(SQLCONN * conn)
{
   PQfinish(((PG_CONN *) conn)->conn);
}

int
pg_prepare(ClipMachine * mp, SQLCONN * conn, char *sql)
{
   PG_STMT *stmt;

   stmt = malloc(sizeof(PG_STMT));
   memset(stmt, 0, sizeof(PG_STMT));
   stmt->stmt_item = _clip_store_c_item(mp, stmt, _C_ITEM_TYPE_SQL, destroy_pg_stmt);
...
int
pg_command(ClipMachine * mp, SQLSTMT * stmt, ClipVar * ap)
{
   int status;
   const char *tups;

   pg_bindpars((PG_STMT *) stmt, ap);
   ((PG_STMT *) stmt)->res = PQexec(((PG_STMT *) stmt)->conn->conn, ((PG_STMT *) stmt)->sql);
   status = PQresultStatus(((PG_STMT *) stmt)->res);

   if (status != PGRES_COMMAND_OK)
      {
	 _clip_trap_err(mp, 0, 0, 0, subsys, ER_BADSTATEMENT, PQresultErrorMessage(((PG_STMT *) stmt)->res));
	 return -1;
      }
...
char *
pg_testparser(ClipMachine * mp, char *sql, ClipVar * ap)
{
   PG_STMT stmt;
...
int
pg_createrowset(ClipMachine * mp, SQLROWSET * rs, ClipVar * ap, ClipVar * idname, const char *gen_idSQL)
{
   PG_ROWSET *rowset = (PG_ROWSET *) rs;
   ...
   if (!stmt->res)
      {
	 _clip_trap_err(mp, 0, 0, 0, subsys, ER_BADSTATEMENT, PQresultErrorMessage(stmt->res));
	 return 1;
      }
   if (PQresultStatus(stmt->res) != PGRES_TUPLES_OK)
      {
	 _clip_trap_err(mp, 0, 0, 0, subsys, ER_BADSELECT, PQresultErrorMessage(stmt->res));
	 return 1;
      }

   rowset->rowset_item = _clip_store_c_item(mp, rowset, _C_ITEM_TYPE_SQL, destroy_pg_rowset);
   ...
   for (i = 0; i < rowset->nfields; i++)
      {
	 strncpy(rowset->fields[i].name, PQfname(stmt->res, i), MAXFIELDNAME);
	 ...
	 if (idname && idname->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType && idname->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf)
	    {
	       if (!strcasecmp(rowset->fields[i].name, idname->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf))
		  {
		     rowset->id = i;
		  }
	    }
...
int
pg_refresh(ClipMachine * mp, SQLROWSET * rs, SQLSTMT * s, ClipVar * ap)
{
   PG_ROWSET *rowset = (PG_ROWSET *) rs;
   PG_STMT *stmt = (PG_STMT *) s;
   int i, j, len;
...
char *
pg_getvalue(SQLROWSET * rowset, int fieldno, int *len)
{
   if (((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno])
      {
	 if (!((PG_ROWSET *) rowset)->binary)
	    {
	       *len = strlen((char *) ((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno]);
	       return (char *) ((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno];
	    }
	 else
	    {
	       *len = *((int *) (((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno]));
	       return (char *) (((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno]) + 4;
	    }
      }
   return NULL;
}
...
void
pg_setvalue(SQLROWSET * rowset, int fieldno, char *value, int len)
{
   if (((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno])
      {
	 free(((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno]);
      }
   if (value)
      {
	 if (((PG_ROWSET *) rowset)->binary)
	    {
	       ((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno] = malloc(len + 4);
	       *((int *) (((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno])) = len;
	       memcpy(((char *) (((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno])) + 4, value, len);
	    }
	 else
	    {
	       ((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno] = malloc(len + 1);
	       memcpy(((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno], value, len);
	       ((char *) (((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno]))[len] = 0;
	    }
      }
   else
      {
	 ((PG_ROWSET *) rowset)->data[((PG_ROWSET *) rowset)->recno - 1][fieldno] = NULL;
      }
}

void
pg_append(SQLROWSET * rowset)
{
   void **row;
   int len;
   ...
   ((PG_ROWSET *) rowset)->data = realloc(((PG_ROWSET *) rowset)->data, sizeof(void *) * ((PG_ROWSET *) rowset)->lastrec);
   row = malloc(len);
   memset(row, 0, len);
...
pg_delete(SQLROWSET * rowset)
{
   ...
   ((PG_ROWSET *) rowset)->data = realloc(((PG_ROWSET *) rowset)->data, sizeof(void *) * (((PG_ROWSET *) rowset)->lastrec - 1));
}

void
pg_newid(ClipMachine * mp, SQLSTMT * stmt)
{
...
int
clip_PG_ISBINARY(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
...
int
clip_PG_IN_INT4(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   char *str;
   int len;
...
int
clip_PG_IN_INT2(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   char *str;
   int len;
...
int
clip_PG_IN_INT8(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   char *str;
   int len;
...
int
clip_PG_IN_FLOAT(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
...
int
clip_PG_IN_DOUBLE(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   int dec = _clip_parni(mp, 3);
   char *str;
...
int
clip_PG_IN_ABSTIME(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   char *date;
   int len;
...
int
clip_PG_IN_TIMESTAMP(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   char *date;
   int len;
...
int
clip_PG_IN_DATE(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   char *date;
   int len;
...
void
_pg_in_iso_datetime(char *date, int l, long *ddd, long *ttt)
{
   int m, d, y;
   long dd, tt;
   int z;
...
int
clip_PG_IN_DATETIME(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   char *date;
   long dd, tt;
...
int
clip_PG_OUT_ABSTIME(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
...
int
clip_PG_OUT_TIMESTAMP(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   int totext = _clip_parl(mp, 3);
   struct tm time;
...
int
clip_PG_OUT_INT4(ClipMachine * mp)
{
...
int
clip_PG_OUT_INT2(ClipMachine * mp)
{
...
int
clip_PG_OUT_INT8(ClipMachine * mp)
{
...
int
clip_PG_OUT_DOUBLE(ClipMachine * mp)
{
   double d = _clip_parnd(mp, 1);
...
int
clip_PG_OUT_FLOAT(ClipMachine * mp)
{
...
int
clip_PG_OUT_DATE(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
...
int
clip_PG_OUT_DATETIME(ClipMachine * mp)
{
   PG_ROWSET *rowset = (PG_ROWSET *) _clip_fetch_c_item(mp, _clip_parni(mp, 1), _C_ITEM_TYPE_SQL);
   long time;
...
int
pg_fetch(ClipMachine * mp, SQLROWSET * rs, int recs, ClipVar * eval, int every, ClipVar * ors)
{
   PG_ROWSET *rowset = (PG_ROWSET *) rs;
   PG_STMT *stmt = rowset->stmt;
   int i, j, er = 0, len;
	 ...
	 for (j = 0; j < rowset->nfields; j++)
	    {
	       if (!PQgetisnull(stmt->res, rowset->loaded, j))
		  {
		     ...
		     if (rowset->binary)
			{
			   rec[j] = malloc(len + 4);
			   *((int *) (rec[j])) = len;
			   memcpy(((char *) rec[j]) + 4, PQgetvalue(stmt->res, rowset->loaded, j), len);
			}
		     else
			{
			   rec[j] = malloc(len + 1);
...
int
pg_start(ClipMachine * mp, SQLCONN * c, const char *p1, const char *p2)
{
   ...
   if (PQresultStatus(res) != PGRES_COMMAND_OK)
      {
	 _clip_trap_err(mp, 0, 0, 0, subsys, ER_START, PQresultErrorMessage(res));
	 return 1;
      }
   if (p1)
      {
...
int
pg_commit(ClipMachine * mp, SQLCONN * c)
{
   PG_CONN *conn = (PG_CONN *) c;
   PGresult *res;
   ...
   if (PQresultStatus(res) != PGRES_COMMAND_OK)
      {
	 _clip_trap_err(mp, 0, 0, 0, subsys, ER_COMMIT, PQresultErrorMessage(res));
	 return 1;
      }
   conn->at = 0;
   return 0;
...
int
pg_rollback(ClipMachine * mp, SQLCONN * c)
{
   PG_CONN *conn = (PG_CONN *) c;
   PGresult *res;
   ...
   if (PQresultStatus(res) != PGRES_COMMAND_OK)
      {
	 _clip_trap_err(mp, 0, 0, 0, subsys, ER_ROLLBACK, PQresultErrorMessage(res));
	 return 1;
      }
   conn->at = 0;
   return 0;
...
/* ************************************************************************* */
int
pg_lo_create(ClipMachine * mp, SQLCONN * c, unsigned int OID)
{
   PG_CONN *conn = (PG_CONN *) c;
   Oid lobjId;
...
/* ************************************************************************* */
int
pg_lo_open(ClipMachine * mp, SQLCONN * c, unsigned int OID, int mode)
{
   PG_CONN *conn = (PG_CONN *) c;
   int oid_fd;
   int lo_mode = 0;
...
/* ************************************************************************* */
int
pg_lo_import(ClipMachine * mp, SQLCONN * c, const char *filename)
{
   PG_CONN *conn = (PG_CONN *) c;
   Oid lobjId;
...
/* ************************************************************************* */
int
pg_lo_export(ClipMachine * mp, SQLCONN * c, unsigned int OID, const char *filename)
{
   PG_CONN *conn = (PG_CONN *) c;
...
/* ************************************************************************* */
int
pg_lo_write(ClipMachine * mp, SQLCONN * c, int oid_fd, const char *buffer, int length)
{
   PG_CONN *conn = (PG_CONN *) c;
   int rt;
...
/* ************************************************************************* */
int
pg_lo_read(ClipMachine * mp, SQLCONN * c, int oid_fd, char *buffer, int length)
{
   PG_CONN *conn = (PG_CONN *) c;
   int rt;
...
/* ************************************************************************* */
int
pg_lo_seek(ClipMachine * mp, SQLCONN * c, int oid_fd, int offset, int whence)
{
   PG_CONN *conn = (PG_CONN *) c;
   int rt;
...
/* ************************************************************************* */
int
pg_lo_tell(ClipMachine * mp, SQLCONN * c, int oid_fd)
{
   PG_CONN *conn = (PG_CONN *) c;
   int rt;
...
/* ************************************************************************* */
int
pg_lo_close(ClipMachine * mp, SQLCONN * c, int oid_fd)
{
   PG_CONN *conn = (PG_CONN *) c;
   int rt;
...
/* ************************************************************************* */
int
pg_lo_unlink(ClipMachine * mp, SQLCONN * c, unsigned int OID)
{
   PG_CONN *conn = (PG_CONN *) c;
   int rt;
   if (!conn->at)
