...
void      gtk_extext_get_text_data_foreach_line(GtkExText * text,
						GtkExTextLineData * lp, TextDataCallback st, gpointer user_data);
...
guchar   *text_get_text_with_expanded_tabs(GtkExText * text, gint linestart,
					   gint start, gint end, gint * rtabs, gint * rspaces);
...
gint      draw_text_chunk(GtkExText * text, GdkDrawable * draw_area,
			  GtkExTextStyle * style, gchar * chunk, gint length, gint x, gint y, gint h);
   ...
   if (text->highlight)
   ...
   else
    {
       gtk_signal_emit(GTK_OBJECT(text),
		       extext_signals[PROPERTY_TEXT_INSERT], property_first, pos, pos + len, &signal_return_value);
...
text_remove(GtkExText * text, gint pos, gint len)
{
   ...
   if (text->highlight)
    ...
    {
       gtk_signal_emit(GTK_OBJECT(text),
		       extext_signals[PROPERTY_TEXT_REMOVE], property_first, pos, pos + len, &signal_return_value);
       if (!signal_return_value)
  ...
  /* the same */
   else if (prop_end && prop_end->endpos >= endpos)
    {
       ...
       gtk_extext_property_insert(text, prop->style->key, endpos,
				  prop_end->endpos, prop_end->user_data, PROPERTY_INSERT, property_first);
...
gtk_extext_property_insert(GtkExText * text,
			   gchar * key, gint startpos, gint endpos, gpointer data, gint type, GtkExTextProperty * prev)
{
   gint      diff;
...
gtk_extext_class_init(GtkExTextClass * _class)
{
   ...
   extext_signals[INSERT_TEXT] =
    gtk_signal_new("insert_text", GTK_RUN_LAST, object_class->type,
		   ...
		   gtk_marshal_NONE__POINTER_INT_POINTER, GTK_TYPE_NONE, 3, GTK_TYPE_POINTER, GTK_TYPE_INT, GTK_TYPE_POINTER);
   ...
   extext_signals[CHANGED] =
    gtk_signal_new("changed", GTK_RUN_LAST, object_class->type,
		   GTK_SIGNAL_OFFSET(GtkExTextClass, changed), gtk_marshal_NONE__NONE, GTK_TYPE_NONE, 0);

   extext_signals[MOVE_TO_ROW] =
    gtk_signal_new("move_to_row", GTK_RUN_LAST, object_class->type,
		   GTK_SIGNAL_OFFSET(GtkExTextClass, move_to_row), gtk_marshal_NONE__INT, GTK_TYPE_NONE, 1, GTK_TYPE_INT);

   extext_signals[MOVE_TO_COLUMN] =
    gtk_signal_new("move_to_column", GTK_RUN_LAST, object_class->type,
		   GTK_SIGNAL_OFFSET(GtkExTextClass, move_to_column), gtk_marshal_NONE__INT, GTK_TYPE_NONE, 1, GTK_TYPE_INT);

   extext_signals[CUT_CLIPBOARD] =
    gtk_signal_new("cut_clipboard", GTK_RUN_LAST, object_class->type,
		   GTK_SIGNAL_OFFSET(GtkExTextClass, cut_clipboard), gtk_marshal_NONE__NONE, GTK_TYPE_NONE, 0);

   extext_signals[COPY_CLIPBOARD] =
    gtk_signal_new("copy_clipboard", GTK_RUN_LAST, object_class->type,
		   GTK_SIGNAL_OFFSET(GtkExTextClass, copy_clipboard), gtk_marshal_NONE__NONE, GTK_TYPE_NONE, 0);

   extext_signals[PASTE_CLIPBOARD] =
    gtk_signal_new("paste_clipboard", GTK_RUN_LAST, object_class->type,
		   GTK_SIGNAL_OFFSET(GtkExTextClass, paste_clipboard), gtk_marshal_NONE__NONE, GTK_TYPE_NONE, 0);
   ...
   extext_signals[PROPERTY_TEXT_INSERT] =
    gtk_signal_new("property_text_insert", GTK_RUN_LAST,
		   object_class->type, GTK_SIGNAL_OFFSET(GtkExTextClass,
							 property_text_insert),
		   gtk_marshal_BOOL__POINTER_INT_INT, GTK_TYPE_BOOL, 3, GTK_TYPE_POINTER, GTK_TYPE_INT, GTK_TYPE_INT);
   ...
   extext_signals[UNDO_CHANGED] = gtk_signal_new("undo_changed",
						 ...
						 object_class->type,
						 GTK_SIGNAL_OFFSET
						 (GtkExTextClass, undo_changed), gtk_marshal_NONE__NONE, GTK_TYPE_NONE, 0);
  ...
  /* called if cursor has moved to a new property */
   ...
   extext_signals[MARGIN_CLICKED] = gtk_signal_new("margin_enter",
						   ...
						   GTK_SIGNAL_OFFSET
						   (GtkExTextClass,
						    margin_enter), gtk_marshal_NONE__INT, GTK_TYPE_NONE, 1, GTK_TYPE_INT);
   ...
   extext_signals[MARGIN_CLICKED] = gtk_signal_new("margin_exit",
						   ...
						   GTK_SIGNAL_OFFSET
						   (GtkExTextClass,
						    margin_exit), gtk_marshal_NONE__INT, GTK_TYPE_NONE, 1, GTK_TYPE_INT);
   ...
   if (GTK_WIDGET_DRAWABLE(widget))
    {
       ...
       gtk_paint_shadow(widget->style, widget->window,
			GTK_STATE_NORMAL, GTK_SHADOW_IN, NULL, widget, "text", 1, 1, width, height);
    }
}
   ...
   if (GTK_WIDGET_REALIZED(widget))
    {
       ...
       gdk_window_move_resize(text->text_area,
				  ...
				  (TEXT_BORDER_ROOM * 2)), MAX(1, (gint) allocation->height - (TEXT_BORDER_ROOM * 2)));
...
gtk_extext_realize(GtkWidget * widget)
{
   ...
   attributes.event_mask |= (GDK_EXPOSURE_MASK |
			     ...
			     GDK_POINTER_MOTION_MASK | GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_KEY_PRESS_MASK);
   ...
   if (gdk_im_ready() && (text->ic_attr = gdk_ic_attr_new()) != NULL)
	...
	GDK_IM_PREEDIT_NOTHING | GDK_IM_PREEDIT_POSITION | GDK_IM_STATUS_NONE | GDK_IM_STATUS_NOTHING;
...
gtk_extext_style_set(GtkWidget * widget, GtkStyle * old)
{
   ...
   gtk_extext_style_insert(text, "Default", GTK_WIDGET(text)->style->font,
			   &GTK_WIDGET(text)->style->text[GTK_STATE_NORMAL], NULL, 0);
   gtk_extext_set_draw_numbers(text, text->draw_numbers);
   text->gutter_width = text->number_width + text->pixmap_max_width;
}
   ...
   if (area->x == 0 && area->y == 0 && area->width >= w && area->height >= h)
       ...
       if (text->gutter_width)
	{
	   gdk_draw_line(draw, GTK_WIDGET(text)->style->black_gc,
			 text->gutter_width - TEXT_BORDER_ROOM, 0, text->gutter_width - TEXT_BORDER_ROOM, h);
	}
    }
   ...
   if (text->draw_numbers)
    {
       ...
       gdk_draw_text(draw_area, default_style->font, text->gc,
		     text->number_width - (textwidth + TEXT_BORDER_ROOM), y + (max_height - 3), numbers, strlen(numbers));
    }
   if (text->draw_pixmaps && linedataptr->lineptr && linedataptr->lineptr->pixmap)
       ...
       if (linedataptr->lineptr->mask)
	{
	   ...
	   gdk_gc_set_clip_origin(GTK_WIDGET(text)->style->black_gc,
				  text->gutter_width - text->pixmap_max_width + TEXT_BORDER_ROOM, y + TEXT_BORDER_ROOM + 1);
	}
       if (draw_area)
	  gdk_draw_pixmap(draw_area, GTK_WIDGET(text)->style->black_gc,
			  linedataptr->lineptr->pixmap, 0, 0,
			  text->gutter_width - text->pixmap_max_width + TEXT_BORDER_ROOM, y + TEXT_BORDER_ROOM + 1, -1, -1);
       if (linedataptr->lineptr->mask)
	{
...
find_cursor(GtkExText * text, gdouble x, gdouble y, gboolean select)
{
   ...
   if (y < 0)
   ...
   else
    {
	   ...
	   && (text->scroll_line_start->line_number + text->scroll_line_count + 1 <= text->line_count))
	{
	   gint      line = 0;
...
gtk_extext_insert_with_style(GtkExText * text, const gchar * chars, gint length, gchar * stylekey, gpointer userdata)
{
   ...
   if (stylekey)
      gtk_extext_property_insert(text, stylekey, pos - length, pos, userdata,
				 PROPERTY_MERGE, text->scroll_line_start->property_first);
}
...
gtk_extext_selection_received(GtkWidget * widget, GtkSelectionData * selection_data, guint time)
{
   ...
   if ((text->selection_start_pos != text->selection_end_pos) &&
       (!text->has_selection || (selection_data->selection == clipboard_atom)))
    {
       reselect = TRUE;
       ...
       if (text->has_selection)
	{
	   gtk_extext_delete_text(text,
				  MIN(text->selection_start_pos,
				      text->selection_end_pos), MAX(text->selection_start_pos, text->selection_end_pos));
	}
       else
   ...
   switch (type)
    ...
    case CTEXT:
	  ...
	  count = gdk_text_property_to_text_list(selection_data->type,
						 selection_data->format, selection_data->data, selection_data->length, &list);
...
gtk_extext_syntax_entry_new(GList * entries, gchar * name, gchar * start,
			    gchar * end, GdkColor * fg, GdkColor * bg, GdkFont * font, guint16 flags, gpointer data)
{
   GtkExTextSyntaxEntry *syntax;
...
gtk_extext_pattern_entry_new(GList * entries, gchar * name, gchar * start,
			     GdkColor * fg, GdkColor * bg, GdkFont * font, guint16 flags, gpointer data)
{
...
gtk_extext_embedded_entry_new(GList * entries, gchar * name, gchar * outside,
			      gchar * inside, GdkColor * fg, GdkColor * bg, GdkFont * font, guint16 flags, gpointer data)
{
...
property_text_remove(GtkExText * text, GtkExTextProperty * prop, gint start, gint end)
{
  ...
  /* If the whole text chunk was inside a highlight, parse from prop->start to prop->end */
   if ((prop = gtk_extext_property_get_at_pos(text, start, prop))
       && ((gint) prop->startpos <= start) && ((gint) (prop->endpos - diff) > start))
    {
       start = prop->startpos;
...
property_text_insert(GtkExText * text, GtkExTextProperty * prop, gint start, gint end)
{
   ...
   diff = end - start;
   if ((prop = gtk_extext_property_get_at_pos(text, start, prop))
       && ((gint) prop->startpos < start) && ((gint) (prop->endpos + diff) >= end))
    {
       start = prop->startpos;
	       ...
	       for (j = i; j < (i + len); j++)
		{
		   if ((mlen = gtk_extext_regex_match(text, j, &embedded->inside)) > 0 && (j + mlen) <= (i + len))
			...
			gtk_extext_property_insert(text, embedded->key, j,
						   j + mlen, GINT_TO_POINTER(EMBEDDED_TABLE), PROPERTY_REMOVEALL, prev);
	   ...
	   if (gtk_extext_regex_match(text, s, &sentry->regex_start) > 0 && GTK_EXTEXT_INDEX(text, s - 1) != '\\')
	    {
		  ...
		  i = text->length;
	       prev =
		gtk_extext_property_insert(text, sentry->key, s, i, GINT_TO_POINTER(SYNTAX_TABLE), PROPERTY_REMOVEALL, prev);
	       found = TRUE;
	       break;
	    }
       ...
       for (list = table->entries; list; list = list->next)
	{
	   ...
	   if ((len = gtk_extext_regex_match(text, i, &pentry->regex)) > 0 && end >= i + len)
		...
		gtk_extext_property_insert(text, pentry->key, i, i + len,
					   GINT_TO_POINTER(PATTERN_TABLE), PROPERTY_REMOVEALL, prev);
