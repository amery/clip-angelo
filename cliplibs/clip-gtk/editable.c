...
int
clip_GTK_EDITABLECLAIMSELECTION(ClipMachine * ClipMachineMemory)
{
   C_widget *cedt = _fetch_cw_arg(ClipMachineMemory);
   gboolean claim = _clip_parl(ClipMachineMemory, 2);
   gint32 time = _clip_parni(ClipMachineMemory, 3);
   CHECKCWID(cedt, GTK_IS_EDITABLE);
...
clip_GTK_EDITABLEHASSELECTION(ClipMachine * ClipMachineMemory)
{
   C_widget *cedt = _fetch_cw_arg(ClipMachineMemory);
   CHECKCWID(cedt, GTK_IS_EDITABLE);
   _clip_retl(ClipMachineMemory, GTK_EDITABLE(cedt->widget)->has_selection);
   return 0;
 err:
   return 1;
}
...
int
clip_GTK_EDITABLECHANGED(ClipMachine * ClipMachineMemory)
{
   C_widget *cedt = _fetch_cw_arg(ClipMachineMemory);
   CHECKCWID(cedt, GTK_IS_EDITABLE);
...
clip_GTK_EDITABLEGETPOSITION(ClipMachine * ClipMachineMemory)
{
   C_widget *cedt = _fetch_cw_arg(ClipMachineMemory);
   CHECKCWID(cedt, GTK_IS_EDITABLE);
   _clip_retni(ClipMachineMemory, gtk_editable_get_position(GTK_EDITABLE(cedt->widget)));
   return 0;
 err:
   return 1;
}
