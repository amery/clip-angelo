...
_gtk_type_progress()
{
   return GTK_TYPE_PROGRESS;
}

long
_clip_type_progress()
{
   return GTK_WIDGET_PROGRESS;
}

const char *
_clip_type_name_progress()
{
   return "GTK_WIDGET_PROGRESS";
}

int
clip_INIT___PROGRESS(ClipMachine * ClipMachineMemory)
{
...
clip_GTK_PROGRESSSETTEXTALIGNMENT(ClipMachine * ClipMachineMemory)
{
   ...
   if (_clip_parinfo(ClipMachineMemory, 2) == UNDEF_type_of_ClipVarType)
      x_align = GTK_PROGRESS(cprg->widget)->x_align;
   if (_clip_parinfo(ClipMachineMemory, 3) == UNDEF_type_of_ClipVarType)
      y_align = GTK_PROGRESS(cprg->widget)->y_align;
