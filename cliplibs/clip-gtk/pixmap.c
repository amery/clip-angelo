...
/* forwards */
static gchar **_load_xpm_from_bmp(ClipMachine * ClipMachineMemory, char *filename, int *ncolors, unsigned long *rows, unsigned long *cols);
...
clip_GDK_PIXMAPCOLORMAPCREATEFROMXPM(ClipMachine * ClipMachineMemory)
{
   ...
   CHECKCOBJOPT(cwin, GDK_IS_WINDOW(cwin));
   CHECKOPT2(2, MAP_type_of_ClipVarType, NUMERIC_type_of_ClipVarType);
   CHECKCOBJOPT(cclrmap, GDK_IS_COLORMAP(cclrmap));
