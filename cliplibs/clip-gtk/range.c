...
static SignalTable range_signals[] = {
   ...
   {"trough-keys", GSF(handle_trough_keys_signal), ESF(object_emit_signal),
    GTK_TROUGH_KEYS_SIGNAL},
...
*/
int
clip_GTK_RANGEGETOLDVALUE(ClipMachine * ClipMachineMemory)
{
   C_widget *crange = _fetch_cw_arg(ClipMachineMemory);

   CHECKCWID(crange, GTK_IS_RANGE);

   _clip_storni(ClipMachineMemory, GTK_RANGE(crange->widget)->old_value, 2, 0);
   _clip_storni(ClipMachineMemory, GTK_RANGE(crange->widget)->old_lower, 3, 0);
   _clip_storni(ClipMachineMemory, GTK_RANGE(crange->widget)->old_upper, 4, 0);
