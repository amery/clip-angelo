...
/* Signal handlers */
gint
handle_destroy_signal(GtkObject * wid, C_signal * cs)
{
...
_clip_type_object()
{
   return GTK_WIDGET_OBJECT;
}

const char *
_clip_type_name_object()
{
   return "GTK_WIDGET_OBJECT";
}

int
clip_INIT___OBJECT(ClipMachine * ClipMachineMemory)
{
...
/*printf("Widget name: %s, event: %d \n",gtk_widget_get_name(widget), event->type);*/
   ...
   if (!cs->cw)
      return ret;
   if (event->type != cs->sigid && cs->sigid != GTK_EVENT)
      return ret;

   ClipMachineMemory = cs->cw->cmachine;
   memset(stack, 0, sizeof(stack));
  //stack[0] = cs->cw->obj;
   _clip_mclone(cs->cw->cmachine, &stack[0], &cs->cw->obj);

   if (!cs->cw->evntenabled)
      return FALSE;
...
/*printf("Widget name: %s, event: GDK_CONFIGURE \n",gtk_widget_get_name(widget));*/
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_X, event->configure.x);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_Y, event->configure.y);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_WIDTH, event->configure.width);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_HEIGHT, event->configure.height);
       break;
    case GDK_EXPOSE:
       {
	  ClipVar   area;
	  ...
	  _clip_map(ClipMachineMemory, &area);

/*printf("Widget name: %s, event: GDK_EXPOSE \n",gtk_widget_get_name(widget));*/
	  _clip_mputn(ClipMachineMemory, &area, HASH_X, event->expose.area.x);
	  _clip_mputn(ClipMachineMemory, &area, HASH_Y, event->expose.area.y);
	  _clip_mputn(ClipMachineMemory, &area, HASH_WIDTH, event->expose.area.width);
	  _clip_mputn(ClipMachineMemory, &area, HASH_HEIGHT, event->expose.area.height);

	  _clip_madd(ClipMachineMemory, &stack[1], HASH_AREA, &area);
	  _clip_mputn(ClipMachineMemory, &stack[1], HASH_COUNT, event->expose.count);
	  break;
       }
    ...
    case GDK_3BUTTON_PRESS:
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_BUTTON, event->button.button);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_X, event->button.x);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_Y, event->button.y);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_XROOT, event->button.x_root);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_YROOT, event->button.y_root);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_PRESSURE, event->button.pressure);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_XTILT, event->button.xtilt);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_YTILT, event->button.ytilt);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_STATE, event->button.state);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_SOURCE, event->button.source);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_TYPE, event->button.type);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_TIME, event->button.time);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_DEVICIED, event->button.deviceid);
       break;
    case GDK_MOTION_NOTIFY:
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_X, event->motion.x);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_Y, event->motion.y);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_XROOT, event->motion.x_root);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_YROOT, event->motion.y_root);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_PRESSURE, event->motion.pressure);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_XTILT, event->motion.xtilt);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_YTILT, event->motion.ytilt);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_SOURCE, event->motion.source);
       _clip_mputn(ClipMachineMemory, &stack[1], HASH_DEVICIED, event->motion.deviceid);
	...
	{
	   _clip_mputn(ClipMachineMemory, &stack[1], HASH_KEYVAL, keycode);
	   _clip_mputn(ClipMachineMemory, &stack[1], HASH_STATE, event->key.state);
	   _clip_mputn(ClipMachineMemory, &stack[1], HASH_LENGTH, event->key.length);
	   _clip_mputc(ClipMachineMemory, &stack[1], HASH_STRING, event->key.string, event->key.length);
	}
       break;
    default:
       break;
...
_sig_name_by_id(long id)
{
   ...
   for (wt_item = _wtype_table_get_first(); wt_item; wt_item = wt_item->next)
       ...
       for (sig_table = wt_item->signal_table; sig_table && sig_table->sigfunction; sig_table++)
	{
       ...
       for (extra_sig_table = wt_item->extra_signal_table;
	    extra_sig_table && extra_sig_table->sigfunction; extra_sig_table = extra_sig_table->next)
	{
	   if (extra_sig_table->sigid == id)
...
static int
_signal_connect(ClipMachine * ClipMachineMemory, gboolean after)
{
   ...
   if (_clip_parinfo(ClipMachineMemory, 2) == CHARACTER_type_of_ClipVarType)
      sig_table = _sig_table_by_name(cwid, _clip_parc(ClipMachineMemory, 2));
   else
      sig_table = _sig_table_by_id(cwid, _clip_parni(ClipMachineMemory, 2));
   if (sig_table && sig_table->sigfunction)
       ...
       if (sigfound || extra_sigfound || sid < 1000)	/* sid<1000 - event */
	{
	   ...
	   _clip_mclone(ClipMachineMemory, &cs->cfunc, _clip_spar(ClipMachineMemory, 3));
	}

       if ((sid == GTK_BUTTON_PRESS || sid == GTK_2BUTTON_PRESS || sid == GTK_3BUTTON_PRESS))
	{
	  //cwid->event_connected = TRUE;
	   if (after)
	      ret = gtk_signal_connect_after(GTK_OBJECT(cwid->widget), "button-press-event", GSF(sfunc), cs);
	   else
	      ret = gtk_signal_connect(GTK_OBJECT(cwid->widget), "button-press-event", GSF(sfunc), cs);
	}
       if (sigfound && sid != GTK_BUTTON_PRESS)
	{
	   if (after)
	      ret = gtk_signal_connect_after(GTK_OBJECT(cwid->widget), signame, GSF(sfunc), cs);
	   else
	    {
	       ret = gtk_signal_connect(GTK_OBJECT(cwid->widget), signame, GSF(sfunc), cs);
	    }
	}
...
int
clip_GTK_SIGNALCONNECT(ClipMachine * ClipMachineMemory)
{
   return _signal_connect(ClipMachineMemory, FALSE);
}

int
clip_GTK_SIGNALCONNECTAFTER(ClipMachine * ClipMachineMemory)
{
   return _signal_connect(ClipMachineMemory, TRUE);
}

int
clip_GTK_SIGNALDISCONNECT(ClipMachine * ClipMachineMemory)
{
   C_widget *cwid = _fetch_cw_arg(ClipMachineMemory);

   int       conn_id = _clip_parni(ClipMachineMemory, 2);
...
int
clip_GTK_SIGNALHANDLERBLOCK(ClipMachine * ClipMachineMemory)
{
   C_widget *cwid = _fetch_cw_arg(ClipMachineMemory);
...
int
clip_GTK_SIGNALHANDLERUNBLOCK(ClipMachine * ClipMachineMemory)
{
   C_widget *cwid = _fetch_cw_arg(ClipMachineMemory);
...
int
clip_GTK_SIGNALHANDLERSDESTROY(ClipMachine * ClipMachineMemory)
{
   C_widget *cwid = _fetch_cw_arg(ClipMachineMemory);
...
int
clip_GTK_SIGNALNEW(ClipMachine * ClipMachineMemory)
{
   long      type = _clip_parnl(ClipMachineMemory, 1);

   char     *sig_name = _clip_parc(ClipMachineMemory, 2);
...
int
clip_GTK_SIGNALEMIT(ClipMachine * ClipMachineMemory)
{
   C_widget *cwid = _fetch_cw_arg(ClipMachineMemory);
   ...
   _clip_retl(ClipMachineMemory, sig_table && sig_table->sigfunction);
   return ret;
 err:
   return 1;
}
...
object_emit_event(C_widget * cwid, const gchar * signal_name)
{
   ...
   if (cvev && cvev->ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == MAP_type_of_ClipVarType)
    {
       double    d;
	...
	case GDK_2BUTTON_PRESS:
	case GDK_3BUTTON_PRESS:
	   _clip_mgetn(cwid->cmachine, cvev, HASH_BUTTON, &d);
	   ev.button.button = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_X, &d);
	   ev.button.x = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_Y, &d);
	   ev.button.y = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_XROOT, &d);
	   ev.button.x_root = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_YROOT, &d);
	   ev.button.y_root = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_PRESSURE, &d);
	   ev.button.pressure = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_XTILT, &d);
	   ev.button.xtilt = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_YTILT, &d);
	   ev.button.ytilt = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_STATE, &d);
	   ev.button.state = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_SOURCE, &d);
	   ev.button.source = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_DEVICIED, &d);
	   ev.button.deviceid = (guint) d;
	   break;
	case GDK_KEY_PRESS:
	case GDK_KEY_RELEASE:
	   _clip_mgetn(cwid->cmachine, cvev, HASH_KEYVAL, &d);
	   ev.key.keyval = (guint) d;
	   _clip_mgetn(cwid->cmachine, cvev, HASH_STATE, &d);
	   ev.key.state = (guint) d;
	   _clip_mgetc(cwid->cmachine, cvev, HASH_STRING, &ev.key.string, &ev.key.length);
	   _clip_mgetn(cwid->cmachine, cvev, HASH_LENGTH, &d);
	   ev.key.length = (guint) d;
	   break;
	default:
	   break;
	}
...
int
clip_GTK_SIGNALEMITSTOP(ClipMachine * ClipMachineMemory)
{
   C_widget *cwid = _fetch_cw_arg(ClipMachineMemory);
...
int
clip_GTK_SIGNALEMITSTOPBYNAME(ClipMachine * ClipMachineMemory)
{
   C_widget *cwid = _fetch_cw_arg(ClipMachineMemory);
...
int
clip_GTK_GETOBJBYHANDLE(ClipMachine * ClipMachineMemory)
{
   long      handle = _clip_parnl(ClipMachineMemory, 1);
   ...
   CHECKARG(1, NUMERIC_type_of_ClipVarType);
   cwid = _fetch_cwidgetn(ClipMachineMemory, handle);
   if (cwid)
      _clip_mclone(ClipMachineMemory, RETPTR(ClipMachineMemory), &cwid->obj);
   else
    {
       cobj = _fetch_cobjectn(ClipMachineMemory, handle);
       if (cobj)
	{
...
int
clip_GDK_SETSHOWEVENTS(ClipMachine * ClipMachineMemory)
{
   gint      show_events = BOOL_OPTION(ClipMachineMemory, 1, TRUE);
