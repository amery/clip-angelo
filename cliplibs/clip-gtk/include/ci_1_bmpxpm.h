...
void BMPreadfileheader (FILE * fp, unsigned long *ppos,
			unsigned long *poffBits);
void BMPreadinfoheader (FILE * fp, unsigned long *ppos, unsigned long *pcx,
			unsigned long *pcy, unsigned short *pcBitCount,
			int *pclass);
int BMPreadrgbtable (FILE * fp, unsigned long *ppos, unsigned short cBitCount,
		     int class, pixval * R, pixval * G, pixval * B);
int BMPreadrow (FILE * fp, unsigned long *ppos, lpixel * row,
		unsigned long cx, unsigned short cBitCount, pixval * R,
		pixval * G, pixval * B);
lpixel **BMPreadbits (FILE * fp, unsigned long *ppos, unsigned long offBits,
		      unsigned long cx, unsigned long cy,
		      unsigned short cBitCount, int class, pixval * R,
		      pixval * G, pixval * B);
...
unsigned long BMPlenbits (int class, unsigned long bitcount, unsigned long x,
			  unsigned long y);
unsigned long BMPlenfile (int class, unsigned long bitcount, unsigned long x,
			  unsigned long y);
...
colorhist_vector ppm_computecolorhist (lpixel ** pixels, int cols, int rows,
...
colorhash_table ppm_computecolorhash (lpixel ** pixels, int cols, int rows,
				      int maxcolors, int *colorsP);
...
typedef struct
{				/* rgb values and ascii names (from
				 * rgb text file) */
...
void gen_cmap (colorhist_vector, int, pixval, int, rgb_names *, int,
	       cixel_map *, int *);
