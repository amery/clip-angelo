...
int
clip_INIT_MYSQL(ClipMachine * mp)
{
   (*mp->nsqldrivers)++;
   *mp->sqldrivers = realloc(*mp->sqldrivers, sizeof(SQLDriver) * (*mp->nsqldrivers));
	strcpy((*mp->sqldrivers)[*mp->nsqldrivers - 1].id_of_SQLDriver, "MS");
	strcpy((*mp->sqldrivers)[*mp->nsqldrivers - 1].name_of_SQLDriver, "MySQL");
	strcpy((*mp->sqldrivers)[*mp->nsqldrivers - 1].desc_of_SQLDriver, "Generic MySQL for CLIP driver, v.1.0");
   (*mp->sqldrivers)[*mp->nsqldrivers - 1].connect = ms_createconn;
   return 0;
}

static void
destroy_ms_stmt(void *s)
{
...
int
ms_prepare(ClipMachine * mp, SQLCONN * c, char *sql)
{
   ...
   stmt->stmt_item = _clip_store_c_item(mp, stmt, _C_ITEM_TYPE_SQL, destroy_ms_stmt);
...
int
ms_command(ClipMachine * mp, SQLSTMT * s, ClipVar * ap)
{
   MS_STMT *stmt = (MS_STMT *) s;
   ...
   if ((rows = mysql_affected_rows(conn->conn)) == -1)
      {
	 _clip_trap_err(mp, 0, 0, 0, subsys, ER_BADSTATEMENT, "Unable to determine amount of affected rows");
	 return -1;
      }
   return rows;
}
...
char *
ms_testparser(ClipMachine * mp, char *sql, ClipVar * ap)
{
   MS_STMT stmt;
...
int
ms_createrowset(ClipMachine * mp, SQLROWSET * rs, ClipVar * ap, ClipVar * idname, const char *gen_idSQL)
{
   ...
   ms_bindpars(stmt, ap);
   rowset->rowset_item = _clip_store_c_item(mp, rowset, _C_ITEM_TYPE_SQL, destroy_ms_rowset);
...
int
ms_refresh(ClipMachine * mp, SQLROWSET * rs, SQLSTMT * s, ClipVar * ap)
{
   MS_ROWSET *rowset = (MS_ROWSET *) rs;
   MS_STMT *stmt = (MS_STMT *) s;
   int i;
   ...
   if (status)
      {
	 _clip_trap_err(mp, 0, 0, 0, subsys, ER_BADSTATEMENT, mysql_error(stmt->conn->conn));
	 return -1;
      }
...
char *
ms_getvalue(SQLROWSET * rs, int fieldno, int *len)
{
   MS_ROWSET *rowset = (MS_ROWSET *) rs;
...
void
ms_setvalue(SQLROWSET * rs, int fieldno, char *value, int len)
{
   MS_ROWSET *rowset = (MS_ROWSET *) rs;
   if (rowset->data[rowset->recno - 1][fieldno])
      {
...
ms_append(SQLROWSET * rowset)
{
   ...
   ((MS_ROWSET *) rowset)->data = realloc(((MS_ROWSET *) rowset)->data, sizeof(void *) * ((MS_ROWSET *) rowset)->lastrec);
...
int
ms_fetch(ClipMachine * mp, SQLROWSET * rs, int recs, ClipVar * eval, int every, ClipVar * ors)
{
   MS_ROWSET *rowset = (MS_ROWSET *) rs;
   MS_STMT *stmt = rowset->stmt;
   MYSQL_ROW row;
...
int
ms_start(ClipMachine * mp, SQLCONN * c, const char *p1, const char *p2)
{
   MS_CONN *conn = (MS_CONN *) c;
   int status;
...
int
ms_commit(ClipMachine * mp, SQLCONN * c)
{
   MS_CONN *conn = (MS_CONN *) c;
   int status;
...
int
ms_rollback(ClipMachine * mp, SQLCONN * c)
{
   MS_CONN *conn = (MS_CONN *) c;
   int status;
