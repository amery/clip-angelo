...
/* #ifdef OS_CYGWIN*/
       for (i = 0; i < ncolumns; i++)
	{
	   if (i < acol->count_of_ClipArrVar
	      ...
	      columns[i] =
	       _clip_locale_to_utf8(acol->ClipVar_items_of_ClipArrVar[i].ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.
				    buf_of_ClipBuf);
	   else
...
/*#else
			for(i=0; i < ncolumns; i++ )
			{
				if ( i < acol->count && acol->items[i].ClipType_t_of_ClipVar.ClipVartype_type_of_ClipType == CHARACTER_type_of_ClipVarType )
					columns[i] = acol->items[i].ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf;
				else
					columns[i] = empty_string;
			}
			wid = gtk_clist_new_with_titles(ncolumns,columns);
...
clip_GTK_CLISTMOVETO(ClipMachine * ClipMachineMemory)
{
   ...
   gint      xalign = _clip_parni(ClipMachineMemory, 4);

   gint      yalign = _clip_parni(ClipMachineMemory, 5);
