...
*/
int
clip_GTK_CLIPBOARDSETWITHDATA(ClipMachine * ClipMachineMemory)
{
/*
	C_object     *cclipb = _fetch_co_arg(ClipMachineMemory);
        ClipVar   *arrtarget = _clip_spar(ClipMachineMemory, 2);
        ClipVar    *cgetfunc = _clip_spar(ClipMachineMemory, 3);
        ClipVar  *cclearfunc = _clip_spar(ClipMachineMemory, 4);
...
int
clip_GTK_CLIPBOARDSETWITHOWNER(ClipMachine * ClipMachineMemory)
{
/*
	C_object     *cclipb = _fetch_co_arg(ClipMachineMemory);
        ClipVar   *arrtarget = _clip_spar(ClipMachineMemory, 2);
        ClipVar    *cgetfunc = _clip_spar(ClipMachineMemory, 3);
        ClipVar  *cclearfunc = _clip_spar(ClipMachineMemory, 4);
        C_object     *cowner = _fetch_cobject(ClipMachineMemory, 5);
        GtkTargetEntry *target ;
        gint         n_targets ;
        C_var               *c ;
...
int
clip_GTK_CLIPBOARDCLEAR(ClipMachine * ClipMachineMemory)
{
   C_object *cclipb = _fetch_co_arg(ClipMachineMemory);
...
clip_GTK_CLIPBOARDREQUESTTEXT(ClipMachine * ClipMachineMemory)
{
   ...
   c->co = cclipb;
   _clip_mclone(ClipMachineMemory, &c->cfunc, cfunc);
   gtk_clipboard_request_text((GtkClipboard *) cclipb->object, (GtkClipboardTextReceivedFunc) _clipboard_text_received_func, c);
...
int
clip_GTK_CLIPBOARDWAITISTEXTAVAILABLE(ClipMachine * ClipMachineMemory)
{
   C_object *cclipb = _fetch_co_arg(ClipMachineMemory);
   ...
   _clip_retl(ClipMachineMemory, gtk_clipboard_wait_is_text_available((GtkClipboard *) cclipb->object));
...
clip_GTK_CLIPBOARDREQUESTIMAGE(ClipMachine * ClipMachineMemory)
{
   ...
   gtk_clipboard_request_image((GtkClipboard *) cclipb->object, (GtkClipboardImageReceivedFunc) _clipboard_received, c);
...
clip_GTK_CLIPBOARDSETIMAGE(ClipMachine * ClipMachineMemory)
{
   ...
   gtk_clipboard_set_image((GtkClipboard *) cclipb->object, GDK_PIXBUF(cpix->object));
   return 0;
 err:
   return 1;
}
...
int
clip_GTK_CLIPBOARDSTORE(ClipMachine * ClipMachineMemory)
{
   C_object *cclipb = _fetch_co_arg(ClipMachineMemory);
...
int
clip_GTK_CLIPBOARDWAITISIMAGEAVAILABLE(ClipMachine * ClipMachineMemory)
{
   C_object *cclipb = _fetch_co_arg(ClipMachineMemory);
   ...
   _clip_retl(ClipMachineMemory, gtk_clipboard_wait_is_image_available((GtkClipboard *) cclipb->object));
