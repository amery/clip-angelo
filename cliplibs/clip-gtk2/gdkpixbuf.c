...
clip_GDK_PIXBUFSCALE(ClipMachine * ClipMachineMemory)
{
   ...
   gdk_pixbuf_scale(GDK_PIXBUF(cpxbsrc->object),
		    pixbuf, dest_x, dest_y, dest_width, dest_height, offset_x, offset_y, scale_x, scale_y, interp_type);
   if (pixbuf)
    {
       C_object *cpix;
...
clip_GDK_PIXBUFSAVE(ClipMachine * ClipMachineMemory)
{
   ...
   ret = gdk_pixbuf_save(GDK_PIXBUF(cpxb->object),
			 ...
			 list[12], list[13], list[14], list[15], list[16], list[17], list[18], list[19], NULL);
...
clip_GDK_PIXBUFCOMPOSITECOLORSIMPLE(ClipMachine * ClipMachineMemory)
{
   ...
   pixbuf = gdk_pixbuf_composite_color_simple(GDK_PIXBUF(cpxb->object),
					      dest_width, dest_height, type, overall_alpha, check_size, color1, color2);
   if (pixbuf)
    {
       cpixbuf = _list_get_cobject(ClipMachineMemory, pixbuf);
       if (!cpixbuf)
