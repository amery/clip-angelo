...
static    gint
handle_cycle_focus_signal(GtkWidget * widget, GtkDirectionType direction, C_signal * cs)
{
   PREPARECV(cs, cv);
   _clip_mputn(cs->cw->cmachine, &cv, HASH_DIRECTION, direction);
   INVOKESIGHANDLER(widget, cs, cv);
...
static SignalTable menushell_signals[] = {
   ...
   {"cycle-focus", GSF(handle_cycle_focus_signal), ESF(object_emit_signal),
    GTK_CYCLE_FOCUS_SIGNAL},
...
int
clip_GTK_MENUSHELLCANCEL(ClipMachine * ClipMachineMemory)
{
   C_widget *cmns = _fetch_cw_arg(ClipMachineMemory);
