...
{
   _wtype_table_put(_clip_type_check_menu_item,
		    _clip_type_name_check_menu_item, _gtk_type_check_menu_item, _gtk_type_container, check_menu_item_signals);
   return 0;
}
...
int
clip_GTK_CHECKMENUITEMNEWWITHMNEMONIC(ClipMachine * ClipMachineMemory)
{
   ClipVar  *cv = _clip_spar(ClipMachineMemory, 1);
...
int
clip_GTK_CHECKMENUITEMGETINCONSISTENT(ClipMachine * ClipMachineMemory)
{
   C_widget *citm = _fetch_cw_arg(ClipMachineMemory);

   CHECKCWID(citm, GTK_IS_CHECK_MENU_ITEM);
   _clip_retl(ClipMachineMemory, gtk_check_menu_item_get_inconsistent(GTK_CHECK_MENU_ITEM(citm->widget)));
   return 0;
 err:
   return 1;
}
...
int
clip_GTK_CHECKMENUITEMSETINCONSISTENT(ClipMachine * ClipMachineMemory)
{
   C_widget *citm = _fetch_cw_arg(ClipMachineMemory);

   gboolean  iconsistent = _clip_parl(ClipMachineMemory, 2);

   CHECKCWID(citm, GTK_IS_CHECK_MENU_ITEM);
   CHECKARG(2, LOGICAL_type_of_ClipVarType);
   gtk_check_menu_item_set_inconsistent(GTK_CHECK_MENU_ITEM(citm->widget), iconsistent);
   return 0;
 err:
   return 1;
}
...
int
clip_GTK_CHECKMENUITEMSETDRAWASRADIO(ClipMachine * ClipMachineMemory)
{
   C_widget *citm = _fetch_cw_arg(ClipMachineMemory);
...
int
clip_GTK_CHECKMENUITEMGETDRAWASRADIO(ClipMachine * ClipMachineMemory)
{
   C_widget *citm = _fetch_cw_arg(ClipMachineMemory);

   CHECKCWID(citm, GTK_IS_ITEM);

   _clip_retl(ClipMachineMemory, gtk_check_menu_item_get_draw_as_radio(GTK_CHECK_MENU_ITEM(citm->widget)));
