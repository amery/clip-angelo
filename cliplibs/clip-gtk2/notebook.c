...
/*********************** SIGNALS **************************/
gint
handle_change_current_page_signal(GtkNotebook * notebook, gint page_num, C_signal * cs)
{
   PREPARECV(cs, cv);
...
gint
handle_move_focus_out_signal(GtkNotebook * notebook, GtkDirectionType type, C_signal * cs)
{
   PREPARECV(cs, cv);
...
gint
handle_select_page_signal(GtkNotebook * notebook, gboolean select, C_signal * cs)
{
   PREPARECV(cs, cv);
...
static SignalTable notebook_signals[] = {
   {"change-current-page", GSF(handle_change_current_page_signal),
    ESF(object_emit_signal), GTK_CHANGE_CURRENT_PAGE_SIGNAL},
   {"move-focus-out", GSF(handle_move_focus_out_signal),
    ESF(object_emit_signal), GTK_MOVE_FOCUS_OUT_SIGNAL},
   {"select-page", GSF(handle_select_page_signal), ESF(object_emit_signal),
    GTK_SELECT_PAGE_SIGNAL},
...
int
clip_GTK_NOTEBOOKSETTABLABELTEXT(ClipMachine * ClipMachineMemory)
{
   C_widget *cntb = _fetch_cw_arg(ClipMachineMemory);
   C_widget *cchild = _fetch_cwidget(ClipMachineMemory, _clip_spar(ClipMachineMemory, 2));
   ...
   LOCALE_TO_UTF(text);
   gtk_notebook_set_tab_label_text(GTK_NOTEBOOK(cntb->widget), cchild->widget, text);
   FREE_TEXT(text);
   return 0;
 err:
   return 1;
...
int
clip_GTK_NOTEBOOKGETMENULABELTEXT(ClipMachine * ClipMachineMemory)
{
   C_widget *cntb = _fetch_cw_arg(ClipMachineMemory);
   C_widget *cchild = _fetch_cwidget(ClipMachineMemory, _clip_spar(ClipMachineMemory, 2));
   gchar *text;
   ...
   CHECKARG2(2, MAP_type_of_ClipVarType, NUMERIC_type_of_ClipVarType);
   CHECKCWID(cchild, GTK_IS_WIDGET);
   text = (gchar *) gtk_notebook_get_menu_label_text(GTK_NOTEBOOK(cntb->widget), GTK_WIDGET(cchild->widget));
   LOCALE_TO_UTF(text);
...
int
clip_GTK_NOTEBOOKGETSCROLLABLE(ClipMachine * ClipMachineMemory)
{
   C_widget *cntb = _fetch_cw_arg(ClipMachineMemory);

   CHECKCWID(cntb, GTK_IS_NOTEBOOK);
   _clip_retl(ClipMachineMemory, gtk_notebook_get_scrollable(GTK_NOTEBOOK(cntb->widget)));
   return 0;
 err:
   return 1;
}
...
int
clip_GTK_NOTEBOOKGETSHOWBORDER(ClipMachine * ClipMachineMemory)
{
   C_widget *cntb = _fetch_cw_arg(ClipMachineMemory);

   CHECKCWID(cntb, GTK_IS_NOTEBOOK);
   _clip_retl(ClipMachineMemory, gtk_notebook_get_show_border(GTK_NOTEBOOK(cntb->widget)));
   return 0;
 err:
   return 1;
}
...
int
clip_GTK_NOTEBOOKGETSHOWTABS(ClipMachine * ClipMachineMemory)
{
   C_widget *cntb = _fetch_cw_arg(ClipMachineMemory);

   CHECKCWID(cntb, GTK_IS_NOTEBOOK);
   _clip_retl(ClipMachineMemory, gtk_notebook_get_show_tabs(GTK_NOTEBOOK(cntb->widget)));
   return 0;
 err:
   return 1;
}
...
int
clip_GTK_NOTEBOOKGETTABLABELTEXT(ClipMachine * ClipMachineMemory)
{
   C_widget *cntb = _fetch_cw_arg(ClipMachineMemory);
   C_widget *cchild = _fetch_cwidget(ClipMachineMemory, _clip_spar(ClipMachineMemory, 2));
   gchar *text;
   ...
   CHECKARG2(2, MAP_type_of_ClipVarType, NUMERIC_type_of_ClipVarType);
   CHECKCWID(cchild, GTK_IS_WIDGET);
   text = (gchar *) gtk_notebook_get_tab_label_text(GTK_NOTEBOOK(cntb->widget), GTK_WIDGET(cchild->widget));
   LOCALE_TO_UTF(text);
   _clip_retc(ClipMachineMemory, text);
   FREE_TEXT(text);
   return 0;
...
int
clip_GTK_NOTEBOOKGETTABPOS(ClipMachine * ClipMachineMemory)
{
   C_widget *cntb = _fetch_cw_arg(ClipMachineMemory);

   CHECKCWID(cntb, GTK_IS_NOTEBOOK);
   _clip_retni(ClipMachineMemory, (int) gtk_notebook_get_tab_pos(GTK_NOTEBOOK(cntb->widget)));
   return 0;
 err:
   return 1;
}
...
int
clip_GTK_NOTEBOOKSETCURRENTPAGE(ClipMachine * ClipMachineMemory)
{
   C_widget *cntb = _fetch_cw_arg(ClipMachineMemory);
   gint page = _clip_parni(ClipMachineMemory, 2);
...
int
clip_GTK_NOTEBOOKGETNPAGES(ClipMachine * ClipMachineMemory)
{
   C_widget *cntb = _fetch_cw_arg(ClipMachineMemory);

   CHECKCWID(cntb, GTK_IS_NOTEBOOK);

   _clip_retni(ClipMachineMemory, gtk_notebook_get_n_pages(GTK_NOTEBOOK(cntb->widget)));
