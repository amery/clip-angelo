...
/* Signal handlers */
gint
handle_destroy_signal(GtkObject * obj, C_signal * cs)
{
...
/*printf("Widget name: %s, event: GDK_EXPOSE \n",gtk_widget_get_name(widget));*/
	 ...
	 //_clip_mclone(ClipMachineMemory, &stack[1], &area);
	  _clip_mputn(ClipMachineMemory, &stack[1], HASH_COUNT, event->expose.count);

	  cwin = _list_get_cobject(ClipMachineMemory, event->expose.window);
	  if (!cwin)
	     cwin = _register_object(ClipMachineMemory, event->expose.window, GDK_TYPE_WINDOW, NULL, NULL);
	  if (cwin)
	     _clip_madd(ClipMachineMemory, &stack[1], HASH_WINDOW, &cwin->obj);
	  _clip_destroy(ClipMachineMemory, &area);
	  break;
       }
       ...
       else
	{
	   ...
	   _clip_mputn(ClipMachineMemory, &stack[1], HASH_HARDWARE_KEYCODE, event->key.hardware_keycode);
	}
       break;
    default:
       break;
   ...
   _clip_destroy(ClipMachineMemory, &stack[1]);
  //return TRUE;
   return ret;
}
   ...
   if (_clip_parinfo(ClipMachineMemory, 2) == CHARACTER_type_of_ClipVarType)
    {
      //sig_table = _sig_table_by_name(cwid, _clip_parc(ClipMachineMemory,2));
       sig_table = _sig_table_by_name(cwid->type, _clip_parc(ClipMachineMemory, 2));
    }
   else
      sig_table = _sig_table_by_id(cwid->type, _clip_parni(ClipMachineMemory, 2));
   ...
   if (sig_table && sig_table->sigfunction)
       ...
       if ((sid == GTK_BUTTON_PRESS || sid == GTK_2BUTTON_PRESS || sid == GTK_3BUTTON_PRESS))
	{
	  //cwid->event_connected = TRUE;
	   if (after)
	      ret = g_signal_connect_after(GTK_OBJECT(cwid->widget), "button-press-event", GSF(sfunc), cs);
	  //ret = gtk_signal_connect_after(GTK_OBJECT(cwid->widget),
	  //      "button-press-event",GSF(sfunc),cs);
	   else
	      ret = g_signal_connect(GTK_OBJECT(cwid->widget), "button-press-event", GSF(sfunc), cs);
       ...
       if (sigfound && sid != GTK_BUTTON_PRESS)
	{
	   if (after)
	    {
	       ret = g_signal_connect_after(GTK_OBJECT(cwid->widget), signame, GSF(sfunc), cs);
	      //ret = gtk_signal_connect_after(GTK_OBJECT(cwid->widget),
	      //      signame,GSF(sfunc),cs);
	    }
	   else
	    {
	       ret = g_signal_connect(GTK_OBJECT(cwid->widget), signame, GSF(sfunc), cs);
	      //ret = gtk_signal_connect(GTK_OBJECT(cwid->widget),
	      //      signame,GSF(sfunc),cs);
	    }
	}
...
static int
_object_signal_connect(ClipMachine * ClipMachineMemory, gboolean after)
{
   ...
   if (_clip_parinfo(ClipMachineMemory, 2) == CHARACTER_type_of_ClipVarType)
    {
       sig_table = _sig_table_by_name(cobj->type, _clip_parc(ClipMachineMemory, 2));
    }
   else
      sig_table = _sig_table_by_id(cobj->type, _clip_parni(ClipMachineMemory, 2));
   ...
   if (sig_table && sig_table->sigfunction)
       ...
       if (sigfound)
	{
	   if (after)
	    {
	       ret = g_signal_connect_after(GTK_OBJECT(cobj->object), signame, GSF(sfunc), cs);
	    }
	   else
	    {
	       ret = g_signal_connect(GTK_OBJECT(cobj->object), signame, GSF(sfunc), cs);
	    }
	}
    }
...
clip_GTK_SIGNALCONNECT(ClipMachine * ClipMachineMemory)
{
   ...
   if (cwid->objtype == GTK_OBJ_WIDGET)
      ...
      return _object_signal_connect(ClipMachineMemory, FALSE);
...
clip_GTK_SIGNALCONNECTAFTER(ClipMachine * ClipMachineMemory)
{
   ...
   if (cwid->type != 0)
      ...
      return _object_signal_connect(ClipMachineMemory, TRUE);
   ...
   if (sig_table && sig_table->emitsigfunction)
    {
	  ...
	  ret = sig_table->emitsigfunction(_list_get_cobject(ClipMachineMemory, GTK_OBJECT(cwid->widget)), sig_table->signame);
      //int sigfound = gtk_signal_lookup(signame, GTK_WIDGET_TYPE(cwid->widget));
    }
