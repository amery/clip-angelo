 ...
 */
static char *
conv_10(wide_int num, bool_int is_unsigned, bool_int * is_negative, char *buf_end, int *len)
{
   char     *p = buf_end;
	   ...
	   switch (*fmt)
	    ...
	    case 'u':
		  ...
		  i_num = (wide_int) va_arg(ap, int);
	       ...
	       s = conv_10(i_num, (*fmt) == 'u', &is_negative, &num_buf[NUM_BUF_SIZE], &s_len);
	    ...
	    case 'E':
	       ...
	       s = conv_fp(*fmt, fp_num, alternate_form,
			   (adjust_precision == NO) ? FLOAT_DIGITS : precision, &is_negative, &num_buf[1], &s_len);
	       if (is_negative)
		  prefix_char = '-';
	       else if (print_sign)
		  prefix_char = '+';
