...
!strcasecmp(rowset->fields[i].name, vp->ClipStrVar_s_of_ClipVar.ClipBuf_str_of_ClipStrVar.buf_of_ClipBuf))
	 ...
	 if (rowset->fields[i].type == SQL_BINARY || rowset->fields[i].type == SQL_VARBINARY || rowset->fields[i].type == SQL_LONGVARBINARY)
...
clip_ODBC_IN_TIMESTAMP(ClipMachine * mp)
{
   ...
   if (str)
	 ...
	 ret->ClipDateTimeVar_dt_of_ClipVar.julian_of_ClipDateTimeVar = _clip_jdate(*((short *) str + 2), *((short *) str + 1), *((short *) str));
	 ret->ClipDateTimeVar_dt_of_ClipVar.time_of_ClipDateTimeVar = (*((short *) str + 3) * 60 * 60 * 1000) + (*((short *) str + 4) * 60 * 1000) + (*((short *) str + 5) * 1000);
      }
   return 0;
}
