...
IF EMPTY(lStart)
	IF _tally >= nLast
		...
		nLast+=m->_sx_step
		IF !EMPTY(m->_lMeter) THEN Meter(2,,_tally,nTotal)	//nTotal ⮫쪮 ��� SDF
	ENDIF
	_tally++
ELSE		//���㫥���
	nLast:=m->_sx_step
...
nFld:=Fseek(_handle,0,2)	//������ ��� �஢�ન
FSeek(_handle,0)
Fread(_handle,@cFld,28)
//��� ����, CDX_flag,CodePage
cNewType:=First(cFld)
nStart:=Bin2I(SUBSTR(cFld,9,2))		//��砫� ������
IF nStart>nFld .OR. nStart<65 .OR. nFld<65 THEN RETURN aStr	//����� �����-��

m->__RealFlds:=ReadBin(_handle,0,nStart-36,1)
lCpDBF:=BETWEEN(CPDBF(),1200,1299)
m->_aCommon:={lCpDBF .AND. !EMPTY(m->_lForced),;//������ �����஢����
	      cNewType,;			//�ਣ���� 1-�� �����
	      cNewType,;			//���������� 1-� ����
	      lCpDBF,;				//���� OemToAnsi
	      .F.;				//���� AutoInc
...
DO WHILE .T.	//����砭�� �����
	IF (Fread(_handle,@cFld,32) <>32) .OR.;
	   ...
	   (FSeek(_handle,0,1)>nStart) THEN EXIT	//����砭�� �����
...
IF !EMPTY(m->_lForced)
	...
	IF !InList(ASC(cNewType),3,131,245,48,49)	//��������� RDD
		FSeek(_handle,0)
...
LOCAL hBase
MemVar _aCommon
IF VALTYPE(_aCommon)=='A' .AND. !EMPTY(_aCommon[1])	//ValType ��-�� ������ ����
	hBase:=FOPEN(cBase, 2)
	FSeek(hBase,0)
