...
MEMVAR _DefaultRDD,_DefaultMemo,_DefaultCDX,__ContentType,_fc,;
	...
	_aDbStruct,__adBStruct,_CDXName,__aPrev,cOrig,cValType, ;
	_tmp, _base, _HtmlFile, _XlsFile, _XmlFile, _IniPFile, _A, _IniFile, __CDXName
...
m->_ClipWText:={}       //����� ���⥬�
m->_ClipWChoice:=.F.	//�᫨ .T. - �㤥� �।������ �롮�.
m->_lCnvWClip:=.F.	//�८�ࠧ��뢠�� ��� ����஢���� � �����
...
DO CASE
	...
	CASE _i>5
		...
		ENDIF
END
InsString(m->GetList[1],ALLTRIM(cKb))
**********
FUNC Xbrowse
  ...
  _s:=''
   DO While FreadLn(_h, @_s1, 512)
	...
	IF EMPTY(_s) .OR. First(_s) $ '*;/'
		...
		LOOP
	ENDIF
	_s:=Translate_Charset(m->cCodePage, host_charset(), _s)
	*Parce(_s,'/*',@_s)
...
ELSE
  WHILE .T.
	m->_aDbStruct:=RealStruct(_handle,@NeedForced)	//��६����� ��������� (������쭠�)
...
#else
	IF !(cFirstByte $ '�����') .OR. !EMPTY(NeedForced)
#endif
	  IF !EMPTY(NeedForced)
  ...
  IF EMPTY(lExit)
	...
	CASE m->_aCommon[2] $ chr(245)	//'�' DBFCDX
		IF UPPER(Substr(cName,-4))<>'.DBF';
			THEN _memo:='.'+SUBSTR(cName,-3,2)+'T'
		i:=1

	CASE cFirstByte $ chr(131)	//'�'	DBFNTX
		i:=2
...
//�-� � ini ����୮ �����
    ...
    lNeedMemo:=(cFirstByte $ chr(245)+chr(131)+chr(48)+chr(49))	//'��01'
    ...
    ENDIF
    cFirstByte:=SUBSTR(chr(245)+chr(131)+CHR(48)+CHR(49),i,1)	//��� ���������� ���ࠢ����� MEMO
