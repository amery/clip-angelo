...
IF EMPTY(nAuto)
	IF BETWEEN(;
	   ...
	   ASCAN(_Pole,m->__aDbStruct[_i,1])<>0
		KEYBOARD(_CTRLHOME+REPL(_RIGHT,_i-1))	//������ �� ����
	ENDIF
ELSE
	EVAL(aAct[nAuto,2])
ENDIF
...
IF IndexCount()>=1 .AND. Sx_TagCount(1)=0	//�� � ⥪�饬 ��⠫��� ��� MDX
...
BEGIN SEQUENCE
  DO CASE
	...
	ENDIF
	AchKeys(.F.)
	IF !EMPTY(m->lAgain) THEN SetTag(3)	//�᫨ ��������
  ENDCASE
END
__tagNom:=Sx_TagNo()
__tagName:=OrdName()
...
BEGIN SEQUENCE
	...
	FERASE(__CdxName)
	MainUse(M_EXCL,.T.)
	OrdSetFocus(1,__CdxName)	//�����-�� �㦥� ��� Kill
*	Sx_KillTag(.t.)
...
IsTags()
IF IndexCount()>=1 .AND. Sx_TagCount()=0	//�� � ⥪�饬 ��⠫��� ��� MDX
	OrdSetFocus(1,__CdxName)
ENDIF
...
BEGIN SEQUENCE
	IF IsShift().AND.!EMPTY(cSrch)	// � Shift- �த������� ���᪠
		SKIP
...
IF (nSearch:=Len(cSearch))#0
  ...
  DO WHILE InKey()#K_ESC .AND. (nSize - nCpos)>nsearch
	...
	IF (nAt:=AT(cSearch,cString))#0
		...
		IF MGoTo(nAtPos) .AND. Eval(Compile(cAddCond))	// ������� !
...
IF Between((_i:=WaitKey(20)-47),1,10)
	...
	ELSE
		Mgoto(m->_StackPos[_i],.t.) //THEN go _tmr - � ᠬ�� MgoTo
	ENDIF
ELSE
...
ELSEIF _i==2 .AND. GetName(cMsg,'_ksR',,cPictRus) .AND.;
	...
	nHl:=12
ELSEIF _i==3 .AND. GetName('��� (5-������)','_kmfo',,'99999')
	cMsg:=_i:=vkrz1(m->_kmfo)
ELSEIF _i==4 .AND. GetName('���� (8-������)','_kokpo',,'99999999')
...
IF GetName(_ZU+_txt+IF(EMPTY(lRest),EMPTY_IS_ALL,''),_varu) .AND.;
	...
	*ScrSave()
	...
	#IFNDEF ENGLISH
		IF (cSkl:=Substr(_txt,-1)) $ '��'
			_txt:=CHG_EXT(_txt,1,IF(cSkl='�','�','�'))
		ELSE
