...
LOCAL prsc1, j
   IF .NOT. EMPTY( var1 ) .AND. .NOT. EMPTY( var2 )
      ...
      IF m->kolsc > 0
         FOR j := 1 TO m->kolsc
            IF var1 = m->MSSCH[ j ]
               STORE m->MSSUM1[ j ] + var2 TO m->MSSUM1[ j ]
               prsc1 := .T.
               EXIT
            ENDIF
         NEXT
      ENDIF
      IF .NOT. prsc1
         IF m->kolsc == LEN( m->MSSCH )
            AADD( m->MSSCH, Nil )
            AADD( m->MSSUM1, Nil )
         ENDIF
         STORE m->kolsc + 1 TO m->kolsc
         m->MSSCH[ m->kolsc ] = var1
         m->MSSUM1[ m->kolsc ] = var2
      ENDIF
   ENDIF
RETURN var1
...
LOCAL prsc1, j
   IF .NOT. EMPTY( var1 )               // .AND.(.NOT.EMPTY(var2).OR..NOT.EMPTY(var3))
      ...
      IF m->kolnam > 0
         FOR j := 1 TO m->kolnam
            IF var1 = m->MSNAIM[ j ]
               STORE m->MSSM1[ j ] + var2 TO m->MSSM1[ j ]
               IF var3 <> Nil
                  STORE m->MSSM2[ j ] + var3 TO m->MSSM2[ j ]
               ENDIF
               prsc1 := .T.
               EXIT
            ENDIF
      ...
      ENDIF
      IF .NOT. prsc1
         IF m->kolnam == LEN( m->MSNAIM )
            AADD( m->MSNAIM, Nil )
            AADD( m->MSSM1, Nil )
            AADD( m->MSSM2, Nil )
         ENDIF
         STORE m->kolnam + 1 TO m->kolnam
         m->MSNAIM[ m->kolnam ] = var1
         m->MSSM1[ m->kolnam ] = var2
         IF var3 <> Nil
            m->MSSM2[ m->kolnam ] = var3
         ENDIF
      ENDIF
   ENDIF
RETURN var1
...
LOCAL i, rsum
   ...
   FOR i := 1 TO m->kolnam
      IF m->MSNAIM[ i ] = fsch
         rsum := rsum + IIF( nsum = 1,m->MSSM1[ i ], m->MSSM2[ i ] )
      ENDIF
   NEXT
RETURN rsum
...
RETURN IIF( m->rou_dec = 0, TRANSFORM( vnum, "999 999 999 999" ), TRANSFORM( vnum, "999 999 999.99" ) )
...
LOCAL rez
   IF PCOUNT() = 1
      date2 := m->tdate
   ENDIF
   date2 := CTOD( "01/" + PADL( IIF( m->kvartal, ( INT( ( MONTH( date2 ) - 1 ) / 3 ) ) * 3 + 1, MONTH( date2 ) ), 2, "0" ) + SUBSTR( DTOC( date2 ), 6, 3 ) )
   IF m->kvartal
...
LOCAL mssot  := { "���", "������", "������", "���������", "�������", "��������", "ᥬ����", "��ᥬ����", "���������" }
LOCAL msdes  := { "��������", "�ਤ����", "��ப", "���줥���", "����줥���", "ᥬ줥���", "��ᥬ줥���", "���ﭮ���" }
LOCAL ms1des := { "������", "�����������", "����������", "�ਭ������", "����ୠ�����", "��⭠�����", "���⭠�����", "ᥬ�������", "��ᥬ�������", "����⭠�����" }
LOCAL msed   := { "����", "���", "���", "������", "����", "�����", "ᥬ�", "��ᥬ�", "������" }
LOCAL msed1  := { "����", "���" }
LOCAL msrod  := { .T., .T., .F., .T. }
   ...
   IF kod <> 0
      ...
      IF valut
         strrez := TRIM( strrez ) + " " + SUBSTR( chislo, len + 2, 2 ) + IIF( ( kod > 9 .AND. kod < 21 ) .OR. skop = 0 .OR. skop > 4, " 業⮢", IIF( skop = 1, " 業�", " 業��" ) )
      ELSE
         strrez := TRIM( strrez ) + " " + SUBSTR( chislo, len + 2, 2 ) + IIF( ( kod > 9 .AND. kod < 21 ) .OR. skop = 0 .OR. skop > 4, " ������", IIF( skop = 1, " �������", " �������" ) )
      ENDIF
   ELSE
      strrez := TRIM( strrez ) + "="
   ENDIF
...
LOCAL i, strtmp, vname, poz1
   STORE SPACE( rlen ) TO m->part2, m->part3, m->part4, m->part5
   str := TRIM( str )
   FOR i := 1 TO 5
      vname := "part" + STR( i, 1 )
      IF LEN( str ) <= rlen
...
FUNCTION DateY
   ...
   RETURN IIF( YEAR( DATE() ) < 2000, Addmonth( DATE(), IIF( TYPE( "add_mnth" ) = "N", m->add_mnth, 0 ) ), DATE() )
