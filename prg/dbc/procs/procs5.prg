...
LOCAL mslist := ARRAY( LI_LEN )
   ...
   LI_BEOF    := { | a | FEOF( a ) }
   LI_BBOF    := { | a | FBOF( a ) }
   LI_B1      := { | a | DEVPOS( LI_Y2, LI_X1 + 2 ), DEVOUT( IIF( LI_PRFLT, "������" + STR( LI_TEKZP, 5 ), STR( RECNO(), 6 ) ) + "/" + STR( LI_KOLZ, 6 ) ) }
   LI_FREEZE  := 0
   LI_RCOU    := { || RECCOUNT() }
   LI_MSREC   := Nil
   LI_RECNO   := { || RECNO() }
...
FUNCTION FGOBOT( mslist )
   ...
   IF LI_PRFLT
      LI_TEKZP := LI_KOLZ
      GO IIF( LI_KOLZ < m->klrecf, LI_MSREC[ LI_KOLZ ], LI_MSREC[ m->klrecf ] )
   ELSE
      GO BOTTOM
   ENDIF
RETURN Nil
