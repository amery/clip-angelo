...
C_FLAGS="-Wall -fPIC -I$CLIPROOT/include $DEBUGFLAGS $OPTFLAGS"
...
echo "DESTDIR=$DESTDIR"																				>&3
echo "EXESUFF=$EXESUFF" 																			>&3
echo "DLLREALSUFF=$DLLREALSUFF" 																	>&3
echo "Makefile_end_in=$Clip_M_Dir/init/Makefile.end.in"									>&3
echo "Makefile_o_in=$Clip_M_Dir/init/Makefile.o.in" 										>&3
echo "Makefile_02_in=$Clip_M_Dir/init/Makefile.02.in" 									>&3
echo "Makefile_end_in=$Clip_M_Dir/init/Makefile.end.in"									>&3
echo "CONFIGURE_SH=/$Clip_M_Dir/init/configure.sh"											>&3
echo "Clip_M_Dir=$Clip_M_Dir"																		>&3
...
if [ "$USE_TASKS" = yes -a ! -f libcliptask/task.o ]
then
	cd libcliptask
	./cl_conf
...
if [ "$USE_TASKS" = yes -a -f libcliptask/USE_PTH ]
then
...
then
	...
	echo "TASK=libcliptask/libcliptask.a"														>&3
fi
...
echo "ICONV_INC=$ICONV_INC"   																	>&3
echo "ICONV_LIB=\"$ICONV_LIB\""																	>&3
echo "PO_BINS=\"$PO_BINS\""																		>&3
...
echo export `cat Makefile.incl | grep "^[A-Z_][A-Z_]*=" | cut -d"=" -f1` > Makefile.export
...
cp -f$V Makefile.ini ../
mkdir -p$V $CLIPROOT/include
cp -f$V Makefile.ini $CLIPROOT/include/Makefile.ini
...
exec																													3>temp$$1$$2
echo "#define CLIP_VERSION \"`cat tools/release_version`\""											>&3
