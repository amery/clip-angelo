...
#		echo "#                                    release sequence : $release_sequence$SpaceReleaseSequence         #" 	>&0
		...
		if [ $trt -lt 0 ] || [ $trt -gt $MenuNr ] ; then
			...
			#if [ $? = 0 ] ; then
				#banner "$Parameter ${Menu_command[$varX]}  done."				>&0
				#read -p "Press <enter> to continue " varX						>&0
			#else
				#banner "$Parameter ${Menu_command[$varX]} error(s)"			>&0
			#fi
			#let trt=-1
			...
			while [[ $varX -lt 0 ]] || [[ $varX -gt 4 ]] ; do
				...
				OnScreen 10
				echo "#####################################"			>&0
				echo "#   [0]  |  cancel/abort            #"			>&0
				echo "#####################################"			>&0
				echo "#   [1]  |  Uninstall 'local'       #"			>&0
				echo "#   [2]  |  Uninstall 'home'        #"			>&0
				echo "#   [3]  |  Uninstall 'usr.local'   #"			>&0
				echo "#   [4]  |  Uninstall 'opt'         #"			>&0
				echo "#   [5]  |  Uninstall 'sys'         #"			>&0
				echo "#####################################"			>&0
				echo ""															>&0
				read -p "Your choice + <enter> : " varX 				>&0
			done
